...
$('#phlebo_shedule_date').change(function () {
    ...
    $.ajax({
        url: base_url() + 'phlebo-api_v4/get_phlebo_schedule',
        type: 'post',
        data: {bdate: this.value, city_id:b_city},
        beforeSend: function () {
...
$("#add_family_member").click(function () {
    ...
    if (f_name.trim() != '' && f_relation.trim() != '' && f_dob.trim() != '') {
        f_cnt = 0;
        if (f_mobile.trim() != '') {
        ...
        if (f_cnt == 0) {
            $.ajax({
                url: base_url() + 'phlebo-api_v4/add_family_member1',
                type: 'post',
                data: {uid: uid, name: f_name, relation_fk: f_relation, email: f_email, phone: f_mobile,f_dob:f_dob},
                beforeSend: function () {
        ...
        $("#f_error").html("Name and relation and birth date are required.");
    }
});
...
function info_validation() {
    ...
    if (validation_step == 1) {
        if ($("input[name='type_user_family']:checked").length > 0) {
            ...
            if (typ == "family") {
                ...
                if (crelation != '') {
                    ...
                    $("#contener_1_error").html("Select family member.");
                }
            } else {
                $("#contener_1").attr("style", "display:none;");
                $("#contener_2").attr("style", "");
            ...
            $("#contener_1_error").html("Select type.");
        }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (jsondata.status == 0) {
                            ...
                            window.location.href = base_url() + "user_test_master/book_test/" + book_test_id + "/" + jsondata.data[0].id;
                        }
                        $("#contener_3_error").html('');
                    },
                    error: function (jqXhr) {
