...
LEFT JOIN `job_master_receiv_amount` jmr 
 ...
 WHERE   jm.`status` != '0' ";
    ...
    function job_received() {
        ...
        $this->load->view('header', $data);
        $this->load->view('nav', $data);
        $this->load->view('dailycollection_received_report', $data);
        $this->load->view('footer');
    }
    ...
    function export_csv_new() {
        ...
        if (($data["report"] == 'details') && ($start_date != '' || $end_date != '')) {
            $output[] = array("PATIENT NAME" => 'PATIENT NAME', "JOB ID" => "JOB ID", "ORDER ID" => "ORDER ID","BRANCH"=>"BRANCH","TEST CITY"=>"TEST CITY", "ADDED BY" => "ADDED BY", "DOCTOR" => "DOCTOR", "ORDER DATE" => "ORDER DATE", "AMOUNT PAID" => "AMOUNT PAID", "MODE OF PAYMENT" => "MODE OF PAYMENT", "RECEIVER NAME" => "RECEIVER NAME", "AMOUNT COLLECTION DATE" => "AMOUNT COLLECTION DATE", "REMARK" => "REMARK");
    ...
    function export_csv_new1() {
        ...
        if (($data["report"] == 'details') && ($start_date != '' || $end_date != '')) {
            ...
            print_r($data['view_all_data']); die("OK");
            if ($data["payment"] == 'CREDITORS' || $data["payment"] == 'CASH' || $data["payment"] == 'all') {
                if ($data["payment"] == 'CREDITORS') {
                    $data['view_all_data'] = array();
                }
