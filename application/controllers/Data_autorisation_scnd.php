...
class Data_autorisation_scnd extends CI_Controller {
    ...
    function index() {
        ...
        if (!empty($branch1)) {
            $cntr_arry = array($branch1);
        }

        $branch = implode(",", $cntr_arry);
        $data["user"] = $this->user_model->getUser($data["login_data"]["id"]);
        $data['success'] = $this->session->flashdata("success");
        $totalRows = $this->data_autorize_scnd_model->autho_list_count($start_date, $end_date, $cntr_arry);
        ...
        $data["query"] = $this->data_autorize_scnd_model->autho_list($config["per_page"], $page, $start_date, $end_date, $cntr_arry);
    ...
    function get_data_parameter() {
        ...
        $barc = $this->input->get_post('barcode');
        ...
        $barcd_test = $this->data_autorize_scnd_model->get_val("SELECT 
  `test_sample_barcode`.* 
FROM
  `test_sample_barcode` 
  INNER JOIN `job_master` 
    ON `job_master`.`id` = `test_sample_barcode`.`job_fk` 
WHERE `test_sample_barcode`.barcode = '" . $barc . "' 
  AND `test_sample_barcode`.`status` = '1' 
  AND `job_master`.`status` IN (7, 8) 
  AND `job_master`.`branch_fk` = '" . $branch . "' ");

        foreach ($barcd_test as $kry) {
            if (!in_array($kry["test_fk"], $tid)) {
                $tid[] = $kry["test_fk"];
            }
        }
        ...
        foreach ($data as $all) {
            ...
            <td><input type="text" value="' . $all->result_value * $all->multiply_by . '" name="paravalue_' . $cnt . '">
                <input type="hidden" name="add_values_id_' . $cnt . '" value="' . $all->id . '"></td>
                <input type="hidden" name="job_id_' . $cnt . '" value="' . $job_details[0]->id . '"></td>
                <input type="hidden" name="parameter_id_' . $cnt . '" value="' . $all->parameter_id . '"></td>
                <input type="hidden" name="count" value="' . $cnt . '"></td>
            </tr>';
            $cnt++;
    ...
    function accept_results_new() {
        ...
        for ($i = 1; $i <= $count; $i++) {
            ...
            if($_GET["debug"]==1){
                echo "<pre>";print_r($_POST);print_r($tid);die("ok");
            }
            foreach ($tid as $test) {
