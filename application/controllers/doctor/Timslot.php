...
class Timslot extends CI_Controller {
  ...
  function index() {
    ...
    for($i=0; $slot <= $end_time; $i++) {
		...
		$timeslotid=$this->doctor_timeslotmodel->master_fun_insert("doctor_timeslot",array("doctor_fk"=>$doctor,"weekend"=>$s,"start_time"=>date('H:i:s', $start_time),"end_time"=>date('H:i:s', $slot),"createddate"=>date("Y-m-d H:i:s")));
		...
		if($start_time > strtotime("24:00:99") && $start_time < strtotime("09:59:59") || $start_time > strtotime("13:59:59") && $start_time < strtotime("16:59:55") || $start_time > strtotime("20:59:59") && $start_time < strtotime("23:59:00")){
			...
			$this->doctor_timeslotmodel->master_fun_insert("doctor_deleteslot",array("doctorid"=>$id,"slotid"=>$timeslotid,"cretedate"=>date("Y-m-d H:i:s")));
         
		 }
		
		if(date('H:i:s', $slot)=="23:45:00"){ 

		$timeslotid1=$this->doctor_timeslotmodel->master_fun_insert("doctor_timeslot",array("doctor_fk"=>$doctor,"weekend"=>$s,"start_time"=>date('H:i:s',$slot),"end_time"=>date('H:i:s',strtotime(date('H:i:s',$slot) . ' +15 minutes')),"createddate"=>date("Y-m-d H:i:s")));
		...
		if($start_time1 > strtotime("24:00:99") && $start_time1 < strtotime("09:59:59") || $start_time1 > strtotime("13:59:59") && $start_time1 < strtotime("16:59:55") || $start_time1 > strtotime("20:59:59") && $start_time1 < strtotime("23:59:00")){
			...
			$this->doctor_timeslotmodel->master_fun_insert("doctor_deleteslot",array("doctorid"=>$id,"slotid"=>$timeslotid1,"cretedate"=>date("Y-m-d H:i:s")));
	...
	$checkdock=$id; 
        ...
        $this->load->view('doctor/d_slotlist', $data);
...
public function slotupdate() {
	...
	if($dslot != null){
$doctdslot=implode(",",$dslot);		
	$doctdeslot=$this->doctor_timeslotmodel->get_val("SELECT id,start_time FROM `doctor_timeslot` WHERE id IN($doctdslot) and doctor_fk='0' AND status='1'");
foreach($doctdeslot as $dot){
	
	$getnewdslot=$this->doctor_timeslotmodel->fetchdatarow("id","doctor_timeslot",array("doctor_fk"=>$doctor,"status"=>'1',"TIME(start_time)"=>$dot->start_time));
	
	if($getnewdslot != null){
	
	$this->doctor_timeslotmodel->master_fun_insert("doctor_deleteslot",array("doctorid"=>$doctor,"slotid"=>$getnewdslot->id,"cretedby"=>$data["login_data"]["id"],"cretedate"=>date("Y-m-d H:i:s")));  
	...
	if($getnewdslot1 != null){
		...
		$this->doctor_timeslotmodel->updateRowWhere("doctorbook_slot",array("doctorfk"=>$doctor,"dslotfk"=>$bookslot->dslotfk,"status"=>'1'),array("dslotfk"=>$getnewdslot1->id,"updated_by"=>$data["login_data"]["id"],"updateddate"=>date("Y-m-d H:i:s")));
