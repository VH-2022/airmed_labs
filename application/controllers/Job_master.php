...
class Job_master extends CI_Controller
{
    ...
    function pending_list_search()
    {
        ...
        foreach ($data['query'] as $key) {
            ...
            $data['query'][$cnt]["doctor_name"] = $doctor_data[0]["full_name"];
            $data['query'][$cnt]["doctor_mobile"] = $doctor_data[0]["mobile"];
            $data['query'][$cnt]["pro_name"] = $pro_data[0]["pro_name"];
            $package_ids = $this->job_model->get_job_booking_package($key["id"]);
    ...
    function pending_list_search_dev()
    {
        ...
        //total rows count
        $totalRec = 0; // count($this->job_model->new_row_srch_job_list_count($search_data, $config["per_page"], $page));
    ...
    function job_details()
    {
        ...
        if ($this->session->userdata("amount_history_error")) {
            $data['amount_history_error'] = $this->session->userdata("amount_history_error");
            $this->session->unset_userdata("amount_history_error");
        }
        ...
        $data["cancel_entry_user"] = $this->job_model->get_val("SELECT id,name,cancel_entery_whatsapp,phone FROM `admin_master` WHERE status !='0' AND cancel_entery_whatsapp = '1'");
    ...
    function send_otp_user(){
        ...
        $getUser = $this->job_model->get_val("
            SELECT phone FROM admin_master
            WHERE status!='0'
            AND cancel_entery_whatsapp='1'
            AND id='".$user_id."'
        ");
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://live-server-1110.wati.io/api/v1/sendTemplateMessage?whatsappNumber='.$phoneNo,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_CUSTOMREQUEST => 'POST',
            CURLOPT_POSTFIELDS => json_encode([
                "template_name" => "user_send_otp_04",
                "broadcast_name" => "Film Delivered to Patient - ".$phoneNo,
                "parameters" => [
                    ["name"=>"1","value"=>$otp]
                ]
            ]),
        ...
        if(isset($responseData['result']) && $responseData['result']==1){
            ...
            $exist = $this->job_model->get_val("SELECT * FROM cancel_entry_otp WHERE job_id='".$job_id."'");

            if(empty($exist)){
                $this->job_model->master_fun_insert("cancel_entry_otp",[
                    "job_id"=>$job_id,
                    "user_id"=>$user_id,
                    "otp"=>$otp,
                    "status"=>"pending"
                ...
                $this->job_model->master_fun_update_multi("cancel_entry_otp",
                    ["job_id"=>$job_id],
                    ["otp"=>$otp,"user_id"=>$user_id,"status"=>"pending"]
                );
            }
        ...
        echo json_encode(["status"=>"error","message"=>"WhatsApp OTP failed"]);
        exit;
    }

    function verify_otp_user(){
        $data["login_data"] = logindata();
        $data["user"] = $this->user_model->getUser($data["login_data"]["id"]);
        $job_id = $this->input->post('job_id');
        $otp    = $this->input->post('otp');

        $check = $this->job_model->get_val("
            SELECT * FROM cancel_entry_otp
            WHERE job_id='".$job_id."'
            AND otp='".$otp."'
            AND status='pending'
        ");
        ...
        if(!empty($check)){
            $this->job_model->master_fun_update_multi(
                "cancel_entry_otp",
                ["job_id"=>$job_id],
                ["status"=>"verified"]
            );
            ...
            $this->job_model->master_fun_update_multi(
                "job_master",
                ["id"=>$job_id],
                ["status"=>10,"payable_amount" => 0,"price" => 0]
            );

            $this->job_model->master_fun_insert("job_log", array("job_fk" => $job_id, "created_by" => "", "updated_by" => $data["login_data"]["id"], "deleted_by" => "", "job_status" => $customer_last_job_id[0]["status"] . "-" . 10, "message_fk" => "3", "date_time" => date("Y-m-d H:i:s")));
  ...
  END AS col1  FROM `parameter_referance_range` WHERE STATUS='1' AND `parameter_fk`='" . $para_key["id"] . "'";
    ...
    function export_csv()
    {
        ...
        if (!isset($_REQUEST['de'])) {
            ...
            fputcsv($handle, array("No.", "Reg No.", "Order Id", "Test City", "Branch", "Date", "Patient Name", "Mobile No.", "Doctor", "Doctor_code", "Doctor_Mobile", "PRO", "Speciality", "Special Discount", "Test/Package Name", "Test Type", "Job Status", "Payment Type", "Sample From", "New Patient/old Patient", "Portal", "Remark", "Added By", "Total Price", "Discount(RS.)", "Received Amount", "Cash", 'Card', "Payumoney", "CHEQUE", "Debited From Wallet", "Creditor Remain", "Due Amount", "Sample Received Date", "Sample Received Time", "Processing Date", "Processing Time", "Completed Time", "Report Generated Time", "Phlebo TAT (In Hour)", "Processing TAT (In Hour)", "Max TAT (In Hour)", "Processing Delay (In Hour)", "Collection", "IPD", 'Age', 'Outsource'));
        }
        /* Nishit code start */
        $cnt = 1;
        // $s_d = explode("/", $search_data["date"]);
        // $s_date = $s_d[2] . "-" . $s_d[1] . "-" . $s_d[0];

        // $end_d = explode("/", $search_data["end_date"]);
        ...
        foreach ($new_array as $key) {
            ...
            if ($key["is_show"] == 1 ) {
                ...
                if (!isset($_REQUEST['de'])) {
                    ...
                    fputcsv($handle, array($cnt, $key["id"], $key["order_id"], $key["test_city_name"], $key["branch_name"], $key["date"], $patient_name, $key["mobile"], $key["doctor_name"], $key["doc_code"], $key["doctor_mobile"], (empty($key["pro"]) ? 'NO PRO MEETING' : $key["pro"]), $key["speciality"], $key["ra"] . "%", $key["test_name"], $key["test_type"], $j_status, $key["payment_type"], $key["sample_from"], $formpatient, $key["portal"], $key["note"], $added_by, $key["price"], $discount, $due, $cash, $card, $payumoney, $cheque, $dabitt_from_wallet[0]["dabit"], $creditor_cash_due, $key["payable_amount"], $sample_date, $sample_time, $processing_date, $processing_time, $completed_time1, $report_generate_time1, $phlebo_tat, $processing_tat, $max_tat, $is_processing_delay, $key["amountreceived"], $key["ipd"], $ageShow, $outsource));
                }
                $cnt++;
            }
        }
    ...
    function export_csv_123()
    {
        ...
        if (!isset($_REQUEST['de'])) {
            ...
            //fputcsv($handle, array("No.", "Reg No.", "Order Id", "Test City", "Branch", "Date", "Patient Name", "Mobile No.", "Doctor", "Test/Package Name", "Test Type", "Job Status", "Payment Type", "Sample From", "New Patient/old Patient", "Portal", "Remark", "Added By", "Total Price", "Discount(RS.)", "Received Amount", "Cash", 'Card', "Payumoney", "CHEQUE", "Debited From Wallet", "Creditor Remain", "Due Amount", "Sample Received Date", "Sample Received Time", "Processing Date", "Processing Time", "Completed Time", "Report Generate Date", "Report Generate Time", "Phlebo TAT (In Hour)", "Processing TAT (In Hour)", "Max TAT (In Hour)", "Processing Delay (In Hour)"));
            //fputcsv($handle, array("No.", "Reg No.", "Order Id", "Test City", "Branch", "Date", "Patient Name", "Mobile No.", "Doctor", "Doctor_code","Doctor_Mobile", "PRO", "Speciality","Special Discount", "Test/Package Name", "Test Type", "Job Status", "Payment Type", "Sample From", "New Patient/old Patient", "Portal", "Remark", "Added By", "Total Price", "Discount(RS.)", "Received Amount", "Cash", 'Card', "Payumoney", "CHEQUE", "Debited From Wallet", "Creditor Remain", "Due Amount", "Sample Received Date", "Sample Received Time", "Processing Date", "Processing Time", "Completed Time", "Report Generated Time", "Phlebo TAT (In Hour)", "Processing TAT (In Hour)", "Max TAT (In Hour)", "Processing Delay (In Hour)"));
            fputcsv($handle, array("No.", "Reg No.", "Order Id", "Test City", "Branch", "Date", "Patient Name", "Mobile No.", "Doctor", "Doctor_code", "Doctor_Mobile", "PRO", "Speciality", "Special Discount", "Test/Package Name", "Test Type", "Job Status", "Payment Type", "Sample From", "New Patient/old Patient", "Portal", "Remark", "Added By", "Total Price", "Discount(RS.)", "Received Amount", "Cash", 'Card', "Payumoney", "CHEQUE", "Debited From Wallet", "Creditor Remain", "Due Amount", "Sample Received Date", "Sample Received Time", "Processing Date", "Processing Time", "Completed Time", "Report Generated Time", "Phlebo TAT (In Hour)", "Processing TAT (In Hour)", "Max TAT (In Hour)", "Processing Delay (In Hour)", "test name", "test price", "test disc %", "test disc", "billing", "total disc", "total billing", "IPD", 'Age', "Outsource"));
        }
        /* Nishit code start */
        $cnt = 1;
                    ...
                    if (count($test_codes) > 0) {
                        foreach ($test_codes as $code) {
                            ...
                            if ($cnt == count($test_codes))
                                fputcsv($handle, array($cnt, $key["id"], $key["order_id"], $key["test_city_name"], $key["branch_name"], $key["date"], $patient_name, $key["mobile"], $key["doctor_name"], $key["doc_code"], $key["doctor_mobile"], (empty($key["pro"]) ? 'NO PRO MEETING' : $key["pro"]), $key["speciality"], $key["ra"] . "%", $key["test_name"], $key["test_type"], $j_status, $key["payment_type"], $key["sample_from"], $formpatient, $key["portal"], $key["note"], $added_by, $key["price"], $discount, $due, $cash, $card, $payumoney, $cheque, $dabitt_from_wallet[0]["dabit"], $creditor_cash_due, $key["payable_amount"], $sample_date, $sample_time, $processing_date, $processing_time, $completed_time1, $report_generate_time1, $phlebo_tat, $processing_tat, $max_tat, $is_processing_delay, $code["test_name"], $code["p1"], $code["code1"], $code["code1"] > 0 ? ($code["p1"] * $code["code1"] / 100) : 0, $code["code1"] > 0 ? $code["p1"] - ($code["p1"] * $code["code1"] / 100) : $code["p1"], $t_dsc, $t_bil, $key["ipd"], $ageShow, $outsource));
                            else
                                fputcsv($handle, array($cnt, $key["id"], $key["order_id"], $key["test_city_name"], $key["branch_name"], $key["date"], $patient_name, $key["mobile"], $key["doctor_name"], $key["doc_code"], $key["doctor_mobile"], (empty($key["pro"]) ? 'NO PRO MEETING' : $key["pro"]), $key["speciality"], $key["ra"] . "%", $key["test_name"], $key["test_type"], $j_status, $key["payment_type"], $key["sample_from"], $formpatient, $key["portal"], $key["note"], $added_by, $key["price"], $discount, $due, $cash, $card, $payumoney, $cheque, $dabitt_from_wallet[0]["dabit"], $creditor_cash_due, $key["payable_amount"], $sample_date, $sample_time, $processing_date, $processing_time, $completed_time1, $report_generate_time1, $phlebo_tat, $processing_tat, $max_tat, $is_processing_delay, $code["test_name"], $code["p1"], $code["code1"], $code["code1"] > 0 ? ($code["p1"] * $code["code1"] / 100) : 0, $code["code1"] > 0 ? $code["p1"] - ($code["p1"] * $code["code1"] / 100) : $code["p1"], $key["ipd"], $ageShow, $outsource));
                        }
                    } else
...
Sonepat Tel: 0130-2242938, 2242939 </b><div>
    ...
    function update_job_test()
    {
        ...
        $total_price = 0;
        $payable_price = 0;
        $this->job_model->master_fun_update("booked_job_test", array("job_fk", $jid), array("status" => "0",'updated_date'=>date("Y-m-d H:i:s"),'updated_by'=>$data["login_data"]["id"]));
  ...
  `active_package` LEFT JOIN `package_master` ON `package_master`.`id` = `active_package`.`package_fk` WHERE `active_package`.`status` = '1' AND `due_to` >= '" . date("Y-m-d") . "' AND package_master.id='" . $tn[1] . "'  AND `active_package`.`user_fk` = '" . $get_user_details[0]["cust_fk"] . "' AND job_fk !='$jid' AND `active_package`.`parent`='0'");
            ...
            $this->job_model->master_fun_insert("booked_job_test", array("job_fk" => $jid, "test_city" => $get_user_details[0]["test_city"], "test_fk" => "p-" . $key, "price" => $pricepack,'created_date'=>date("Y-m-d H:i:s"),'created_by'=>$data["login_data"]["id"]));
        ...
        foreach ($selected_test as $key) {
            ...
            $this->job_model->master_fun_insert("booked_job_test", array("job_fk" => $jid, "test_city" => $get_user_details[0]["test_city"], "test_fk" => "t-" . $key, "price" => $new_price,'created_date'=>date("Y-m-d H:i:s"),'created_by'=>$data["login_data"]["id"]));
        ...
        foreach ($selected_panel_test as $key) {
            ...
            $this->job_model->master_fun_insert("booked_job_test", array("job_fk" => $jid, "test_city" => $get_user_details[0]["test_city"], "test_fk" => "pt-" . $key["test_fk"], "price" => $tst_price[0]["price"], "panel_fk" => $key["panel_fk"],'created_date'=>date("Y-m-d H:i:s"),'created_by'=>$data["login_data"]["id"]));
            //echo "select price from test_master_city_price where test_fk='" . $key . "' and city_fk='" . $get_user_details[0]["test_city"] . "' and status='1'";die();
        ...
        foreach ($referral_list as $referral) {
            if (!empty($data["cntr_arry"])) {
                if (in_array($referral['id'], $data["cntr_arry"])) {
                    $refer .= '<option data-ipd="' . $referral['ipd'] . '" value="' . $referral['id'] . '"';
    ...
    function check_test_parameter_val()
    {
        ...
        if (!empty($job_id) && !empty($job_id1)) {
            ...
            foreach ($j_details as $nkey) {
                ...
                $nwar[] = array("jid" => $nkey["job_fk"], "details" => array($nkey),"outsource"=>$outsource,'outsourceHours'=>$outsourceHours);
            }
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <input class="newadd" <?php
                                                            ...
                                                            ?> id="check_<?= $test['testid'] ?>" value="1" type="checkbox"> On New Page?
                                </td>
                                <td>
...
GROUP BY `test_master`.`id`");
            ...
            function assingPRO()
            {
                ...
                $search_data["date"] = '01/07/2024';
                $search_data["end_date"] = '15/07/2024';
                $data['query'] = $this->job_model->new_row_srch_job_list($search_data, '', "");
                ...
                foreach ($data['query'] as $query) {
                    //   echo  $query['id']; die;
                    $docMasterResult = $this->job_model->get_val("SELECT * FROM `doctor_master` WHERE `id` = '" . $query['doctor'] . "'");
                    ...
                    $this->service_model->master_fun_update1("job_master", array("id" =>  $query['id']), array("pro_id" => $doctor_pro_id));
                }
            }
            ...
            function hours_list()
            {
                ...
                $this->load->view('header');
                $this->load->view('nav', $data);
                $this->load->view('hours_job_list', $data);
                $this->load->view('footer');
            }

            function hours_list_search()
            {
                $this->benchmark->mark('code_start');
                if (!is_loggedin()) {
                    redirect('login');
                ...
                $url = "http://" . $_SERVER['HTTP_HOST'] . ":" . $_SERVER['SERVER_PORT'] . $_SERVER['REQUEST_URI'];
                $this->session->set_userdata("job_master_r", $url);
                $this->load->view('hours_job_list_search', $data);
            }
                function jobcount()
                {
                    $branch = $data['branch'] = $search_data["branch"] = $data["branch"] = $this->input->get_post('branch');
                ...
                function hours_list_export()
                {
                    ...
                    $data['userBranch'] = $cntr_arry;
                    $data['branchlist']= $branchlist= $this->registration_admin_model->get_val("SELECT * from branch_master where status!='0'");
                    /* Check test city */
                    $allow_test_city = array();
                    if ($data["login_data"]["type"] == 1) {
                        $u_allow_city = $this->registration_admin_model->get_val("SELECT * from user_test_city where user_fk='" . $data["login_data"]["id"] . "' and status='1'");
                    ...
                    foreach ($new_array as $key) {
                        ...
                        if($hours=="8to16"){
                            $hoursTitle="8 hours to 16 hours";
                        }
                        if($hours=="16to20"){
                            $hoursTitle="16 hours to 20 hours";
                        }
                        if($hours=="20to24"){
                            $hoursTitle="20 hours to 24 hours";
                        }
                        if($hours=="24to36"){
                            $hoursTitle="24 hours to 36 hours";
                        }
                        if($hours=="36to72"){
                            $hoursTitle="36 hours to 72 hours";
                        }
                        if($hours=="32to168"){
                            $hoursTitle="72 hours & +";
                        }
