...
class Indent_usereagent extends CI_Controller {
    ...
    public function addusestock() {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            $getstocks = $this->Intent_model->get_val("SELECT id,batch_no,(`quantity`-`used`) AS used FROM `inventory_stock_master` WHERE id='$batchno' AND status='1'");
    ...
    function getreagentbanch() {
        ...
        if ($id != "") {
            ...
            $query = $this->Intent_model->get_val("SELECT id,batch_no,(`quantity`-`used`) AS used FROM `inventory_stock_master` WHERE item_fk='$id' AND status='1'");
