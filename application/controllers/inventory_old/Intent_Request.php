...
WHERE inventory_intent_master.intent_id = '" . $tid . "' 
  AND inventory_intent_master.status != '0'");
        ...
        $item_statonary = $this->Intent_model->get_val("SELECT it.id,it.reagent_name,it.unit_fk AS utk_st,ium.name AS UnitSub FROM inventory_item AS it LEFT JOIN `inventory_unit_master` AS ium ON ium.`id` = it.`unit_fk` AND ium.`status`='1' WHERE it.status = '1' AND it.category_fk = 1 ");

        $item_Consumables = $this->Intent_model->get_val("SELECT it.id,it.reagent_name,it.unit_fk AS UNIT_FK,ium.name AS UnitName FROM inventory_item AS it LEFT JOIN `inventory_unit_master` AS ium ON ium.`id` = it.`unit_fk` AND ium.`status` = '1' WHERE it.status = '1' AND it.category_fk = 2");
        ...
        <div class="form-group">
            ...
            <select class="chosen chosen-select" name="item" id="consumables_id" onchange="select_item_edit('Consumables', this);">
                <option value="">--Select--</option>
                <?php
        ...
        <div class="form-group">
            ...
            <select class="chosen chosen-select" name="item" id="stationary_id" onchange="select_item_edit('Stationary', this);">
                <option value="">--Select--</option>
                <?php
        ...
        <table class="table">
            ...
            foreach ($edit_item_details as $ikey) {
                ...
                <tr id="tr_edit_<?= $cnt; ?>">
                    ...
                    <td><a href="javascript:void(0);" onclick="delete_city_price_edit('<?= $cnt; ?>', '<?= $ikey["item_name"] ?>', '<?= $ikey["category_fk"] ?>')"><i class="fa fa-trash"></i></a></td>
                </tr>
                <?php
                $cnt++;
            }
    ...
    public function poigenerate($indenid) {
        ...
        if ($indenid != "") {
            ...
            if ($data['indedrequiest'] != null) {
                ...
                $data['poitenm'] = $this->Intent_model->get_val("SELECT p.id,p.quantity,p.category_fk as itemid,i.reagent_name,i.category_fk as cattype,i.quantity as qty,i.box_price FROM inventory_intent_request p LEFT JOIN inventory_item i ON i.`id`=p.category_fk WHERE p.status='1' and p.indent_fk='$indenid'");
                        ...
                        if ($itemprice > 0 && $nosval1 > 0 && $itemvendor[$i] != "") {
                            ...
                            if (in_array($vendorid, $venderarra)) {
                                ...
                                $sametotal += $totalamount;
								 ...
								 $sametotal=($paybleamount+$getlastpoprice[0]["poprice"]);
                                ...
                                $poid = $this->Intent_model->master_fun_insert('inventory_pogenrate', array("branchfk" => $branchid, "ponumber" => $po_number, "poprice" => $paybleamount, "vendorid" => $itemvendor[$i], "indentcode" => $intent_id, "requestid" => $indenid, "cretedby" => $login_id, "creteddate" => date("Y-m-d H:i:s")));
                            }
                            ...
                            $poidarraay = array($vendorid => $poid);
