...
//echo "<pre>"; print_R($data['query']); die();
    ...
    function pending_list() {
        ...
        if (!empty($data["login_data"]['branch_fk'])) {
            ...
            $data['citylist'] = $this->registration_admin_model->get_val("SELECT * from test_cities where status!='0' and id in (" . implode(",", $cntr_arry1) . ")");
        ...
        $data['departmentlist'] = $this->registration_admin_model->get_val("SELECT id,name from test_department_master where status ='1'");
    ...
    function pending_list_search() {
        ...
        $department = $data['department'] = $search_data["department"] = $this->input->get_post('department');
        ...
        /* PLM start */
        if (empty($branch) && $plm != '') {
            $plm_branch = $this->registration_admin_model->get_val("SELECT GROUP_CONCAT(id) AS id FROM branch_master WHERE `status`='1' AND parent_fk='" . $plm . "' GROUP BY parent_fk");
        ...
        $cnt = 0;
        if ($department != "") {
            $checkdepart = "AND test_master.department_fk='$department'";
        } else {
            $checkdepart = "";
        }
        ...
        foreach ($data['query'] as $key) {
            ...
            $job_test_list = $this->job_model->get_val("SELECT `job_test_list_master`.*,`test_master`.`test_name` FROM `job_test_list_master` INNER JOIN `test_master` ON `job_test_list_master`.`test_fk`=`test_master`.`id` WHERE `job_test_list_master`.`job_fk`='" . $key["id"] . "' $checkdepart ");
...
//}
    ...
    function change_cc() {
        ...
        if ($jid != '' && $notify != '') {
            ...
            $payable_price = 0;
            if ($notify >= 0) {
                $collection_charge_status = 0;
                if ($notify > 0) {
                    $collection_charge_status = 1;
                }
                $price = $j_data[0]["price"] + $notify - $j_data[0]["cc"];


                $received_amount = $j_data[0]["price"] - $j_data[0]["payable_amount"] - ($j_data[0]["price"] * $j_data[0]["discount"] / 100);
                $payable_notify = $price - ($price * $j_data[0]["discount"] / 100);
                $payable_price = $payable_notify - $received_amount;
                $this->job_model->master_fun_update("job_master", array("id", $jid), array("collection_charge" => round($collection_charge_status), "cc" => $notify, "price" => round($price), "payable_amount" => round($payable_price)));
            }
            //$this->job_model->master_fun_insert("job_log", array("job_fk" => $jid, "created_by" => "", "updated_by" => $data["login_data"]["id"], "deleted_by" => "", "job_status" => '', "message_fk" => "20", "date_time" => date("Y-m-d H:i:s")));
            redirect("job-master/job-details/" . $jid);
        } else {
    ...
    function change_report_status() {
        ...
        $this->load->library("Util");
        $util = new Util;
        $name = $util->get_invoice_without($id);
        redirect("/upload/result/" . $name . "?" . time());
    }
...
Sonepat Tel: 0130-2242938, 2242939 </b><div>
    ...
    function update_job_test() {
        ...
        foreach ($selected_test as $key) {
            $return_test_price = $util->getTEstPrice($jid, array($key));
...
WHERE `job_master`.`id`='" . $p_key["job_fk"] . "' AND `job_master`.`status`!='0' AND `job_master`.`sample_collection`='0'");
    ...
    function ack($id) {
        ...
        $data["login_data"] = logindata();
        $this->load->model('add_result_model');
        $data['query'] = $this->job_model->job_details($id);
...
//    exit();
    ...
    function get_branch() {
        ...
        $refer = '';
        if (!empty($city)) {
            $referral_list = $this->job_model->master_fun_get_tbl_val("branch_master", array('status' => 1, "parent_fk" => $city), array("branch_name", "asc"));
            $referral_list = $this->job_model->get_val("select * from branch_master where status='1' and (parent_fk ='" . $city . "' or id='" . $city . "') order by branch_name asc");
        } else {
...
ORDER BY `branch_name` ASC");
    ...
    function send_report_sms() {
        ...
        if ($job_fk != '' && !empty($mobile_no) && trim($sms) != '') {
            $job_details = $this->job_model->get_val("SELECT job_master.`id`,`doctor_master`.`full_name`,`doctor_master`.`mobile`,`doctor_master`.`mobile1`,`doctor_master`.`mobile2`,`doctor_master`.`mobile_no_1`,`doctor_master`.`mobile_no_2`,`doctor_master`.`mobile_no_3` FROM job_master INNER JOIN `doctor_master` ON job_master.`doctor`=`doctor_master`.`id` WHERE job_master.`id`='" . $job_fk . "'");
            ...
            foreach ($mobile_no as $key) {
                $this->job_model->master_fun_insert("admin_alert_sms", array("mobile_no" => $key, "message" => $sms, "created_date" => date("Y-m-d H:i:s"), 'job_fk' => $job_fk));
            }
            if (!empty($job_details)) {
                if ($job_details[0]['mobile_no_1'] == 1) {
                    $this->job_model->master_fun_insert("admin_alert_sms", array("mobile_no" => $job_details[0]["mobile"], "message" => $sms, "created_date" => date("Y-m-d H:i:s"), 'job_fk' => $job_fk));
                }

                if ($job_details[0]['mobile_no_2'] == 1) {
                    $this->job_model->master_fun_insert("admin_alert_sms", array("mobile_no" => $job_details[0]["mobile1"], "message" => $sms, "created_date" => date("Y-m-d H:i:s"), 'job_fk' => $job_fk));
                }
                if ($job_details[0]['mobile_no_3'] == 1) {
                    $this->job_model->master_fun_insert("admin_alert_sms", array("mobile_no" => $job_details[0]["mobile2"], "message" => $sms, "created_date" => date("Y-m-d H:i:s"), 'job_fk' => $job_fk));
                }
            }

            //$this->job_model->master_fun_insert("send_report_sms", $data);
            $update_sms = $this->job_model->master_fun_update("job_master", array("id", $job_fk), array("send_sms" => '1'));
            if ($update_sms) {
...
//			<option value="">--Select Test--</option>';
    ...
    public function total_sample_test() {
        ...
        $completed_status = "0";
        $approve_status = "0";
        $sample_collected_status = "0";
        $processing_status = "0";


        $query = $this->job_model->get_val("SELECT GROUP_CONCAT(id) as TID,STATUS,COUNT(id)  as TotalStatus FROM job_master WHERE  id !='' and status != 0 AND `date` >='" . $date . "' AND `date` <='" . $end_date . "'  GROUP BY status");

        foreach ($query as $val) {

            $totaljob += $val['TotalStatus'];

            if ($val['STATUS'] == 1) {

                $Waiting_status = $val['TotalStatus'];
            }
            if ($val['STATUS'] == 6) {
                $approve_status = $val['TotalStatus'];
            }

            if ($val['STATUS'] == 7) {
                $sample_collected_status = $val['TotalStatus'];
            }

            if ($val['STATUS'] == 8) {
                $processing_status = $val['TotalStatus'];
            }

            if ($val['STATUS'] == 2) {
                $completed_status = $val['TotalStatus'];
            }
        }

        $sms_message = $this->job_model->master_fun_get_tbl_val("sms_master", array('status' => 1, "title" => "daily_report"), array("id", "asc"));
        $sms_message = preg_replace("/{{TJBS}}/", $totaljob, $sms_message[0]["message"]);
        $sms_message = preg_replace("/{{WFA}}/", $Waiting_status, $sms_message);
        $sms_message = preg_replace("/{{APR}}/", $approve_status, $sms_message);
        $sms_message = preg_replace("/{{SC}}/", $sample_collected_status, $sms_message);
        $sms_message = preg_replace("/{{PRC}}/", $processing_status, $sms_message);
        $sms_message = preg_replace("/{{CPTD}}/", $completed_status, $sms_message);


        $this->load->helper("sms");
        $notification = new Sms();

        $this->job_model->master_fun_insert("admin_alert_sms", array("mobile_no" => '9879572294', "message" => $sms_message, "created_date" => date("Y-m-d H:i:s")));
    }
    ...
    function update_register_date($cid) {
        ...
        if ($this->form_validation->run() != False) {
            ...
            $registration_date = $new_date . ' ' . $time;
            $data['query'] = $this->job_model->master_fun_update("job_master", array("id", $cid), array("date" => $registration_date));
