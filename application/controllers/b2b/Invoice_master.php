...
class Invoice_master extends CI_Controller {
    ...
    function invoice_pdf() {
        ...
        $data['from'] = $this->input->get('from');
        $data['todate'] = $this->input->get('todate');
        $data['intype'] = $this->input->get('intype');
        $cquery = "";
        if ($data['date'] != '' || $data['from'] != '' || $data['todate']) {
            ...
            $id = $data['query1'][0]['labid'];
            $data['bank'] = $this->invoice_model->master_fun_get_tbl_val("lab_bankdetail", array('lab_fk' => "$id", 'status' => '1'), array("bank_name", "asc"));
            $totalamount = 0;
            /* foreach($data['query1'] as $key){ 
              $totalamount +=round($key['amt']);
              }
              $data['totalamount']=round($totalamount); */
            //$data['amountstaring']=$this->convert_number_to_words($totalamount);
            $data['amountstaring'] = "";
            /* $data['totalamount']=$totalamount; */


            $pdfFilePath = FCPATH . "/upload/b2binvoice/paymentreport.pdf";
            $html = $this->load->view('b2b/report_pdf_view', $data, true);
...
<div style="height:20px;"><p class="rslt_p_brdr"></p>
            ...
            $base_url = base_url() . "upload/b2binvoice/paymentreport.pdf";
            $filename = FCPATH . "/upload/b2binvoice/paymentreport.pdf";
...
<div style="height:20px;"><p class="rslt_p_brdr"></p>
    ...
    function invoice_csv() {
        ...
        if ($data['date'] != '' || $data['from'] != '' || $data['todate']) {
            ...
            fputcsv($handle, array("No", "Reg.Date", "LabId", "Patientsname", "Investigation","Collection Charge", "Amount"));

            $query = $this->invoice_model->sample_listinvoce($data['date2'], $data['todate2'], $data['from']);
            $i = 0;
            foreach ($query as $row) {
                $i++;
                fputcsv($handle, array($i, $row["regdate"], $row['labid'], ucwords($row['patientname']), $row['testname'],$row['collection_charge'], round($row['amt'])));
            }
            fclose($handle);
            exit;
        } else {
