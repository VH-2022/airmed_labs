...
class Lab_master extends CI_Controller {
    ...
    function details() {
		...
		$data['test_list'] = $this->logistic_model->test_list1($lab_fk,$city);
		$data['pack_list'] = $this->logistic_model->test_list2($lab_fk,$city);
		
		$data["dueamountmonth"]= $this->logistic_model->get_val("SELECT  logistic_log.`createddate`,DATE_FORMAT(logistic_log.`createddate`,'%M-%Y' ) AS months  , SUM(sample_job_master.`payable_amount`) AS amount, SUM(sample_job_master.`price`) AS price,(SELECT SUM(amount) FROM sample_receive_payment AS r WHERE  r.lab_fk=logistic_log.`collect_from` AND DATE_FORMAT(logistic_log.`createddate`,'%m')=r.month AND  DATE_FORMAT(logistic_log.`createddate`,'%Y')=r.year) AS reciveamount  FROM `sample_job_master`
LEFT JOIN `logistic_log` ON logistic_log.`id`= `sample_job_master`.`barcode_fk`
WHERE logistic_log.`status`=1 AND `logistic_log`.`collect_from`='$lab_fk' GROUP BY ( DATE_FORMAT(logistic_log.`createddate`,'%m-%Y' ))ORDER BY ( DATE_FORMAT(logistic_log.`createddate`,'%m-%Y' )) DESC");
