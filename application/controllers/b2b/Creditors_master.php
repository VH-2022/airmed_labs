...
class Creditors_master extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('b2b/creditors_model');
    ...
    function check_email($email) {
        $result = $this->creditors_model->check_email($email, 'sample_creditors_master');
        if ($result == '0') {
            return TRUE;
        } else {
    ...
    public function index() {
        ...
        $this->load->view('b2b/creditors_list', $data);
    ...
    public function add() {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            $post['status'] = '1';
            $insert = $this->creditors_model->insert("sample_creditors_master", $post);
            if ($insert) {
                $branch = $this->input->post('branch');
                foreach ($branch as $br) {
                    $this->creditors_model->insert("sample_creditors_branch", array("creditors_fk" => $insert, "branch_fk" => $br, "created_by" => $data["login_data"]['id']));
                }
            }
            $this->session->set_flashdata("success", "Creditors successfully added.");
            redirect("b2b/creditors_master", "refresh");
        } else {
            $data['branch_list'] = $this->creditors_model->get_all("branch_master", array("status" => 1));
            $this->load->view('header');
            $this->load->view('nav', $data);
            $this->load->view('b2b/creditors_add', $data);
            $this->load->view('footer');
        }
    }
    ...
    function delete($uid) {
        ...
        $data['query'] = $this->creditors_model->update("sample_creditors_master", array("id" => $uid), array("status" => "0", 'deleted_by' => $data["login_data"]["id"]));
    ...
    public function edit($uid) {
        ...
        $data["user"] = $this->user_model->getUser($data["login_data"]["id"]);
        $data["uid"] = $uid;
        $data['query'] = $this->creditors_model->get_one("sample_creditors_master", array("id" => $uid));
        $data['branch_all'] = $this->creditors_model->get_all("sample_creditors_branch", array("creditors_fk" => $data['query']->id, "status" => 1));
        $data['branchs'] = array();
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            $post['status'] = '1';
            $update = $this->creditors_model->update("sample_creditors_master", array('id' => $uid), $post);
            $delete = $this->creditors_model->delete_record("sample_creditors_branch", array("status" => 1, "creditors_fk" => $uid));
            if ($update) {
                $branch = $this->input->post('branch');
                foreach ($branch as $br) {
                    $this->creditors_model->insert("sample_creditors_branch", array("creditors_fk" => $uid, "branch_fk" => $br, "updated_by" => $data["login_data"]['id'], "updated_date" => date('Y-m-d H:i:s')));
                }
            }
            $this->session->set_flashdata("success", "Creditors successfully added.");
            ...
            $this->load->view('header');
            $this->load->view('nav', $data);
            $this->load->view('b2b/creditors_edit', $data);
            $this->load->view('footer');
        }
    }
