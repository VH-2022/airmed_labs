...
WHERE  `logistic_log`.`status`='1' and `logistic_log`.`id`='" . $data["id"] . "'");
    ...
    function lab_bankdeposit_list() {
        ...
        $data["bank"] = $bank;
        if ($state_serch != "" || $data["bank"] != "") {
            $total_row = $this->Lab_bankdeposit_model->lab_num_rows($state_serch, $data["bank"]);
            $config = array();
            $get = $_GET;
            unset($get['offset']);
            $config["base_url"] = base_url() . "b2b/Lab_bankdeposit/lab_bankdeposit_list?pan=$state_serch&bank=$bank";
            $config["total_rows"] = $total_row;
            $config["per_page"] = 50;
            $config['page_query_string'] = TRUE;
            $config['cur_tag_open'] = '<span>';
            ...
            $this->pagination->initialize($config);
            $page = ($this->input->get("per_page")) ? $this->input->get("per_page") : 0;
            $data['query'] = $this->Lab_bankdeposit_model->lab_data($state_serch, $data["bank"], $config["per_page"], $page);
    
            $data["links"] = $this->pagination->create_links();
        } else {
            $total_row = $this->Lab_bankdeposit_model->lab_num_rows();
            ...
            $this->pagination->initialize($config);
            $page = ($this->input->get("per_page")) ? $this->input->get("per_page") : 0;
            $data['query'] = $this->Lab_bankdeposit_model->srch_lab_list($config["per_page"], $page);
    ...
    function maximumCheck($num)
{
    if ($num > 100)
    {
        $this->form_validation->set_message(
                        'test_discount',
                        'The %s field must be less than 100'
                    );
        return FALSE;
    }
    else
