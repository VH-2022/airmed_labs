...
public function import_listadd($lid) {
                ...
                if ($this->csvimport->get_array($file_path)) {
                    ...
                    foreach ($csv_array as $row) {
						...
						if($testdetils != ""){
							...
							$old_test = $this->Logistic_test_modal->master_fun_get_tbl_val("b2b_testspecial_price",array("status" => '1',"test_fk" =>$testid,"lab_id" => $lid), array("id", "desc"));
						
						$testdata=array("lab_id" => $lid, "test_fk" =>$testid);
						if($row['special_price'] != ""){  if($row['special_price'] != ""){ $testdata['price_special']=$row['special_price'];} 
                        if(empty($old_test)) {
							
                            $cnt++;
                            $this->Logistic_test_modal->master_fun_insert("b2b_testspecial_price",$testdata);
                        
						} else {
                         $id=$old_test[0]['id'];
							$cnt++;
                            $this->Logistic_test_modal->master_fun_update("b2b_testspecial_price",array("id",$id),$testdata);
							...
							/* $labdetils=$this->Logistic_model->fetchdatarow('test_discount,city','collect_from',array("id"=>$lid,"status"=>1));
			                $city=$labdetils->city;
							
							$testid=$this->Logistic_test_modal->master_fun_insert("test_master",array("test_name"=> $row['testname']));
							$this->Logistic_test_modal->master_fun_insert("test_master_city_price",array("test_fk"=>$testid,"city_fk"=>$city,"price"=>$row['price'])); */
