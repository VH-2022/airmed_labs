...
function getJobBarcode_new() {
        ...
        if (!empty($id)) {
            $job_data = $this->patholab_api_model->get_val("SELECT id,branch_fk,cust_fk,doctor,booking_info,barcode,test_city,date FROM job_master WHERE STATUS!='0' AND `id`='" . $id . "'");
			$test_data = $this->patholab_api_model->master_fun_get_tbl_val("test_master", array('id' => $test_id), array("id", "asc"));
            ...
            $data['barcode'] = $this->bar128($barcode);
            $data["doctor"] = $doctor_data[0]["full_name"];
			$data["sample"] = $test_data[0]["sample"];
            $F_PPBS = "";
            if(strpos(strtolower($test_data[0]["test_name"]), "fbs") > -1)
                $F_PPBS = "F";
            else if(strpos(strtolower($test_data[0]["test_name"]), "ppbs") > -1)
                $F_PPBS = "PPBS";
            $data["F_PPBS"] = $F_PPBS;
            $this->load->view('barcode_view', $data);
        } else {
