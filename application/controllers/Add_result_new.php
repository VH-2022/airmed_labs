...
for ($i1 = 0; $i1 < $length; $i1++) {
  $this->add_result_model->updateRowWhere('job_test_list_master',array('test_fk'=>$testfk[$i1],'job_fk'=>$id),array("position" =>$i1,"new_page" =>$new_page[$i1]));
            ...
            if (in_array($tst_id, $approved_test)) {
                ...
                if (!empty($pid)) {
                    ...
                    foreach ($para as $para_key) {
						...
						$gettest = $this->add_result_model->get_val("SELECT new_page from job_test_list_master where test_fk='" . $tst_id . "' and job_fk='" . $data['cid'] . "' and status='1'");
