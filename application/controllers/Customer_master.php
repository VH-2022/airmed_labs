...
class Customer_master extends CI_Controller {
    ...
    function customer_add() {
        ...
        $this->form_validation->set_rules('discount', 'Discount', 'trim|required|numeric|greater_than_equal_to[0]|less_than[101]');
...
<p style="color:#7e7e7e;font-size:13px;"> Your Password is: ' . $password . '  </p>
    ...
    function customer_all_details() {
		...
		$data["usertest"] = $this->customer_model->get_val("SELECT id,`test_name` FROM `test_master` WHERE STATUS='1' AND id IN(SELECT DISTINCT test_fk FROM `job_test_list_master` INNER JOIN `job_master` ON job_master.id=job_test_list_master.job_fk WHERE job_test_list_master.`status`='1' AND job_master.`status` IN (8, 2) AND job_master.cust_fk='".$data["cid"]."') ORDER BY `test_master`.`test_name` ASC");
		...
		$fid=$this->input->get("testfor");
		$new_response_formate = array();
		$pid=$data["cid"];
		if($testu != ""){
		$tid=$testu;	
			if ($fid > 0) {
                $dataall = $this->customer_model->get_val("SELECT 
  job_master.id,
  job_master.`order_id`,
...
ORDER BY `job_master`.`id` ASC ");
	...
	$total_parameter=array();
            ...
            foreach ($dataall as $key) {
                ...
                if (!empty($pid)) {
                    $para = $this->customer_model->get_val("SELECT test_parameter_master.id,test_parameter_master.parameter_name,test_parameter_master.parameter_unit FROM `test_parameter_master` WHERE `status`='1' AND id IN (" . implode(",", $pid) . ") ORDER BY FIELD(id," . implode(",", $pid) . ") and test_parameter_master.parameter_name!=''");
                    if (!empty($para)) {
                        $cnt_1 = 0;
                        $para_new = array();
                        foreach ($para as $para_key) {
                            $para_user_val = $this->customer_model->get_val("SELECT value from user_test_result where test_id='" . $tid . "' and parameter_id='" . $para_key["id"] . "' and job_id='" . $key['id'] . "' and  status='1'");
  ...
  END AS col1  FROM `parameter_referance_range` WHERE STATUS='1' AND `parameter_fk`='" . $para_key["id"] . "'";
                                ...
                                $refrance_range = $this->customer_model->get_val($final_qry);
...
foreach ($total_parameter as $key):
            ...
            endforeach;			
			/*  echo "<pre>"; print_r($new_response_formate); die();  */
		}
		$data["parameter"]=$new_response_formate;
