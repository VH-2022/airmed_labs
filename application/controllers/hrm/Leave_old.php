...
class Leave extends CI_Controller {
    ...
    function leave_list() {
        ...
        /* Nishit code start */
        if (!empty($_POST["umonth"]) || !empty($_POST["ucity"])) {
            ...
            if ($this->form_validation->run() != FALSE) {
                ...
                foreach ($uploaddata["data"] as $udkey) {
                    ...
                    $all_record = $this->leave_model->get_all("hrm_month_atendance", array("employee_fk" => $employee_details[0]->id, "month" => $umonth_year[0], "year" => $umonth_year[1], "status" => 1));
                    $present = 0;
                    if (!empty($employee_details)) {
                        ...
                        $daynumber = cal_days_in_month(CAL_GREGORIAN, $umonth_year[0], $umonth_year[1]);
                        $data1 = array(
                        ...
                        if (count($all_record) > 0) {
                            $update = $this->leave_model->update("hrm_month_atendance", array("employee_fk" => $employee_details[0]->id, "month" => $umonth_year[0], "year" => $umonth_year[1], "status" => 1), $data1);
...
//                                        $salary_calculate = 0;
//                                    } else {
//                                    if ($employee_salary >= '6000' && $employee_salary < '9000' && $tds < 1) {
//                                        $salary_calculate = '80';
//                                    }
//                                    if ($employee_salary >= '9000' && $employee_salary < '12000' && $tds < 1) {
//                                        $salary_calculate = '150';
//                                    }
//                                    if ($employee_salary >= '12000' && $employee_salary < '21000' && $tds < 1) {
//                                        $salary_calculate = '200';
//                                    }
//                                    }
//                                    $pt = $salary_calculate;
                                    
                                    $pt = $salary_calculate = $professional_tax;
                                }
                                /* END */
                        ...
                        /* END */
                        /* CTC START */
                        $ctc = $basic + $da + $hra + $convence + $madical + $special_allowance;
                        /* END */
						/*PER DAY BASIC SALARY START*/
						$perday_basic = round(($basic - $leave_cut_amount)/$moth_day);
						/*END*/
                        /* NTH START */
                        $nth = ($basic + $da + $hra + $convence + $madical + $special_allowance + $MAINTENACE + $ot_amount + $petrol + $incentive + $fstvl + $sw_amount + $others) - $pf - $esic - $pt - $tds - $leave_cut_amount - $Other_Deduction;
                        /* END */
                        ...
                        foreach ($salary_array as $value) {
                            ...
                            if (trim(strtoupper($value["salary_field"])) == "SPECIAL ALLOWANCE") {
                                $value = array();
                                $value = array("atendance_fk" => $insert, "salary_field" => "Special Allowance", "value" => $special_allowance, "type" => 1, "earning_type" => 1);
                            }
                            ...
                            if (trim(strtoupper($value["salary_field"])) == "OTHERS EARNING") {
                                $value = array();
                                $value = array("atendance_fk" => $insert, "salary_field" => "Others Earning", "value" => $data1["others"], "type" => 8, "earning_type" => 1);
                            }
                        ...
                        if (!in_array("SPECIAL ALLOWANCE", $added_salary_fields)) {
                        ...
                        if (!in_array("INSENTIVE", $added_salary_fields)) {
                            $new_salary_array[] = array("atendance_fk" => $insert, "salary_field" => "Insentive", "value" => $data1["incentive"], "type" => 5, "earning_type" => 1);
                        }
...
WHERE hd.status = 1 
  ...
  AND ha.`year`='".$current_year."' AND ha.`employee_fk`='".$emp."'";
	...
	/*	$new_common_salary_structure[] = array("salary_name"=>"Present Day Basic Salary");*/
        ...
        $pdfFilePath = FCPATH . "/upload/employee/salary_slip_pdf_leave" . $date . ".pdf";
