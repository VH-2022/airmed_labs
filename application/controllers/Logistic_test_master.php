...
class Logistic_test_master extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('Logistic_test_modal');
    ...
    function test_list() {
        ...
        if ($search != "" || $city != '') {
            ...
            $total_row = $this->Logistic_test_modal->test_list_search_num($search, $city);
            $config = array();
            $config["base_url"] = base_url() . "logistic_test_master/test_list/?search=$search&city=$city";
            ...
            $data['query'] = $this->Logistic_test_modal->test_list_search($search, $city, $config["per_page"], $page);
            $data["links"] = $this->pagination->create_links();
            $data["counts"] = $page;
        } else {
            $totalRows = $this->Logistic_test_modal->num_row('sample_test_master', array('status' => 1));
            $config = array();
            $config["base_url"] = base_url() . "logistic_test_master/test_list/";
            ...
            $data["query"] = $this->Logistic_test_modal->test_list($config["per_page"], $page);
            $data["links"] = $this->pagination->create_links();
            $data["counts"] = $page;
        }
        $cnt = 0;
        ...
        $this->load->view('nav_logistic', $data);
        $this->load->view('l_test_list', $data);
        $this->load->view('footer_logistic');
    }
    ...
    function test_add() {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            $data['query'] = $this->Logistic_test_modal->master_fun_insert("sample_test_master", array("test_name" => $name, "description" => $desc, "fasting_requird" => $fasting));
            $cnt = 0;
            foreach ($city as $frm_price) {
                if (!empty($price[$cnt])) {
                    $this->Logistic_test_modal->master_fun_insert("sample_test_city_price", array("test_fk" => $data['query'], "city_fk" => $frm_price, "price" => $price[$cnt]));
                }
                $cnt++;
            }
            /*
    ...
    function test_delete() {
        ...
        if (!empty($this->session->userdata("test_master_r"))) {
            ...
            redirect("logistic_test_master/test_list", "refresh");
        }
    }
    ...
    function test_csv() {
        ...
        $query = "SELECT  t.`test_name` as testname ,c.`name` as cityname,tc.`price` FROM  `sample_test_master` t  LEFT JOIN sample_test_city_price tc ON tc.test_fk=t.id  LEFT JOIN `test_cities` c ON tc.`city_fk`=c.`id` WHERE t.status = '1' AND tc.city_fk='" . $city . "'";
    ...
    function sub_test_add($tid) {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            redirect('sample_test_master/sub_test_add/' . $this->uri->segment(3));
        } else {
    ...
    function sub_test_delete() {
        ...
        if ($delete) {
            ...
            $this->session->set_userdata('success', $ses);
            redirect('sample_test_master/sub_test_add/' . $pid, 'refresh');
        }
    }
