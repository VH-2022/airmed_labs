...
class Bill_master extends CI_Controller {
    ...
    public function bill_list() {
        ...
        $expense_category_fk = $this->input->get('expense_category_fk');
        $end_date = $this->input->get('end_date');
        $branch_fk = $this->input->get('branch_fk');
		$paystatus = $this->input->get('paystatus');
		
		$srchdata = array("end_date" => $end_date, "description" => $description, "amount" => $amount, "start_date" => $start_date, "expense_category_fk" => $expense_category_fk, "branch_fk" => $branch_fk, "paystatus" => $paystatus);
		
        if ($description != '' || $amount != '' || $start_date != '' || $expense_category_fk != '' || $end_date != '' || $branch_fk != '' || $paystatus != '') {
			...
			$get=$_GET;
			unset($get['offset']);
            $config["base_url"] = base_url() . "bill_master/bill_list?start_date=".$start_date."&end_date=".$end_date."&branch_fk=".$branch_fk."&expense_category_fk=".$expense_category_fk."&paystatus=".$paystatus;
            ...
            if ($type == 1 || $type == 2 || $type == 8) {
                $data["query"] = $this->Bill_model->get_val("SELECT *,bill_master.id,admin_master.name as AdminName,bill_master.created_date as created_date1,`branch_master`.`branch_name`,expense_category_master.name as CategoryName FROM bill_master LEFT JOIN `branch_master` ON bill_master.`branch_fk`=`branch_master`.id LEFT JOIN expense_category_master ON bill_master.expense_category_fk = expense_category_master.id LEFT JOIN admin_master ON admin_master.id = bill_master.created_by WHERE bill_master.branch_fk IN (SELECT branch_fk AS branch FROM `user_branch` WHERE `status`='1' ) AND bill_master.`status`='1' AND expense_category_master.name !='' AND bill_master.`paystatus` IN(0,1,2,3) order by bill_master.id desc limit " . $page . "," . $config["per_page"]);
            
			} else {
                $data["query"] = $this->Bill_model->get_val("SELECT *,bill_master.id,admin_master.name as AdminName,bill_master.created_date as created_date1,`branch_master`.`branch_name`,expense_category_master.name as CategoryName FROM bill_master LEFT JOIN `branch_master` ON bill_master.`branch_fk`=`branch_master`.id LEFT JOIN expense_category_master ON bill_master.expense_category_fk = expense_category_master.id LEFT JOIN admin_master ON admin_master.id = bill_master.created_by WHERE bill_master.branch_fk IN (SELECT branch_fk AS branch FROM `user_branch` WHERE `status`='1' AND user_fk='" . $id . "') AND bill_master.`status`='1' AND expense_category_master.name !='' AND bill_master.`paystatus` IN(0,1,2,3) order by bill_master.id desc limit " . $page . "," . $config["per_page"]);
            }
        }
		//echo "<pre>";
		//print_r($data["query"]);
    ...
    public function bill_add() {
        ...
        $this->form_validation->set_rules('invoice_no', 'Invoice No', 'required|trim|callback_invoice_check');


        if (empty($_FILES['upload_receipt']['name'])) {
            $this->form_validation->set_rules('upload_receipt', 'Upload Receipt', 'required');
        }
    ...
    public function bill_edit() {
        ...
        $this->form_validation->set_rules('expense_date', 'bill Date', 'required|trim');
        $this->form_validation->set_rules('invoice_no', 'Invoice No', 'required|trim|callback_invoice_check_edit');
        $this->form_validation->set_error_delimiters('<span class="text-danger">', '</span>');
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            $post['invoice_no'] = $this->input->post('invoice_no');
            $post['status'] = 1;
    ...
    public function bill_export() {
        ...
        $result = $this->Bill_model->csv_report($start_date,$end_date,$branch_fk,$category_fk,$paystatus);
        /* echo "<pre>";
          print_r($result); die(); */
        ...
        fputcsv($handle, array("Sr.No", "Invoice No", "Bill Date", "Amount", "Branch Name", "Expense Category", "Description", "Last Date", "Status", "Added By"));
		$cnt = 1;
        foreach ($result as $val) {

            fputcsv($handle, array($cnt, $val["invoice_no"], $val["expense_date"], $val["amount"], $val["branch_name"], $val["name"], $val["description"], $val["last_date"],($val["paystatus"]==0)?"Pending":(($val["paystatus"]==1)?"Completed":(($val["paystatus"]==2)?"Booked":"Rejected")), $val["AdminName"]));
			$cnt++;
        }
        fclose($handle);
        exit;
...
//      / echo "<prE>";print_r($data['branch_list']);
        ...
        if ($type == 1 || $type == 2 && $id != "" && $data['billdetils'] != "") {
            ...
            if ($this->form_validation->run() != FALSE) {
                ...
                if (!empty($_FILES['upload_receipt']['name'])) {
                    ...
                    $filename = $_FILES['upload_receipt']['name'];
                    ...
                    if ($this->upload->do_upload('upload_receipt')) {
                        ...
                        $this->session->set_flashdata("duplicate", $error);
                        redirect("bill_master/bill_payment/$id", "refresh");
                    }
                } else {
                ...
                if ($data['billdetils'][0]["paystatus"] == 1) {
                    $data["paydetils"] = $this->Bill_model->duplicate_val("SELECT b.id,b.`billid`,b.`neftno`,b.`amount`,b.`remark`,b.creteddate,b.biil_file,a.`name` FROM bill_payment b LEFT JOIN admin_master a ON a.`id`=b.`cratedby` WHERE b.billid='" . $data['billdetils'][0]["id"] . "' AND b.status='1'");
                } else {
                    $data["paydetils"] = array();
                }
                if ($data["login_data"]["type"] == 8) {
                    $this->load->view('inventory/header');
                    $this->load->view('inventory/nav', $data);
    ...
    public function bill_booked() {
        ...
        $id = $data["login_data"]['id'];
        $eid = $this->uri->segment('3');
        $data['query'] = $this->Bill_model->master_get_update("bill_master", array("id" => $eid), array("paystatus" => "2"), array("id", "desc"));
        $this->session->set_flashdata("success", array("Bill Successfully Booked."));
        redirect("bill_master/bill_payment/" . $eid, "refresh");
    }
    ...
    public function reject() {
        ...
        if ($reject_reason != "") {
            $data['query'] = $this->Bill_model->master_get_update("bill_master", array("id" => $id), array("paystatus" => '3', "reject_reason" => $reject_reason), array("id", "desc"));
        }

        $this->session->set_flashdata("success", array("Bill Successfully Rejected."));
        redirect("bill_master/bill_list", "refresh");
    }

    function invoice_check($str) {
        $invoice_data = $this->Bill_model->get_val("select invoice_no from bill_master where status='1'");
        $error = 1;
        if ($str != "") {
            foreach ($invoice_data as $temp) {
                if ($temp['invoice_no'] == $str) {
                    $error = 0;
                }
            }
        } else {
            $this->form_validation->set_message('invoice_check', 'The {field} is required');
            return FALSE;
        }

        if ($error == 0) {
            $this->form_validation->set_message('invoice_check', 'The {field} is already exist');
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    function invoice_check_edit($str) {
        $id = $this->input->post('id');
        $invoice_data = $this->Bill_model->get_val("select invoice_no from bill_master where status='1' AND id!='$id'");
        $error = 1;
        if ($str != "") {
            foreach ($invoice_data as $temp) {
                if ($temp['invoice_no'] == $str) {
            ...
            $this->form_validation->set_message('invoice_check_edit', 'The {field} is required');
            return FALSE;
        }

        if ($error == 0) {
            $this->form_validation->set_message('invoice_check_edit', 'The {field} is already exist');
            return FALSE;
        } else {
            return TRUE;
        }
