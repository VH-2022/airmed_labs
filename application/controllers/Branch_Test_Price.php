...
//                    ";
    ...
    function branch_test_export_csv($id) {
        ...
        if (!empty($id)) {
            ...
            if ($ttype == 2) {
                $q = "SELECT tp.id,tp.price,tp.test_fk,tp.branch_fk,tp.status,tm.title AS test_name, tp.r_code, tm.id tid FROM package_master tm INNER JOIN test_branch_price tp ON tp.test_fk = tm.id   WHERE tp.branch_fk='$id' AND tm.status ='1' AND tp.`type`='2' and tp.status = '1' ";

                if ($name != "") {
                    $q .= "  AND tm.title LIKE '%$name%' ";
                }
            } else {
                $q = "SELECT tp.id,tp.price,tp.test_fk,tp.branch_fk,tp.status,tm.test_name, tp.r_code, tm.id tid
                FROM test_master tm LEFT JOIN test_branch_price tp
                on tp.test_fk = tm.id 
            ...
            $cnt = 0;
            fputcsv($handle, array("ID", "Branch Code", "Test Name", "Price", "R Code", "Test ID"));
            foreach ($branch_wise_test as $key) {
//                fputcsv($handle, array(++$cnt, $key->id, $key->branch_fk, $key->test_name, $key->price));
                fputcsv($handle, array($key->id, $key->branch_fk, $key->test_name, $key->price, $key->r_code, $key->tid));
            }
            fclose($handle);
            exit;
        }
...
//        $branch_wise_test = $this->Branch_Model->get_val($q);
        ...
        if (!$this->upload->do_upload('testeximport')) {
                ...
                foreach ($csv_array as $row) {
                   ...
                   $qTemp ='LOWER(REPLACE(tm.test_name,'."'".',""))';
                    ...
                    if ($branch_id !="" && is_numeric($price) && $price > 0) {
                        ...
                        if($package_type ==2){
                            $q = "SELECT tp.id,tp.price,tp.test_fk,tp.branch_fk,tp.status,tm.title AS test_name, REPLACE(tm.title,'`','') AS test_name_name 
                               FROM package_master tm INNER JOIN test_branch_price tp ON tp.test_fk = tm.id   
                        ...
                        if(isset($branch_wise_test[0]->id)){
                            ...
                            $q = "SELECT tm.id,tm.test_name, REPLACE(tm.test_name,', '') AS test_name_name 
                             ...
                             $branch_wise_test = $this->Branch_Model->get_val($q);
                                  if(isset($branch_wise_test[0]->id)){
                                $this->Branch_Model->master_get_insert('test_branch_price', array("test_fk"=>$branch_wise_test[0]->id,'branch_fk'=>$branch_id,"price" => $price));
                            }else{
                                $notFoundtest_names[]= $test_name;
                            }
                        ...
                        //         $this->Branch_Model->master_get_insert('test_branch_price', array("test_fk"=>$branch_wise_test[0]->id,'branch_fk'=>$branch_id,"price" => $price));
                        //     }else{
                        //         $instedID = $this->Branch_Model->master_get_insert('test_master', array("test_name" => $test_name,"PRINTING_NAME" => $test_name));

                        //         $this->Branch_Model->master_get_insert('test_branch_price', array("test_fk"=>$instedID,'branch_fk'=>$branch_id,"price" => $price));
                        //      } */
            ...
            $this->session->set_flashdata('success', array("File is uploaded successfully",array(),json_encode($notFoundtest_names)));
            redirect("Branch_Test_Price/edit_test_price/$branch_id", "refresh");
        }
    }
 ...
 function importtestprice_csv() {
    ...
    function importrcode_csv() {
        ...
        if (!$this->upload->do_upload('testeximport')) {
            $error = $this->upload->display_errors();
            $ses = array($error);
            $this->session->set_flashdata("error", $ses);
            ...
            if ($this->csvimport->get_array($file_path)) {
                ...
                foreach ($csv_array as $row) {
                    $test_name = $row['Test Name'];
                    $id = trim($row['ID']);
                    $rcode = trim($row['R Code']);
                    $price = trim($row['Price']);
                    $name = strtolower($test_name);

                    if(!isset($row['R Code']) || !isset($row['Test Name'])){
                        $this->session->set_flashdata("error", array("Invalid File. Please download sample file and update R Code"));
          
                        redirect("Branch_Test_Price/edit_test_price/$branch_id", "refresh");
                    }
                    
                 
                    if ($branch_id !="" && !empty($rcode)) {
        
                        if($package_type ==2){
                            $q = "SELECT tp.id,tp.price,tp.test_fk,tp.branch_fk,tp.status,tm.title AS test_name 
                               FROM package_master tm INNER JOIN test_branch_price tp ON tp.test_fk = tm.id   
                                WHERE tp.branch_fk='$branch_id' AND tm.status ='1' AND tp.`type`='2'  and tp.status='1'  and LOWER(tm.title) LIKE '%$name%'";
                        }else{
                            $q = "SELECT tp.id,tp.price,tp.test_fk,tp.branch_fk,tp.status,tm.test_name
                            FROM test_master tm INNER JOIN test_branch_price tp
                            on tp.test_fk = tm.id 
                            WHERE tp.branch_fk='$branch_id'  AND tm.status = 1 and tp.type='1' and tp.status='1' and LOWER(tm.test_name) ='$name'
                            ";
                        }
                        ...
                        if(isset($branch_wise_test[0]->id)){
                            ...
                            $update = $this->Branch_Model->master_tbl_update("test_branch_price", $branch_wise_test[0]->id, array("r_code" => $rcode));
                        }else{
                          $notFoundName[]= $test_name;
                        }
                    }
            ...
            $this->session->set_flashdata('success', array("File is uploaded successfully",json_encode($notFoundName)));
            redirect("Branch_Test_Price/edit_test_price/$branch_id", "refresh");
        }
    }
