...
public function societyadmin() {
		...
		$this->load->library("pagination");
         $data['cfname'] = $cfname;
            ...
            $totalRows = $this->camping_from_model->soccityadminnum_row($cfname,$did);
			
            $config = array();
            
			$config["base_url"] = base_url()."camping/societyadmin?cfname=".$cfname;
            $config["total_rows"] = $totalRows;
            $config["per_page"] = 50;
            $config['page_query_string'] = TRUE;
            $config["uri_segment"] = 3;
            ...
            $page = ($this->input->get("per_page")) ? $this->input->get("per_page") : 0;
            $data['query'] = $this->camping_from_model->scocityadminsearch($cfname,$did,$config["per_page"],$page);
	...
	$i++;
			...
			$dgetcity=$data["dockdetils"]->city;
			
			
			
			$data["city_list"]=$this->camping_from_model->get_val("SELECT city_fk as id,name FROM `test_cities` WHERE status='1'");
		...
		$this->load->view('header');
        $this->load->view('nav', $data);
        $this->load->view('doctor_scoitycampview', $data);
        $this->load->view('footer');
...
public function getcitydoctor(){
	 if (!is_loggedin()) {
		...
		$this->load->model('doctor/camping_from_model');
		$did= $this->input->get('did');
		$didval= $this->input->get('didval');
		
		$doctlis="<select name='did'  class='form-control chosen-select'><option value=''>Select Doctor</option>";
		
		if($did != ""){
		$getdoctor=$this->camping_from_model->get_val("SELECT id,full_name FROM `doctor_master` WHERE status='1' and city='$did'");
	
		foreach($getdoctor as $doct){
			if($didval==$doct["id"]){ $select="selected"; }else{ $select=""; } 
			$doctlis.="<option value='".$doct["id"]."' $select >".ucwords($doct["full_name"])."</option>";
...
public function society_campconvert() {
		...
		if($dockcamp != ""){ $chekcamp="AND c.id='$dockcamp'"; }else{ $chekcamp=""; }
		
		$getcampragiste=$this->camping_from_model->get_val("SELECT r.name,r.mobile,r.gender,r.age,r.createddate,c.name AS campname FROM `camping_register` r INNER JOIN camping c ON c.id=r.camp_fk WHERE r.status='1' AND c.status='1' AND c.type='2'  and c.created_by='$did' $chekcamp");
			
		$mainquery=array();
			
		$temp ="";
        if($start_date != ''){
            $temp .="AND STR_TO_DATE(j.date,'%Y-%m-%d') >= '".date("Y-m-d",strtotime($start_date))."'";
        }
		if($end_date != ''){
			
            $temp .=" AND STR_TO_DATE(j.date,'%Y-%m-%d') <= '".date("Y-m-d",strtotime($end_date))."'";
        
		}
		
			foreach($getcampragiste as $campdet){
			
				$dateragiter=date("Y-m-d",strtotime($campdet["createddate"]));
				$patientmobile=$campdet["mobile"];
				$getjobcount=$this->camping_from_model->get_val("SELECT COUNT(j.id) AS total,GROUP_CONCAT(DISTINCT j.id) AS jobid FROM `job_master` j INNER JOIN `customer_master` c ON c.id=j.`cust_fk` WHERE j.status != '0' AND j.`doctor`='$did' AND STR_TO_DATE(j.date,'%Y-%m-%d') >= '$dateragiter' AND c.`mobile`='$patientmobile' $temp");
				
				
				 $totaljob=$getjobcount[0]["total"];
				if($totaljob != 0){
					$totaljobid=explode(",",$getjobcount[0]["jobid"]);
					
					$job_test_list = $this->camping_from_model->get_val("SELECT `test_master`.`test_name` FROM `job_test_list_master` INNER JOIN `test_master` ON `job_test_list_master`.`test_fk`=`test_master`.`id` WHERE `job_test_list_master`.`job_fk`='" . $totaljobid[0]."'");
					
					
					$query=array();
					
					$query["name"]=$campdet["name"];
					$query["mobile"]=$campdet["mobile"];
					$query["gender"]=$campdet["gender"];
					$query["age"]=$campdet["age"];
					$query["campname"]=$campdet["campname"];
					$query["job_test_list"]=$job_test_list;
					
					
					array_push($mainquery,$query);
		...
		$getsos_doctor=$this->camping_from_model->get_val("SELECT GROUP_CONCAT(DISTINCT `created_by`) AS doctor FROM `camping` WHERE status='1' AND TYPE='2'");
		
		$docror=rtrim($getsos_doctor[0]["doctor"]);
		
		$data["doctor_list"]=$this->camping_from_model->get_val("SELECT id,full_name FROM `doctor_master` WHERE status='1' and id in($docror) ");
		
		
		$this->load->view('header');
        $this->load->view('nav', $data);
        $this->load->view('scoitycamp_convertview', $data);
        $this->load->view('footer');
		
		
    }

public function campconvert_export() {
		...
		header("Content-type: application/csv");
        header("Content-Disposition: attachment; filename=\"society_campconvert.csv\"");
        header("Pragma: no-cache");
        header("Expires: 0");
        $handle = fopen('php://output', 'w');
		fputcsv($handle, array("Srl no", "Camp name", "Patient name", "Mobile", "Age","Gender"));
		...
		if($end_date != ''){
			...
			foreach($getcampragiste as $campdet){
				...
				$dateragiter=date("Y-m-d",strtotime($campdet["createddate"]));
				$patientmobile=$campdet["mobile"];
				$getjobcount=$this->camping_from_model->get_val("SELECT COUNT(j.id) AS total FROM `job_master` j INNER JOIN `customer_master` c ON c.id=j.`cust_fk` WHERE j.status != '0' AND j.`doctor`='$did' AND STR_TO_DATE(j.date,'%Y-%m-%d') >= '$dateragiter' AND c.`mobile`='$patientmobile' $temp");
				...
				if($totaljob != 0){
					...
					$i++;
					if($campdet["gender"] == "2"){ $gen="Female"; }else{ $gen="Male"; }
					fputcsv($handle, array($i,$campdet["campname"],$campdet["name"],$campdet["mobile"],$campdet["age"],$gen));
...
public function getsoctydoctor(){
	 if (!is_loggedin()) {
            redirect('login');
        }
        $data["login_data"] = logindata();
		...
		$getdoctor=$this->camping_from_model->get_val("SELECT id,full_name FROM `doctor_master` WHERE status='1' and id in($docror) ");
...
public function getdoctorcamp(){
	 if (!is_loggedin()) {
		...
		$doctlis="<select name='dockcamp'  class='form-control chosen-select'><option value=''>Select Camp</option>";
		
		if($did != ""){
		$getdoctor=$this->camping_from_model->get_val("SELECT id,name FROM `camping` WHERE status='1' and type='2' and created_by='$did'");
		
		foreach($getdoctor as $doct){
			if($didval==$doct["id"]){ $select="selected"; }else{ $select=""; } 
			$doctlis.="<option value='".$doct["id"]."' $select >".ucwords($doct["name"])."</option>";
