...
class Add_result extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('tech/master_model');
        $this->load->model('tech/user_model');
        $this->load->model('tech/user_test_master_model');
        $this->load->model('tech/add_result_model');
...
//$data['unit_list'] = $this->add_result_model->unit_list();
        ...
        $this->load->view('tech/view_collected', $data);
...
//$data['unit_list'] = $this->add_result_model->unit_list();
        $this->load->view('header');
        //$this->load->view('nav', $data);
        $this->load->view('tech/view_collected_test', $data);
        //$this->load->view('footer');
    }
...
//$data['unit_list'] = $this->add_result_model->unit_list();
        $this->load->view('header');
        //$this->load->view('nav', $data);
        $this->load->view('tech/view_collected_test1', $data);
        //$this->load->view('footer');
    }
    ...
    function add_value_exists_old() {
        ...
        $single_test_fk = $this->uri->segment(4);
        //echo "<pre>";
        //print_r($_POST);
        //die();
        $data["login_data"] = logindata();
    ...
    function add_value_exists() {
        ...
        $count = $this->input->post('count');
        $para_job_id = $this->input->post('para_job_id');
		$newdata['hb'] = $this->input->post('HB');
		$newdata['mcv'] = $this->input->post('MCV');
		$newdata['plt_cnt'] = $this->input->post('PLT');
		$newdata['total_wbc_cnt'] = $this->input->post('TWC');
		$newdata['polymorphs'] = $this->input->post('POL');
		$newdata['lymphocytes'] = $this->input->post('LYM');
		$newdata['monocytes'] = $this->input->post('MON');
		$newdata['eosinophils'] = $this->input->post('EOS');
		$newdata['basophils'] = $this->input->post('BAS');
		$newdata['ssrbc'] = $this->input->post('SSRBC');
		$newdata['sswbc'] = $this->input->post('SSWBC');
		$newdata['ssplt'] = $this->input->post('SSPLT');
		$newdata['sspsmp'] = $this->input->post('SSPSMP');
        ...
        /* CHECK AUTO APPROVE START */
        $this->check_auto_approve($para_job_id, $send_approve_test_id_array);
        /* END */
        ...
        /* END */
        redirect('tech/add_result/test_details/' . $para_job_id . '/' . $single_test_fk);
    }

    function check_auto_approve($job_fk, $test_fk) {
        $data['cid'] = $job_fk;
        $data['query'] = $this->add_result_model->job_details($job_fk);
        $data['processing_center'] = $this->add_result_model->master_fun_get_tbl_val("processing_center", array('status' => 1, 'lab_fk' => $data['query'][0]["branch_fk"]), array("id", "asc"));
        if (empty($data['processing_center'])) {
            $processing_center = '1';
        } else {
        ...
        foreach ($tid as $tst_id) {
            $check_allow_test = $this->add_result_model->get_val("SELECT id FROM `auto_approve_test` WHERE `status`='1' AND `test_fk`='" . $tst_id . "'");
            if (!empty($check_allow_test)) {
                $get_test_parameter = $this->add_result_model->get_val("SELECT `test_parameter`.*,`test_master`.`test_name`,`test_master`.PRINTING_NAME,`test_master`.report_type FROM `test_parameter` INNER JOIN `test_master` ON `test_parameter`.`test_fk`=`test_master`.`id` WHERE `test_parameter`.`status`='1' AND `test_master`.`status`='1' AND `test_parameter`.`test_fk`='" . $tst_id . "' and `test_parameter`.`center`='" . $processing_center . "' order by `test_parameter`.order asc");
                $pid = array();
                foreach ($get_test_parameter as $tp_key) {
                    if (!empty($tp_key["parameter_fk"])) {
                ...
                if (!empty($pid)) {
                    $para = $this->add_result_model->get_val("SELECT id,parameter_name,parameter_unit FROM `test_parameter_master` WHERE is_group=0 and `status`='1' AND id IN (" . implode(",", $pid) . ") and parameter_name !='' ORDER BY FIELD(id," . implode(",", $pid) . ")");
                    if (!empty($para)) {
                        $cnt_1 = 0;
                        $new_para_array = array();
                        foreach ($para as $para_key) {
                            $formula = $this->add_result_model->get_val("SELECT * from use_formula where test_fk='" . $tst_id . "' and job_fk='" . $data['cid'] . "' and status='1'");
                            $para_user_val = $this->add_result_model->get_val("SELECT id,job_id,test_id,parameter_id,value from user_test_result where test_id='" . $tst_id . "' and parameter_id='" . $para_key["id"] . "' and job_id='" . $data['cid'] . "' and  status='1'");
                            $para[$cnt_1]["user_val"] = $para_user_val;
                            if (!empty($para_user_val)) {
                                $para_ref_rng = $this->add_result_model->get_val("SELECT ref_range_low,ref_range_high FROM `parameter_referance_range` WHERE `status`='1' AND parameter_fk='" . $para_key["id"] . "' order by gender asc");
                                $final_qry = "SELECT id,ref_range_low,ref_range_high,
  CASE
    WHEN (type_period = 'Y') 
    THEN (no_period * 365) 
    ELSE (
  ...
  END AS col1  FROM `parameter_referance_range` WHERE STATUS='1' AND `parameter_fk`='" . $para_key["id"] . "'";
        ...
        $data["new_data_array"] = $new_data_array;
        $cnt = 0;
		$hbid = $mcvid = $platelets_cntid = $total_wbc_cntid = $polymorphsid = $lymphocytesid = $monocytesid = $eosinophilsid = $basophilsid = null;
		$ssrbcid = $sswbcid = $sspltid = $sspsmpid = null;
		
		if($this->session->userdata('hb')!=null){
			$hbid = $this->session->userdata('hb');
			$this->session->unset_userdata('hb');
		}
		if($this->session->userdata('mcv')!=null){
			$mcvid = $this->session->userdata('mcv');
			$this->session->unset_userdata('mcv');
		}
		if($this->session->userdata('plt_cnt')!=null){
			$platelets_cntid = $this->session->userdata('plt_cnt');
			$this->session->unset_userdata('plt_cnt');
		}
		if($this->session->userdata('total_wbc_cnt')!=null){
			$total_wbc_cntid = $this->session->userdata('total_wbc_cnt');
			$this->session->unset_userdata('total_wbc_cnt');
		}
		if($this->session->userdata('polymorphs')!=null){
			$polymorphsid = $this->session->userdata('polymorphs');
			$this->session->unset_userdata('polymorphs');
		}
		if($this->session->userdata('lymphocytes')!=null){
			$lymphocytesid = $this->session->userdata('lymphocytes');
			$this->session->unset_userdata('lymphocytes');
		}
		if($this->session->userdata('monocytes')!=null){
			$monocytesid = $this->session->userdata('monocytes');
			$this->session->unset_userdata('monocytes');
		}
		if($this->session->userdata('eosinophils')!=null){
			$eosinophilsid = $this->session->userdata('eosinophils');
			$this->session->unset_userdata('eosinophils');
		}
		if($this->session->userdata('basophils')!=null){
			$basophilsid = $this->session->userdata('basophils');
			$this->session->unset_userdata('basophils');
		}
		if($this->session->userdata('ssrbc')!=null){
			$ssrbcid = $this->session->userdata('ssrbc');
			$this->session->unset_userdata('ssrbc');
		}
		if($this->session->userdata('sswbc')!=null){
			$sswbcid = $this->session->userdata('sswbc');
			$this->session->unset_userdata('sswbc');
		}
		if($this->session->userdata('ssplt')!=null){
			$sspltid = $this->session->userdata('ssplt');
			$this->session->unset_userdata('ssplt');
		}
		if($this->session->userdata('sspsmp')!=null){
			$sspsmpid = $this->session->userdata('sspsmp');
			$this->session->unset_userdata('sspsmp');
		}
		
        foreach ($new_data_array as $knew) {
            $its_normal = 1;
            if ($knew[0]["parameter"][0]["user_val"][0]["test_id"]!=617 && $knew[0]["parameter"][0]["user_val"][0]["test_id"]!=236) {
                foreach ($knew[0]["parameter"] as $tpkey) {
                    if ($tpkey["user_val"][0]["value"] < $tpkey["para_ref_rng"][0]["ref_range_low"] || $tpkey["user_val"][0]["value"] > $tpkey["para_ref_rng"][0]["ref_range_high"]) {
                        $its_normal = 0;
                    }
                }
            }
			
			$hemoglobin = $mcv = $platelets_cnt = $total_wbc_cnt = $Polymorphs = $lymphocytes = $Monocytes = $Eosinophils = $Basophils = null;
                        
                        
                        
                            
                            			if($knew[0]["parameter"][0]["user_val"][0]["test_id"]==236){
				foreach ($knew[0]["parameter"] as $tpkey) {
					if ($tpkey["user_val"][0]["value"] < $tpkey["para_ref_rng"][0]["ref_range_low"] || $tpkey["user_val"][0]["value"] > $tpkey["para_ref_rng"][0]["ref_range_high"]) {												
						if($tpkey["user_val"][0]["parameter_id"]!=$ssrbcid && $tpkey["user_val"][0]["parameter_id"]!=$sswbcid && $tpkey["user_val"][0]["parameter_id"]!=$sspltid && $tpkey["user_val"][0]["parameter_id"]!=$sspsmpid){
							$its_normal = 1; 
						}
					}
					//rbcid - 43871, wbcid - 43877, pltid - 43878, psmpid - 43879
					
					if($tpkey["user_val"][0]["parameter_id"]==$hbid){//43790
						$hemoglobin = $tpkey["user_val"][0]["value"];
					}else if($tpkey["user_val"][0]["parameter_id"]==$mcvid){//44671
						$mcv = $tpkey["user_val"][0]["value"];
					}else if($tpkey["user_val"][0]["parameter_id"]==$platelets_cntid){//44522
						$platelets_cnt = $tpkey["user_val"][0]["value"];
					}else if($tpkey["user_val"][0]["parameter_id"]==$total_wbc_cntid){//44521
						$total_wbc_cnt = $tpkey["user_val"][0]["value"];
					}else if($tpkey["user_val"][0]["parameter_id"]==$polymorphsid){//43798
						$Polymorphs = $tpkey["user_val"][0]["value"];
					}else if($tpkey["user_val"][0]["parameter_id"]==$lymphocytesid){//43702
						$lymphocytes = $tpkey["user_val"][0]["value"];
					}else if($tpkey["user_val"][0]["parameter_id"]==$monocytesid){//43873
						$Monocytes = $tpkey["user_val"][0]["value"];
					}else if($tpkey["user_val"][0]["parameter_id"]==$eosinophilsid){//43874
						$Eosinophils = $tpkey["user_val"][0]["value"];
					}else if($tpkey["user_val"][0]["parameter_id"]==$basophilsid){//43803
						$Basophils = $tpkey["user_val"][0]["value"];
					}
				}	
			
				//echo $its_normal;
				if($its_normal == 1 && ($hemoglobin!=null && $hemoglobin>=6 && $hemoglobin<=7.99) && ($mcv!=null && $mcv >100)){
					$its_normal = 1;
				}else if($its_normal == 1 && ($hemoglobin!=null && $hemoglobin>=6 && $hemoglobin<=7.99) && ($mcv!=null && $mcv <80)){
					$its_normal = 1;
				}else if($its_normal == 1 && ($hemoglobin!=null && $hemoglobin>=6 && $hemoglobin<=7.99) && ($mcv!=null && $mcv >=80 && $mcv<=100)){
					$its_normal = 1;
				}else if($its_normal == 1 && ($hemoglobin!=null && $hemoglobin>=8 && $hemoglobin<=9.99) && ($mcv!=null && $mcv >100)){
					$its_normal = 1;
				}else if($its_normal == 1 && ($hemoglobin!=null && $hemoglobin>=8 && $hemoglobin<=9.99) && ($mcv!=null && $mcv <80)){
					$its_normal = 1;
				}else if($its_normal == 1 && ($hemoglobin!=null && $hemoglobin>=8 && $hemoglobin<=9.99) && ($mcv!=null && $mcv >=80 && $mcv<=100)){
					$its_normal = 1;
				}else if($its_normal == 1 && ($hemoglobin!=null && $hemoglobin>=10 && $hemoglobin<=12.99) && ($mcv!=null && $mcv >100)){
					$its_normal = 1;
				}else if($its_normal == 1 && ($hemoglobin!=null && $hemoglobin>=10 && $hemoglobin<=12.99) && ($mcv!=null && $mcv <80)){
					$its_normal = 1;
				}else if($its_normal == 1 && ($hemoglobin!=null && $hemoglobin>=10 && $hemoglobin<=12.99) && ($mcv!=null && $mcv >=80 && $mcv<=100)){
					$its_normal = 1;
				}else if($its_normal == 1 && ($hemoglobin!=null && $hemoglobin>=13 && $hemoglobin<=17) && ($mcv!=null && $mcv >100)){
					$its_normal = 1;
				}else if($its_normal == 1 && ($hemoglobin!=null && $hemoglobin>=13 && $hemoglobin<=17) && ($mcv!=null && $mcv <80)){
					$its_normal = 1;
				}else if($its_normal == 1 && ($hemoglobin!=null && $hemoglobin>=13 && $hemoglobin<=17) && ($mcv!=null && $mcv >=80 && $mcv<=100)){
					$its_normal = 1;
				}else{
					$its_normal = 0;
				}
				...
				if($its_normal == 1 && $platelets_cnt!=null && $platelets_cnt<50000){
					$its_normal = 1;
				}else if($its_normal == 1 && $platelets_cnt!=null && ($platelets_cnt>=50000 && $platelets_cnt<=99999)){
					$its_normal = 1;
				}else if($its_normal == 1 && $platelets_cnt!=null && ($platelets_cnt>=100000 && $platelets_cnt<=149999)){
					$its_normal = 1;
				}else if($its_normal == 1 && $platelets_cnt!=null && ($platelets_cnt>=150000 && $platelets_cnt<=450000)){
					$its_normal = 1;
				}else if($its_normal == 1 && $platelets_cnt!=null && ($platelets_cnt>=450001 && $platelets_cnt<=600000)){
					$its_normal = 1;
				}else{
					$its_normal = 0;
				}
				...
				//echo $its_normal;
				if($its_normal == 1 && ($total_wbc_cnt!=null && $total_wbc_cnt>=4000 && $total_wbc_cnt<=10000)){
					if(($Polymorphs>=40 && $Polymorphs<=70) && ($lymphocytes>=20 && $lymphocytes<=40) && ($Monocytes>=2 && $Monocytes<=10) && ($Eosinophils>=1 && $Eosinophils<=6) && ($Basophils>=0 && $Basophils<=2)){
						$its_normal = 1;
					}else if($Polymorphs>70){
						$its_normal = 1;
					}else if($lymphocytes>40){
						$its_normal = 1;
					}else if($Eosinophils>6){
						$its_normal = 1;
					}else if($Monocytes>10){
						$its_normal = 1;
					}else{
						$its_normal = 0;
					}
				}else if($its_normal == 1 && ($total_wbc_cnt!=null && $total_wbc_cnt>10000)){
					if($Polymorphs>70){
						$its_normal = 1;
					}else if($lymphocytes>40){
						$its_normal = 1;
				...
				}else if($its_normal == 1 && ($total_wbc_cnt!=null && $total_wbc_cnt<4000)){
					if($Polymorphs>70){
						$its_normal = 1;
					}else if($lymphocytes>40){
						$its_normal = 1;
            ...
            /* Approval Time Calculation
              From Monday to Saturday 9:00 AM to 9:30 PM
              For Sunday 9:00 AM to 1:00 PM
              Also Before 10 Minutes it will take approval date to next working day time
             */

            $day = date("w"); //check week day
            //$day = 0;
            $starttime = "9:00 AM";
            $endtime = "9:30 PM";
            if ($day == 0) {
            ...
            $currentdate = date("Y-m-d H:i:s"); //, strtotime("8:22 PM")
            ...
            /* Check old entry start */
            $rand = mt_rand(1, 5); //Random approval minutes between 1 to 5
            $approve_time = date("Y-m-d H:i:s", strtotime("+" . $rand . " minutes"));

            if ($currentdate > $startdate && $currentdate < $enddate) {
                $differenceminutes = (strtotime($enddate) - strtotime($currentdate)) / 60;
                if ($differenceminutes > 10) {
                    $approve_time = date("Y-m-d H:i:s", strtotime("+" . $rand . " minutes"));
                } else {
                    $approve_time = date("Y-m-d H:i:s", strtotime("+1 day 10:" . $rand . ":00"));
                }
            } else {
                if (date("a") == "pm") {//,strtotime("AM")				
                    $approve_time = date("Y-m-d H:i:s", strtotime("+1 day 10:" . $rand . ":00"));
                } else {
                    $approve_time = date("Y-m-d H:i:s", strtotime("10:" . $rand . ":00"));
                }
            }
		
            $old_data = $this->add_result_model->get_val("select id from auto_approve_pending_job where job_fk='" . $job_fk . "' and test_fk='" . $knew[0]["parameter"][0]["para_ref_rng"][0]["tst_id"] . "'");
            if (!empty($old_data)) {
                if ($its_normal == 1) {
                    $this->add_result_model->master_fun_update1("auto_approve_pending_job", array("job_fk" => $job_fk, "test_fk" => $knew[0]["parameter"][0]["para_ref_rng"][0]["tst_id"]), array("status" => "2", "approve_time" => $approve_time));
                } else {
                    $this->add_result_model->master_fun_update1("auto_approve_pending_job", array("job_fk" => $job_fk, "test_fk" => $knew[0]["parameter"][0]["para_ref_rng"][0]["tst_id"]), array("status" => "0"));
                }
            } else {
                /* END */
                if ($its_normal == 1) {
                    if($data['query'][0]["branch_fk"] == "89" && $knew[0]["parameter"][0]["user_val"][0]["test_id"]==236){
                        
                    }else{
                        $this->add_result_model->master_fun_insert("auto_approve_pending_job", array("job_fk" => $job_fk, "test_fk" => $knew[0]["parameter"][0]["para_ref_rng"][0]["tst_id"], "approve_time" => $approve_time));
                    }
                }
            }
            $cnt++;
    ...
    function add_value_exists1() {
        ...
        redirect('tech/add_result/test_approve_details/' . $para_job_id . "/" . $tid);
    }
    ...
    function add_value_exists2() {
        ...
        $this->add_result_model->master_fun_insert("job_log", array("job_fk" => $para_job_id, "created_by" => "", "updated_by" => $adminid, "deleted_by" => "", "job_status" => '', "message_fk" => "32", "date_time" => date("Y-m-d H:i:s")));
        //$this->session->set_userdata("closeFancyBox", array('parent.close_popup1();'));
        redirect('tech/add_result/all_test_approve_details?jid=' . $para_job_id . '&tid=' . $this->input->post('tid'));
    }
    ...
    function change_order() {
        ...
        redirect('tech/add_result/test_details/' . $para_job_id);
    }
...
//echo "SELECT * from user_test_result where test_id='".$tst_id."' and parameter_id='" . $para_key["id"] . "' and job_id='".$data['cid']."' and  status='1'";
        ...
        redirect("tech/job_master/job_details/" . $id);
    }
...
//$this->add_result_model->master_fun_update('test_parameter_master', array('id', $pid), $data1);
    ...
    function add_parameter() {
        ...
        $data['event'] = $this->uri->segment(5);
        if ($data['event'] != 'add') {
            $data['edit_p_tid'] = $this->uri->segment(5);
        }
        $data['branch_fk'] = $this->uri->segment(6);
...
WHERE tm.id = '" . $tst_id . "' ");
            ...
            $check_test_status = $this->add_result_model->get_val("select * from use_formula where status='1' AND job_fk='$id'");
            $count11 = 0;
            foreach ($check_test_status as $key) {
...
WHERE tm.id = '" . $tst_id . "' ");
    ...
    function job_mark_completed($cid) {
        $this->load->model('tech/job_model');
        if (!is_loggedin()) {
            redirect('login');
        }
        $this->load->library("util");
		...
		<p style="color:#7e7e7e;font-size:13px;">Thank You.</p>
            ...
            //redirect("tech/job_master/pending_list", "refresh");
        }
    }
...
// Case Back Email start
		...
		<p style="color:#7e7e7e;font-size:13px;">Thank You.</p>
                    ...
                    $this->email->to(implode(",", $c_email));
                    if ($branch == 96) {
                        $this->email->cc('badshah14298@gmail.com'); //badshah14298@gmail.com for Airmed Tech SURAT - UNITY PATHOLOGY LAB Branch
                    }
                    //  $this->email->to('jeel@virtualheight.com');
                    $this->email->from($this->config->item('admin_booking_email'), "AirmedLabs");
                    $this->email->subject("Report Completed");
