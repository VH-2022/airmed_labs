...
class Job_master extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('tech/registration_admin_model');
    ...
    function pending_list() {
        ...
        $this->load->view('tech/pending_job_list_new', $data);
...
//echo "<pre>";print_r($key);
        ...
        $url = "http://" . $_SERVER['HTTP_HOST'] . ":" . $_SERVER['SERVER_PORT'] . $_SERVER['REQUEST_URI'];
        $this->session->set_userdata("job_master_r", $url);
        $this->load->view('tech/pending_job_list_search', $data);
    }
...
//echo "<pre>"; print_R($data['query']); die();
    ...
    function changing_status_job() {
        ...
        if ($status == 7) {
            ...
            $status_update = $this->job_model->master_fun_update("job_master", array("id", $job_id), array("status" => 7));
            $this->job_model->master_fun_insert("job_log", array("job_fk" => $job_id, "created_by" => "", "updated_by" => $data["login_data"]["id"], "deleted_by" => "", "job_status" => "6-7", "message_fk" => "3", "date_time" => date("Y-m-d H:i:s")));
            
            $status_update = $this->job_model->master_fun_update("job_master", array("id", $job_id), array("status" => 8));
            $this->job_model->master_fun_insert("job_log", array("job_fk" => $job_id, "created_by" => "", "updated_by" => $data["login_data"]["id"], "deleted_by" => "", "job_status" => "7-8", "message_fk" => "3", "date_time" => date("Y-m-d H:i:s")));
            echo 1; die();  
        }
    ...
    function getstock() {
        ...
        /* END */
        $get_approve_test_parameter = $this->add_result_model->get_val("SELECT GROUP_CONCAT(test_fk) AS test_fk FROM `approve_job_test` WHERE `status`='1' AND test_fk>0 AND job_fk='" . $jobid . "' GROUP BY job_fk");
    ...
    function prescription_spam($id) {
        ...
        redirect("job_master/prescription-report", "refresh");
    }
...
WHERE sample_job_master_receiv_amount.job_fk = '" . $data["b2b_job_detais"][0]["id"] . "' 
  AND sample_job_master_receiv_amount.status = '1'");
        ...
        $data['alertdetils'] = $this->registration_admin_model->get_val("SELECT smsalert,emailalert from branch_master where  status='1' and city='" . $data['branchdis'] . "'");
...
//print_r($data['query']); die();
        ...
        $data['phlebo_assign_job'] = $this->job_model->master_fun_get_tbl_val("phlebo_assign_job", array('status' => 1, "job_fk" => $this->uri->segment(4)), array("id", "asc"));
        $p_prc = 0;
...
//echo "<pre>"; print_R($data["query"]); die();
        ...
        $this->load->view('header');
        $this->load->view('nav', $data);
        $this->load->view('tech/job_details', $data);
        $this->load->view('footer');
    }
  ...
  END AS col1  FROM `parameter_referance_range` WHERE STATUS='1' AND `parameter_fk`='" . $para_key["id"] . "'";
    ...
    function export_csv() {
        ...
        if (!empty($data["login_data"]['branch_fk'])) {
            foreach ($data["login_data"]['branch_fk'] as $key) {
                $cntr_arry[] = $key["branch_fk"];
                $cntr_arry_fnl[] = $key["branch_fk"];
            }
            $data['branchlist'] = $this->registration_admin_model->get_val("SELECT * from branch_master where status='1' and id in (" . implode(",", $cntr_arry_fnl) . ")");
        }
        $data['test_cities'] = $this->registration_admin_model->get_val("SELECT * from test_cities where status='1'");
        $test_packages = explode("_", $test_pack);
        $alpha = $test_packages[0];
        ...
        if (empty($branch) && $plm != '') {
            if (empty($branch) && !empty($city) && empty($plm)) {
                $plm_branch = $this->job_model->get_val("SELECT GROUP_CONCAT(id) AS id FROM `branch_master` WHERE `status`='1' AND city='" . $city . "' and id in (" . implode(",", $cntr_arry_fnl) . ") GROUP BY status");
            }
            if (empty($branch) && !empty($plm)) {
                $plm_branch = $this->job_model->get_val("select GROUP_CONCAT(id) AS id from branch_master where status='1' and (parent_fk ='" . $plm . "' or id='" . $plm . "') and id in (" . implode(",", $cntr_arry_fnl) . ") GROUP BY status");
            }
            if (empty($branch) && empty($city) && empty($plm)) {
                $plm_branch = $this->job_model->get_val("select GROUP_CONCAT(id) AS id from branch_master where status='1' and id in (" . implode(",", $cntr_arry_fnl) . ") GROUP BY status");
            }
            $plm_branch = explode(",", $plm_branch[0]["id"]);
            $cntr_arry = array_merge(array($plm), $plm_branch);
        } else {
            ...
            if (!empty($_FILES['userfile']['name'])) {
                for ($i = 0; $i < $file_loop; $i++) {
                    ...
                    if (!$this->upload->do_upload()) {
                        ...
                        redirect('job_master/job-details/' . $cid);
                    } else {
    ...
    function remove_report() {
        ...
        redirect("tech/job_master/job_details/" . $jid, "refresh");
    }
    ...
    function prescription_report() {
        ...
        if ($user != "" || $date != "" || $mobile != "" || $status != "") {
            ...
            $get = $_GET;
            unset($get['offset']);
            $config["base_url"] = base_url() . "job_master/prescription-report?" . http_build_query($get);
            $config["total_rows"] = $total_row;
            $config["per_page"] = 100;
            $config['page_query_string'] = TRUE;
            $config['cur_tag_open'] = '<span>';
            ...
            $config["base_url"] = base_url() . "job_master/prescription-report";
            $config["total_rows"] = $total_row;
            $config["per_page"] = 100;
            $config['page_query_string'] = TRUE;
            $config['cur_tag_open'] = '<span>';
    ...
    function prescription_details() {
        ...
        $upd = $this->job_model->master_fun_update("prescription_upload", array("id", $this->uri->segment(4)), array("is_read" => "1"));
        $data["login_data"] = logindata();
        $data["user"] = $this->user_model->getUser($data["login_data"]["id"]);
        $data['cid'] = $this->uri->segment(4);
        $data['success'] = $this->session->flashdata("success");
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            $upd = $this->job_model->master_fun_update("prescription_upload", array("id", $this->uri->segment(4)), array("status" => "2"));
        ...
        $this->job_model->master_fun_update("prescription_upload", array("id", $this->uri->segment(4)), array("is_read" => "1"));
...
<script  type="text/javascript">
    ...
    function suggest_test() {
        ...
        $this->session->set_flashdata("success", array("Test Suggested Successfully"));
        redirect("job_master/prescription-details/" . $cid, "refresh");
    }
...
<p style="color:#7e7e7e;font-size:13px;"> Your Booked Amount is Rs. ' . $payable . '  </p>
        ...
        $this->session->set_flashdata("success", array("Booked successfully"));
        redirect("job_master/prescription-details/" . $id, "refresh");
    }
...
//	$ctn = 1 ;
    ...
    function prescription_submit() {
        ...
        $data["login_data"] = logindata();
        $data["user"] = $this->user_model->getUser($data["login_data"]["id"]);
        $cid = $data['cid'] = $this->uri->segment(4);
        $data['success'] = $this->session->flashdata("success");
        $data['error'] = $this->session->flashdata("error");
...
<p style="color:#7e7e7e;font-size:13px;"> Password : ' . $password . '  </p>
            ...
            if ($submit_type == '1') {
                ...
                //print_r($data); die();
                $insert = $this->job_model->master_fun_insert("job_master", $data);
                $this->job_model->master_fun_update("prescription_upload", array("id", $this->uri->segment(4)), array("job_fk" => $insert));
                $testid = array();
                $packageid = array();
                foreach ($test as $key) {
                    $tn = explode("-", $key);
        ...
        <p style="color:#7e7e7e;font-size:13px;"> Payment Type : Cash on Blood Collection</p>
            ...
            if ($submit_type == '0') {
                ...
                if (count(array_count_values($test)) != count($test)) {
                    ...
                    redirect("job_master/prescription-details/$cid");
                }
                $desc = $this->input->post('desc');
                $discount = $this->input->post('discount');
                $this->job_model->master_fun_update("prescription_upload", array("id", $this->uri->segment(4)), array("discount" => $discount, "city" => $this->input->post('test_city')));
                $update = $this->job_model->master_fun_update_multi("suggested_test", array('p_id' => $cid), array("status" => 0));
                $price = 0;
                $test_name_mail = array();
...
//}
    ...
    function payment_received() {
        ...
        $sms_alert = $this->job_model->get_val("select smsalert from branch_master where status='1' AND id='" . $job_details[0]["branch_fk"] . "'")[0]['smsalert'];
        $this->check_send_report($jid);
        redirect("tech/job_master/job_details/" . $jid);
    }
    ...
    function change_doctor() {
        ...
        if ($jid != '' && $referral_by != '') {
            ...
            redirect("tech/job_master/pending_list");
        }
    }
...
ORDER BY `branch_name` ASC");
    ...
    function send_report_sms1() {
        ...
        redirect("tech/job_master/job_details/" . $job_fk, "refresh");
    }
    ...
    function accep_assign_phlebo_job() {
        ...
        if ($insert) {
            ...
            redirect("tech/job_master/job_details/" . $job_id);
        } else {
            redirect("tech/job_master/pending_list");
        }
    }
...
Volume :-  Adequate 
 ...
 Fungus :-  Absent .";
    ...
    function get_doctor_test_list($city_fk = 1) {
        $city_fk = $this->uri->segment(4);
        $branch_fk = $this->uri->segment(6);
        $doctor_fk = $this->uri->segment(6);
        ...
        if ($branch_fk > 0) {
            $test = $this->job_model->get_val("select IF(cut>0,cut,0) as cut,jobdiscount from branch_master where id='" . $this->uri->segment(6) . "'");
            $cut = $test[0]["cut"];
...
//			<option value="">--Select Test--</option>';
    ...
    function update_register_date($cid) {
        ...
        redirect("tech/job_master/job_details/" . $cid, "refresh");
    }
