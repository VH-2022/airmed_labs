    ...
    function edit_test_price($id) {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            if ($ttype == 2) {
                ...
                $q = "SELECT tp.id,tp.price,tp.test_fk,tp.branch_fk,tp.status,tm.title AS test_name FROM package_master tm INNER JOIN test_branch_price tp ON tp.test_fk = tm.id   WHERE tp.branch_fk='$id' AND tm.status ='1' AND tp.`type`='2'";
...
//                    ";
    ...
    function branch_test_export_csv($id) {
        ...
        if (!empty($id)) {
            ...
            $cnt = 0;
            fputcsv($handle, array("ID", "Branch Code", "Test Name", "Price"));
            foreach ($branch_wise_test as $key) {
//                fputcsv($handle, array(++$cnt, $key->id, $key->branch_fk, $key->test_name, $key->price));
                fputcsv($handle, array($key->id, $key->branch_fk, $key->test_name, $key->price));
            }
            fclose($handle);
            exit;
        }
