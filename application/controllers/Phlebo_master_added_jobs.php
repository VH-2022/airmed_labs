...
class Phlebo_master_added_jobs extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('Phlebo_master_addjob_model');
    ...
    function index() {
        ...
        $data['view_report'] = $this->Phlebo_master_addjob_model->report($start_date, $end_date, $city, $phlebo_name, $sample);
        ...
        if (!empty($cid)) {
            $data['phlebo_list'] = $this->Phlebo_master_addjob_model->get_val("select pm.name as PhleboName,pm.id as PID,pm.test_city,tc.id from phlebo_master as pm left join test_cities as tc on pm.test_city = tc.id where pm.test_city = '$cid' AND pm.status='1'");
        } else {
            $data['phlebo_list'] = $this->Phlebo_master_addjob_model->get_val("select pm.name as PhleboName,pm.id as PID from phlebo_master as pm where pm.status='1'");
        }
        
        $data["phlebo_list1"] = $this->master_model->master_fun_get_tbl_val("phlebo_master", array("status" => "1"), array("id", "asc"));
        $this->load->view('header');
        $this->load->view('nav', $data);
        $this->load->view('phlebo_master_addjob_report_view', $data);
    ...
    function getPhleboName() {
        ...
        $refer = '';
        $referral_list = $this->Phlebo_master_addjob_model->get_val("Select ph.id,ph.name as PhleboName,tc.name from phlebo_master as ph left join test_cities as tc on tc.id = ph.test_city where ph.test_city='" . $city . "' AND ph.status = 1");
    ...
    function export() {
        ...
        $result = $this->Phlebo_master_addjob_model->csv_report($start_date, $end_date, $phlebo, $city, $sm_coll);
        if ($_REQUEST["debug"] == 1) {
            print_r($result);
            die();
        }
