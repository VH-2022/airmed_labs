...
//$this->email->to("jeel@virtualheight.com");
  ...
  WHERE `package_master`.`status` = '1' 
    ...
    AND `package_master_city_price`.`city_fk` = '" . $data[0]["test_city"] . "'");
                ...
                $data_new[$cnt]["description"] = $tst_details[0]["description"];
                $data_new[$cnt]["created_date"] = $data[0]["date"];
                $data_new[$cnt]["updated_date"] = "";
    ...
    function check_user_address($uid = null) {
        if ($uid != null) {
            $user_details = $this->service_model->master_fun_get_tbl_val("customer_master", array("status" => 1, 'id' => $uid), array("id", "asc"));
    ...
    function add_job() {
        ...
        if ($user_fk != "" && $testfk != "") {
            if ($user_details["address"]) {
                $insert = $this->service_model->master_fun_insert("job_master", array("cust_fk" => $user_fk, "address" => $user_details["address"], "date" => $date, "order_id" => $order_id, "doctor" => $doctor, "other_reference" => $other_reference, "payment_type" => "Wallet", "portal" => $type));
                $test = explode(',', $testfk);
                $price = 0;
...
<p style="color:#7e7e7e;font-size:13px;"> Rs. ' . $caseback_amount . ' Credited in your Wallet. </p>
        ...
        <div style="width:100%;float:left;background:#333333;padding:1.5%;color:#ccc;font-size:12px;border:1px solid #323232;text-align:center;">
                ...
                echo $this->json_data("0", "Please add your current address in your profile and book test.", "");
            }
        } else {
            echo $this->json_data("0", "Parameter not passed", "");
        }
    ...
    function add_job_half_payment() {
        ...
        if ($user_fk != "" && $testfk != "") {
            ...
            if ($user_details["address"]) {
                ...
                $insert = $this->service_model->master_fun_insert("job_master", array("cust_fk" => $user_fk, "address" => $user_details["address"], "date" => $date, "status" => 0, "order_id" => $order_id, "doctor" => $doctor, "other_reference" => $other_reference, "payment_type" => "PayUMoney", "portal" => $type));
                $test = explode(',', $testfk);
                $price = 0;
                for ($i = 0; $i < count($test); $i++) {
                    $insert_code = $this->service_model->master_fun_insert("job_test_list_master", array("job_fk" => $insert, "test_fk" => $test[$i]));
...
<p style="color:#7e7e7e;font-size:13px;"><b>Message:</b> ' . $message1 . ' </p>
    ...
    function health_feed() {
        ...
        $configuration = array("MERCHANT_ID" => $MERCHANT_ID, "MERCHANT" => $MERCHANT_KEY, "SALT" => $SALT, 'URl' => $URL);
        ...
        if ($user_details["address"]) {
            if ($total >= $amount) {
                if ($user_fk != "" && $amount != "") {
                    ...
                    $insert1 = $this->service_model->master_fun_insert("job_master", array("cust_fk" => $user_fk, "address" => $user_details["address"], "price" => $amount, "date" => $date, "order_id" => $order_id, "payment_type" => "wallet", "portal" => $type));
...
<p style="color:#7e7e7e;font-size:13px;"> Rs. ' . $caseback_amount . ' Credited in your Wallet. </p>
        ...
        <div style="width:100%;float:left;background:#333333;padding:1.5%;color:#ccc;font-size:12px;border:1px solid #323232;text-align:center;">
                ...
                echo $this->json_data("0", "Invalid Amount", "");
            }
        } else {
            echo $this->json_data("0", "Please add your current address in your profile and book package.", "");
        }
    ...
    function package_payu_success() {
        ...
        $user_details = $this->check_user_address($userfk1);
        if ($user_details["address"]) {
            $order_id = random_string('numeric', 13);
            $date = date('Y-m-d H:i:s');
            ...
            $insert1 = $this->service_model->master_fun_insert("job_master", array("cust_fk" => $userfk1, "address" => $user_details["address"], "price" => $price, "date" => $date, "order_id" => $order_id, "payment_type" => "PayUMoney", "status" => 6, "portal" => $type));
...
<script src="https://use.fontawesome.com/44049e3dc5.js"></script>
    ...
    function cash_on_delivery() {
        ...
        if ($uid != NULL && $select_tests != Null && $total != Null) {
            $user_details = $this->check_user_address($uid);
            //echo "<pre>"; print_r($user_details); die();
            if ($user_details["address"] != null) {
                $data = array(
                    "order_id" => $order_id,
                    "cust_fk" => $uid,
                    "date" => $date,
...
' . $payfrom_wallet . '
        ...
        <div style="width:100%;float:left;background:#333333;padding:1.5%;color:#ccc;font-size:12px;border:1px solid #323232;text-align:center;">
                Copyright @ 2016 AirmedLabs. All rights reserved
                    ...
                    $this->email->to($destail[0]['email']);
                    $this->email->cc($this->config->item('admin_alert_phone'));
                    $this->email->from('donotreply@airmedpathlabs.com', 'AirmedLabs');
                    $this->email->subject('Test Book Successfully');
                    $this->email->message($message);
                    $this->email->send();
