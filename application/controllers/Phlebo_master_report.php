...
class Phlebo_master_report extends CI_Controller {
    ...
    function index() {
        ...
        if (!empty($cid)) {
            $data['phlebo_list'] = $this->phlebo_report_model->get_val("select pm.name as PhleboName,pm.id as PID,pm.test_city,tc.id from phlebo_master as pm left join test_cities as tc on pm.test_city = tc.id where pm.test_city = '$cid' AND pm.status='1'");
        } else {
//          $data['phlebo_list'] = $this->phlebo_report_model->get_val("select pm.name as PhleboName,pm.id as PID,pm.test_city,tc.id from phlebo_master as pm left join test_cities as tc on pm.test_city = tc.id");
            $data['phlebo_list'] = $this->phlebo_report_model->get_val("select pm.name as PhleboName,pm.id as PID from phlebo_master as pm where pm.status='1'");
        }
        
        $data["phlebo_list1"] = $this->master_model->master_fun_get_tbl_val("phlebo_master", array("status" => "1"), array("id", "asc"));
        $this->load->view('header');
        $this->load->view('nav', $data);
        $this->load->view('phlebo_master_report_view', $data);
