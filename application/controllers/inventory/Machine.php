...
class Machine extends CI_Controller {
    ...
    function machine_list() {
        ...
        $data['branchlist'] = $this->Test_item_model->get_data("select id,branch_name from branch_master where status='1'");
			...
			$data['branchlist'] = $this->Test_item_model->get_data("select id,branch_name from branch_master where status='1' and id in(SELECT `branch_fk`  FROM `user_branch`  WHERE STATUS = '1' AND user_fk = " . $login_id . ") "); 
		 }
...
//      
            ...
            if (!empty($insert)) {
                foreach ($reg as $key => $value) {
                    ...
                    $this->Test_item_model->master_fun_insert("inventory_item", array('machine ' => $insert, "reagent_name" => $value,
                        "quantity" => '1', "unit_fk" => $this->input->post('unit_fk')[$count], "test_quantity" => $this->input->post('test_quantity')[$count],"per_pack" => $this->input->post('per_pack')[$count],"brand_fk" => $this->input->post('brand_fk')[$count],"remark" => $this->input->post('remark')[$count],"box_price" => $this->input->post('box_price')[$count],"hsn_code" => $this->input->post('hsn_code')[$count],"location" => $this->input->post('location')[$count],"type" => $this->input->post('type')[$count],"category_fk"=>'3', "created_date" => date("Y-m-d H:i:s")));
        ...
        $data['branchlist'] = $this->Test_item_model->get_data("select id,branch_name from branch_master where status='1' order by branch_name asc");
		 
		 }else{
			 
			$data['branchlist'] = $this->Test_item_model->get_data("select id,branch_name from branch_master where status='1' and id in(SELECT `branch_fk`  FROM `user_branch`  WHERE STATUS = '1' AND user_fk = " . $login_id . ") order by branch_name asc "); 
		 }
    ...
    public function machine_edit() {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            if (!empty($insert)) {
                ...
                foreach ($reg as $key => $value) {
                    if ($old_item[$count] > 0) {
        $this->Test_item_model->master_fun_update1("inventory_item", array("id" => $old_item[$count]), array('status' => "1","reagent_name" => $value,"unit_fk" => $this->input->post('unit_fk')[$count], "test_quantity" => $this->input->post('test_quantity')[$count],"per_pack" => $this->input->post('per_pack')[$count],"brand_fk" => $this->input->post('brand_fk')[$count],"remark" => $this->input->post('remark')[$count],"box_price" => $this->input->post('box_price')[$count],"hsn_code" => $this->input->post('hsn_code')[$count],"location" => $this->input->post('location')[$count],'type' =>$this->input->post('type')[$count]));
                    } else {
                        $this->Test_item_model->master_fun_insert("inventory_item", array('machine ' => $ids, "reagent_name" => $value,
                            "unit_fk" => $this->input->post('unit_fk')[$count], "test_quantity" => $this->input->post('test_quantity')[$count], "created_date" => date("Y-m-d H:i:s"),"per_pack" => $this->input->post('per_pack')[$count],"brand_fk" => $this->input->post('brand_fk')[$count],"remark" => $this->input->post('remark')[$count],"box_price" => $this->input->post('box_price')[$count],"hsn_code" => $this->input->post('hsn_code')[$count],"location" => $this->input->post('location')[$count],'type' =>$this->input->post('type')[$count]));
                    }
                    $count++;
                }
            }
        ...
        $data['branchlist'] = $this->Test_item_model->get_data("select id,branch_name from branch_master where status='1' order by branch_name asc");
            ...
            //Vishal COde Start
            $data['item'] = $this->Test_item_model->get_data("SELECT inventory_item.id AS Inventory,inventory_item.reagent_name AS InventoryName,inventory_item.quantity,inventory_item.unit_fk,inventory_item.test_quantity,inventory_item.per_pack,inventory_item.brand_fk,inventory_item.remark,inventory_item.hsn_code,inventory_item.box_price,inventory_item.location,inventory_item.type FROM `inventory_item`  WHERE `inventory_item`.`status`='1'  AND `inventory_item`.`machine`='" . $data["eid"] . "' ");
