    ...
    public function stockdetils() {
        ...
        if ($branch != '' && $itemget != '') {
            ...
            if ($type == 1 || $type == 2 || $type == 8) {
                ...
                $data["query"] = $this->Intent_model->get_val("SELECT s.`id`,s.item_fk,s.batch_no,s.`expire_date`,s.`created_date`,s.`quantity`,s.`used` AS stcok,b.`branch_name`,s.`id`,r.`reagent_name`,r.`category_fk`,inw.`branch_fk` FROM inventory_stock_master s INNER JOIN `inventory_item` r ON r.`id`=s.`item_fk` INNER JOIN inventory_inward_master inw ON inw.`id`=s.`inward_fk` INNER JOIN `branch_master` b ON b.`id`=inw.`branch_fk` WHERE s.`status`='1' and inw.branch_fk='$branch' $search  GROUP BY s.id ORDER BY s.id ASC");
            } else {
    ...
    public function stockdetilsexport() {
        ...
        if ($branch != '' && $itemget != '') {
            ...
            if ($type == 1 || $type == 2 || $type == 8) {
                ...
                $data["query"] = $this->Intent_model->get_val("SELECT s.`id`,s.item_fk,s.batch_no,s.`expire_date`,s.`created_date`,s.`quantity`,s.`used` AS stcok,b.`branch_name`,s.`id`,r.`reagent_name`,r.`category_fk`,SUM(s.`used`) AS stcok,inw.`branch_fk` FROM inventory_stock_master s  INNER JOIN `inventory_item` r ON r.`id`=s.`item_fk` INNER JOIN inventory_inward_master inw ON inw.`id`=s.`inward_fk` INNER JOIN `branch_master` b ON b.`id`=inw.`branch_fk` WHERE s.`status`='1' And inw.branch_fk='$branch' And inw.`branch_fk` in(SELECT `branch_fk`  FROM `user_branch`  WHERE status= '1' $search AND user_fk = " . $login_id . ") GROUP BY r.`id` ORDER BY s.id ASC");
            }
        } else {
        ...
        header("Expires: 0");
        $handle = fopen('php://output', 'w');
        fputcsv($handle, array("No", "Branch Name", "Item", "Batch number", "Expiry date","Added Date", "Quantity", "Used", "Available"));
        $i = 0;

        foreach ($query as $key) {
            $i++;
            fputcsv($handle, array($i, $key["branch_name"], $key["reagent_name"], $key["batch_no"], date("d-m-Y", strtotime($key["expire_date"])),date("d-m-Y", strtotime($key["created_date"])), $key["quantity"], $key["stcok"], ($key["quantity"] - $key["stcok"])));
        }
        fclose($handle);
        exit;
    }
