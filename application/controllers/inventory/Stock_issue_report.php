...
class Stock_issue_report extends CI_Controller {
    ...
    function index() {
        ...
        if ($start_date != "" && $end_date != "" && $branch != "" && $item != "") {
            ...
            $data["query"] = $this->Stock_issue_report_model->get_val("SELECT 
  b.`branch_name`,
  s.`id`,
  s.item_fk,
  r.`reagent_name`,
  ...
  SUM(s.`used`) AS stcok,
  inw.`branch_fk` ,
  inventory_brand.`brand_name` AS BrandName
FROM
  ...
  INNER JOIN `branch_master` b 
    ...
    LEFT JOIN `inventory_brand` ON `inventory_brand`.`id`=r.`brand_fk` WHERE s.`status`='1' and inw.branch_fk='$branch' AND s.item_fk='$item'  GROUP BY s.item_fk ORDER BY s.id ASC");
...
//echo "<pre>";print_r($old_opning_stock);die();
            ...
            foreach ($dates as $key) {
                ...
                if ($key . " 00:00:00" >= $start_date) {
                $NewStock = $NewStock - $issue;
                if(empty($data["query"][0]["BrandName"])){
                    $data["query"][0]["BrandName"]="NA";
                }
                $new_array[] = array("wastage" => $wastage, "srno" => $cnt + 1, "date" => $key, "compony" => $data["query"][0]["BrandName"], "pack_size" => $data['query'][0]["test_quantity"], "lot" => $dkey["batch_no"], "expire_date" => $dkey["expire_date"], "opening" => $open_stock, "received" => $received, "issue" => $issue, "new_bottol_test" => $new_bottol_test, "closing" => $closing_stock - $wastage, "westage" => $udm);
