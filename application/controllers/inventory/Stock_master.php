  ...
  AND inventory_test_machine.machine_fk = '" . $kkey["machine_fk"] . "'");
    ...
    function stock_add() {
        ...
        if ($this->form_validation->run() != FALSE) {
			...
			if($this->input->post('expire_date') != ""){ $expridedate=date("Y-m-d", strtotime($this->input->post('expire_date'))); }else{
    ...
    function getreagent() {
        ...
        $query = $this->Test_item_model->get_val("SELECT `inventory_item`.*,`inventory_category`.`name` AS category FROM `inventory_item` INNER JOIN `inventory_category` ON `inventory_item`.`category_fk`=`inventory_category`.`id` WHERE `inventory_category`.`status`='1' AND `inventory_item`.`status`='1' and branch_fk='$id' AND `inventory_item`.`category_fk` IN (1,2) ORDER BY `inventory_item`.`reagent_name` ASC");
        $lab_stats = '';
...
//    Vishal Start Code
    function edit_stock(){
        ...
        if($this->form_validation->run() != false){
            ...
            /*$this->Test_item_model->master_fun_new_update('inventory_inward_master',array('id',$eid) ,$data1);*/
           ...
           $data_sub = array(
            ...
            $this->Test_item_model->master_fun_new_update('inventory_stock_master',array('inward_fk',$eid), $data_sub);

           $this->session->set_flashdata("success", array("Stock Successfully Updated"));
            redirect("inventory/stock_master", "refresh");
        }
        $data['query'] = $this->Test_item_model->get_val("SELECT ism.item_fk,ism.quantity,ism.batch_no,ism.expire_date,iwm.branch_fk,it.reagent_name,ium.name AS UnitName,br.branch_name FROM inventory_stock_master AS ism LEFT JOIN inventory_inward_master AS iwm ON iwm.id = ism.inward_fk AND iwm.status = '1' LEFT JOIN inventory_item AS it ON it.id = ism.item_fk AND it.status = '1' LEFT JOIN inventory_unit_master AS ium ON ium.id = it.unit_fk AND ium.status = '1' LEFT JOIN branch_master  AS br ON br.id = it.branch_fk AND br.status = '1' WHERE ism.inward_fk = '".$eid."' and ism.status='1'");
        $branch = $data['query'][0]['branch_fk'];
        $data['reagent_list'] =  $this->Test_item_model->get_val("Select it.id,it.reagent_name,ium.id as unit_id,ium.name as UnitName from inventory_item as it LEFT JOIN inventory_unit_master as ium on ium.id = it.unit_fk and ium.status='1' where it.status='1' and it.branch_fk='".$branch."'");
        ...
        $this->load->view('inventory/stockedit', $data);
        $this->load->view('footer');
    }
