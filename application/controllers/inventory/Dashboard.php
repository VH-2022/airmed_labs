...
function total_reagent() {
        ...
        $cnt = 1;
        $table = '';
        $table .= "<thead><th>No</th><th>Reagent Name</th><th>Category Name</th><th>Unit Name</th>Pack Size<th>Pack Size</th><th>Price</th></thead>";
        $table .= "<tbody>";
            ...
            foreach ($data['new_array'] as $test_val) {
                if ($test_val['quantity'] > $test_val['used']) {
                    ...
                    $table .= "<td>" . $test_val['Unit'] . "</td>";
					 $table .= "<td>" . $test_val['per_pack'] . "</td>";
					$table .="<td>" . $test_val['box_price'] ."</td>";
                    $table .= "</tr>";
                    $cnt++;
                }
            }
        } else {
            $table .= "<tr><td colspan='6'>Record Not Found</td>";
        }
        $table .= "</tbody>";
        echo $table;
    }
