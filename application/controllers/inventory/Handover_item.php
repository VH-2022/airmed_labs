...
//Vishal COde End
                $data['query'] = $this->handover_item_model->get_val("SELECT  br.branch_name,it.reagent_name, iu.*,am.name,
                        ism.quantity as Quantity,iu.quantity asIUQuantity,
                        ism.batch_no as Batch
                        FROM inventory_usedreagent AS iu 
                        LEFT JOIN branch_master AS br ON br.id = iu.branchfk 
                        AND br.status = '1' 
                        inner JOIN inventory_item AS it ON it.id = iu.reaqgentfk AND it.status = '1'
                        LEFT JOIN admin_master AS am ON am.id = iu.`handover_to`
                        AND am.`status`='1' 
                        LEFT JOIN inventory_stock_master as ism on ism.id = iu.batchnuno and ism.status='1' 
                        WHERE iu.status = '1' 
                        AND it.category_fk in (1,2) $temp order by iu.id DESC LIMIT $page," . $config['per_page'] . " ");
            } else {
                //Vishal COde Start
                $config["base_url"] = base_url() . "inventory/Handover_item/index?user_fk=$user_fk&branch=$branch";
                ...
                $data['query'] = $this->handover_item_model->get_val("SELECT br.branch_name,it.reagent_name, iu.*,am.name,
                        ism.quantity as Quantity,iu.quantity asIUQuantity,
                        ism.batch_no as Batch FROM inventory_usedreagent AS iu 
                        LEFT JOIN branch_master AS br ON br.id = iu.branchfk 
                        AND br.status = '1' 
                        inner JOIN inventory_item AS it ON it.id = iu.reaqgentfk 
                        AND it.status = '1'
                        LEFT JOIN admin_master AS am ON am.id = iu.`handover_to`
                        AND am.`status`='1'
                        LEFT JOIN inventory_stock_master as ism on ism.id = iu.batchnuno 
                        and ism.status='1' WHERE iu.status = '1' 
                        AND iu.branchfk in(SELECT `branch_fk`  FROM `user_branch`  WHERE STATUS = '1' AND user_fk = " . $login_id . ") 
                        AND it.category_fk in (1,2) $temp order by iu.id desc LIMIT $page," . $config['per_page'] . "");
            }
            //Filteration Close
        } else {
...
//Vishal COde End
        ...
        foreach ($data['query'] as $key) {
            $handover_to = $key['handover_branchfk'];
            $data['query'][$i]['handover_branch_name'] = $this->handover_item_model->get_val("SELECT branch_name from branch_master where id = '$handover_to'");
            $i++;
        }
    ...
    function add() {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            $post['handover_branchfk'] = $this->input->post('handover_branch');
            ...
            if ($sub > $post['quantity'] || $sub == $post['quantity']) {
                ...
                $data11 = array(
                    'branch_fk' => $post['handover_branchfk'],
                    "created_date" => date("Y-m-d H:i:s"),
                    "created_by" => $data["login_data"]["id"]
                );
                $id = $this->handover_item_model->master_fun_insert('inventory_inward_master', $data11);

                $data_sub = array(
                    'inward_fk' => $id,
                    'item_fk' => $post['reaqgentfk'],
                    'batch_no' => $query[0]['batch_no'],
                    'expire_date' => $query[0]['expire_date'],
                    'created_date' => date("Y-m-d H:i:s"),
                    'quantity' => $post['quantity']
                );


                $this->handover_item_model->master_fun_insert('inventory_stock_master', $data_sub);

                $item_name_1 = $post['reaqgentfk'];
                $item_name_data = $this->handover_item_model->get_val("select reagent_name from inventory_item where id = $item_name_1");

                $data12 = array(
                    'reagent_name' => $item_name_data[0]['reagent_name'],
                    "created_date" => date("Y-m-d H:i:s"),
                    "created_by" => $data["login_data"]["id"],
                    "branch_fk" => $post['handover_branchfk'],
                    "status" => '1'
                );
                $id2 = $this->handover_item_model->master_fun_insert('inventory_item', $data12);
            ...
            if ($type == '1' || $type == '2' || $type == '8') {
                $data['branch_list_1'] = $this->handover_item_model->get_data('select id,branch_name from branch_master where status="1"');
            } else {
                $data['branch_list_1'] = $this->handover_item_model->get_data('select br.id,br.branch_name from admin_master LEFT JOIN user_branch on user_branch.user_fk= admin_master.id and admin_master.status="1" LEFT JOIN branch_master as br on br.id = user_branch.branch_fk and user_branch.status="1"  where user_branch.user_fk="' . $login_id . '" and br.status="1" order by br.branch_name asc');
            }
    ...
    function getBatch() {
        ...
        $query = $this->handover_item_model->get_data("SELECT id,batch_no,quantity,expire_date,used,SUM(quantity - used) AS Subtrach 
                FROM inventory_stock_master 
                WHERE item_fk='" . $id . "' 
                AND status='1' AND batch_no !='' AND batch_no IS NOT NULL 
                AND inward_fk !='0' and quantity !=used group by batch_no");
    ...
    public function export() {
        ...
        $user_fk = $this->input->get('user_fk');
        $branch_fk = $this->input->get('branch_fk');

        $result = $this->handover_item_model->csv_report($user_fk, $branch_fk);
        /* echo "<pre>";
          print_r($result);
          exit;
          /* echo "<pre>";
        ...
        header("Content-Disposition: attachment; filename=\"handover_item_list.csv\"");
        header("Pragma: no-cache");
        header("Expires: 0");
        $handle = fopen('php://output', 'w');
        fputcsv($handle, array("Sr.No", "Handover Name", "Branch Name", "Item Name", "Batch No", "Handover Quantity", "Creation Date"));
        $cnt = 1;
        foreach ($result as $val) {

            fputcsv($handle, array($cnt, $val["name"], $val["branch_name"], $val["reagent_name"], $val["Batch"], $val["quantity"], date("d-m-Y", strtotime($val["creteddate"]))));
            $cnt++;
        }
        fclose($handle);
        exit;
