...
class Admin extends CI_Controller
{
    ...
    public function admin_list()
    {
        ...
        $data['query']      = $this->user_model->master_fun_get_tbl_val("admin_master", ['status' => 1, 'type!=' => 0], ["id", "asc"]);
    ...
    public function admin_add()
    {
        ...
        if ($this->form_validation->run() != false) {
            ...
            $data['query'] = $this->user_model->master_fun_insert("admin_master", ["name" => $name, "status" => "1", "email" => $email, "type" => $type, "password" => $password]);

            $this->session->set_flashdata("success", ["Exterior successfull added."]);
            redirect("admin/admin_list", "refresh");
        } else {
            $data['type'] = $this->user_model->master_fun_get_tbl_val("user_type_master", ["status" => 1], ["id", "desc"]);
    ...
    public function admin_delete()
    {
        ...
        $data['query'] = $this->user_model->master_fun_update("admin_master", ["id", $cid], ["status" => "0"]);

        $this->session->set_flashdata("success", ["Exterior successfull deleted."]);
    ...
    public function admin_edit()
    {
        ...
        if ($this->form_validation->run() != false) {
            ...
            $data['query'] = $this->user_model->master_fun_update("admin_master", ["id", $data["cid"]], ["name" => $name, "status" => "1", "email" => $email, "type" => $type, "password" => $password]);

            $this->session->set_flashdata("success", ["House successfull updated."]);

            redirect("admin/admin_list", "refresh");
        } else {
            $data['query'] = $this->user_model->master_fun_get_tbl_val("admin_master", ["id" => $data["cid"]], ["id", "desc"]);
...
>>>>>>> 41be0cd32ac1d511419f632988e26b03a5b224b0
        ...
        if ($type == "1" || $type == "2" || $type == "8" || $type == "5") {
            $data["query"] = $this->user_model->get_val("SELECT i.id, ium.name as unit_name, b.`branch_name`,m.`name` AS machinname, r.reagent_name,s.`batch_no`,i.`quantity`, a.`name` adminname,s.`used`,i.`creteddate`, IF(r.category_fk = 3,r.test_quantity,r.quantity) as packet_quantity FROM `inventory_usedreagent` i LEFT JOIN `branch_master` b ON b.`id`=i.`branchfk` LEFT JOIN inventory_machine m ON m.`id`=i.`machinefk` LEFT JOIN `inventory_item` r ON r.`id`=i.`reaqgentfk` LEFT JOIN `inventory_unit_master` ium ON ium.`id`=r.`unit_fk` LEFT JOIN `inventory_stock_master` s ON s.`id`=i.`indedfk` LEFT JOIN `admin_master` a ON a.`id`=i.`credtedby` WHERE i.status='1' and r.category_fk='3' order by i.id desc");

            $data["new_data"] = [];
            foreach ($data["query"] as $kkeey) {
                $query = $this->user_model->get_val("SELECT s.id,s.`creteddate`, s.`useditem`,s.`timeperfomace`,j.order_id,c.`full_name`,t.`test_name` FROM `inventory_jobstock` s LEFT JOIN `job_master` j ON j.`id`=s.`jobid` LEFT JOIN `customer_master` c ON c.id=j.cust_fk LEFT JOIN test_master t ON t.id=s.`testid` WHERE s.`status`='1' and s.usedreagent_fk='" . $kkeey["id"] . "' ORDER BY s.`id` ASC");

                $kkeey["old_record"] = $query;
                $data["new_data"][]  = $kkeey;
            }
        } else {
            $data["query"] = $this->user_model->get_val("SELECT i.id, ium.name as unit_name, b.`branch_name`,m.`name` AS machinname, r.reagent_name,s.`batch_no`,i.`quantity`, a.`name` adminname,s.used,i.`creteddate`, IF(r.category_fk = 3,r.test_quantity,r.quantity) as packet_quantity FROM `inventory_usedreagent` i LEFT JOIN `branch_master` b ON b.`id`=i.`branchfk` LEFT JOIN inventory_machine m ON m.`id`=i.`machinefk` LEFT JOIN `inventory_item` r ON r.`id`=i.`reaqgentfk` LEFT JOIN `inventory_unit_master` ium ON ium.`id`=r.`unit_fk` LEFT JOIN `inventory_stock_master` s ON s.`id`=i.`indedfk` LEFT JOIN `admin_master` a ON a.`id`=i.`credtedby` WHERE i.status='1' and r.category_fk='3' AND i.branchfk in(SELECT `branch_fk` FROM `user_branch` WHERE status= '1' AND user_fk = " . $login_id . ") order by i.id desc");
        ...
        $data["prescription_upload"] = $this->user_model->master_fun_get_tbl_val("prescription_upload", ['status!=' => 0], ["id", "asc"]);
        if ($data["login_data"]['type'] == 1 || $data["login_data"]['type'] == 2) {
            ...
            $user_branch = $this->user_model->master_fun_get_tbl_val("user_branch", ['status' => 1, "user_fk" => $data["login_data"]['id']], ["id", "asc"]);
        ...
        $lastMonthDate     = date("Y-m-d", strtotime("-7 days", strtotime($todayDate)));
        $outsource_result1 = $this->job_model->get_val("SELECT 1 AS a,GROUP_CONCAT(DISTINCT u.job_fk) AS ids FROM `user_test_outsource` as u LEFT JOIN job_master ON job_master.id=u.job_fk WHERE u.`status`='1' AND u.`created_date` >= '" . $lastMonthDate . " 00:00:00' AND u.`created_date` <= '" . $todayDate . " 23:59:59'and job_master.status != '0' AND job_master.status !='2'  GROUP BY a  order by u.job_fk desc");

        $outsourceIds  = $outsource_result1[0]["ids"];
        $outsourceData = "";
        if (! empty($outsourceIds)) {
            $outsourceData = $this->job_model->get_val("SELECT j.id, j.pro_id, j.cust_fk as cid, j.order_id, c.full_name, c.mobile, c.dob, c.gender, j.cust_fk, j.booking_info, j.doctor, j.test_city, j.branch_fk, j.payable_amount, j.price, j.status, j.views, j.date, j.collection_charge, j.collectioncharge_amount, j.discount, j.barcode, j.send_sms, j.send_email, j.sample_collection, j.prientreport, admin_master.name AS added_by, phlebo_master.name as phlebo_added_by, tc.name as test_city_name , branch_master.branch_name as branch_name,  doctor_master.`full_name` AS doctor_name, doctor_master.`mobile` AS doctor_mobile,pro_master.`pro_name` AS pro_name
                FROM job_master j inner join test_cities tc on j.test_city=tc.id LEFT JOIN admin_master ON admin_master.id=j.added_by LEFT JOIN branch_master ON branch_master.id=j.branch_fk LEFT JOIN doctor_master ON doctor_master.id=j.doctor LEFT JOIN pro_master ON pro_master.id=j.pro_id INNER JOIN customer_master c ON c.id = j.`cust_fk` LEFT JOIN `booking_info` ON booking_info.`id`=j.`booking_info` LEFT JOIN customer_family_master ON customer_family_master.`id`= `booking_info`.`family_member_fk` left join phlebo_master on j.phlebo_added=phlebo_master.id WHERE 1=1  AND j.id IN ( $outsourceIds ) and j.status != '0' AND j.status !='2' AND j.test_city='1' AND j.model_type='1' GROUP BY j.id ORDER BY j.id DESC");
        }

        $data['outsourceData'] = $outsourceData;
        $cnt                   = 0;
        foreach ($data['outsourceData'] as $key) {
            $w_prc           = 0;
            $booked_tests    = $this->job_model->master_fun_get_tbl_val("wallet_master", ['job_fk' => $key["id"]], ["id", "asc"]);
            $emergency_tests = $this->job_model->master_fun_get_tbl_val("booking_info", ['id' => $key["booking_info"]], ["id", "asc"]);
            $f_data          = $this->job_model->master_fun_get_tbl_val("customer_family_master", ['id' => $emergency_tests[0]["family_member_fk"]], ["id", "asc"]);
            $f_data1         = $this->job_model->master_fun_get_tbl_val("relation_master", ['id' => $f_data[0]["relation_fk"]], ["id", "asc"]);
            $doctor_data     = $this->job_model->master_fun_get_tbl_val_with_select("full_name,mobile", "doctor_master", ['id' => $key["doctor"]], ["id", "asc"]);

            $pro_data = $this->job_model->master_fun_get_tbl_val_with_select("pro_name", "pro_master", ['id' => $key["pro_id"]], ["id", "asc"]);
            ...
            if ($data["login_data"]["type"] != 1 && $data["login_data"]["type"] != 2) {
                ...
                $data['outsourceData'][$cnt]["is_print"] = $permission[0]["is_print"];
            } else {
                $data['outsourceData'][$cnt]["is_print"] = 1;
            }
            $relation = "Self";
            if (! empty($f_data1)) {
                $relation                              = ucfirst($f_data[0]["name"] . " (" . $f_data1[0]["name"] . ")");
                $data['outsourceData'][$cnt]["rphone"] = $f_data[0]["phone"];
            }
            $data['outsourceData'][$cnt]["relation"] = $relation;
            ...
            if (empty($f_data)) {
                ...
                if ($key['gender'] == 'male') {
                    $data['outsourceData'][$cnt]["gender"] = 'M';
                } else if ($key['gender'] == 'female') {
                    $data['outsourceData'][$cnt]["gender"] = 'F';
                }
                if ($age[0] != 0) {
                    $data['outsourceData'][$cnt]["age"]      = $age[0];
                    $data['outsourceData'][$cnt]["age_type"] = 'Y';
                }
                if ($age[0] == 0 && $age[1] != 0) {
                    $data['outsourceData'][$cnt]["age"]      = $age[1];
                    $data['outsourceData'][$cnt]["age_type"] = 'M';
                }
                if ($age[0] == 0 && $age[1] == 0 && $age[2] != 0) {
                    $data['outsourceData'][$cnt]["age"]      = $age[2];
                    $data['outsourceData'][$cnt]["age_type"] = 'D';
                }
                if ($age[0] == 0 && $age[1] == 0 && $age[2] == 0) {
                    $data['outsourceData'][$cnt]["age"]      = '0';
                    $data['outsourceData'][$cnt]["age_type"] = 'D';
                }
            } else {
            ...
            $upload_data                                     = $this->job_model->master_fun_get_tbl_val("report_master", ['job_fk' => $key["id"]], ["id", "asc"]);
            $prnt_cnt                                        = $this->job_model->get_val("SELECT id FROM `booked_job_test` WHERE job_fk='" . $key["id"] . "' AND status='1' AND `panel_fk` IS NOT NULL");
            $data['outsourceData'][$cnt]["panel_test_count"] = count($prnt_cnt);
            ...
            foreach ($job_test_list as $st_key) {
                ...
                $outsource          = $this->job_model->get_val("SELECT test_fk,outsource_fk,created_date FROM `user_test_outsource` WHERE status='1' AND job_fk='" . $key["id"] . "' AND  test_fk ='" . $st_key['test_fk'] . "'");
                if (isset($outsource[0])) {
                    $job_tst_lst[] = $st_key;
                }
            }

            /* END */
            $data['outsourceData'][$cnt]["job_test_list"]   = $job_tst_lst;
            $data['outsourceData'][$cnt]["report"]          = $upload_data[0]["original"];
            $data['outsourceData'][$cnt]["whatsappsent"]    = $upload_data[0]["whatsapp_sent_status"];
            $data['outsourceData'][$cnt]["emergency"]       = $emergency_tests[0]["emergency"];
            $data['outsourceData'][$cnt]["cut_from_wallet"] = $w_prc;
            $data['outsourceData'][$cnt]["doctor_name"]     = $doctor_data[0]["full_name"];
            $data['outsourceData'][$cnt]["doctor_mobile"]   = $doctor_data[0]["mobile"];
            $data['outsourceData'][$cnt]["pro_name"]        = $pro_data[0]["pro_name"];
            $package_ids                                    = $this->job_model->get_job_booking_package($key["id"]);
            $data['outsourceData'][$cnt]["package"]         = $package_ids;
            if (! empty($package_ids)) {
                $data['outsourceData'][$cnt]["packagename"] = "";
            }
            /* Nishit code start */
            $is_show = 1;
            if (! empty($test_pack)) {
            ...
            $data['outsourceData'][$cnt]["is_show"] = $is_show;
            ...
            $data['outsourceData'][$cnt]["late_sample_collection"] = $late_sample_collection;
            $data['outsourceData'][$cnt]["late_processing"]        = $late_processing;
    ...
    public function get_dashboard()
    {
        ...
        switch ($type) {
            case "count":
                $test                  = $this->user_model->master_fun_get_tbl_val("test_master", ['status' => 1], ["id", "asc"]);
                ...
                break;
            case "chart":
                $cust                        = $this->user_model->master_fun_get_tbl_val("customer_master", ['status' => 1], ["id", "asc"]);
                $data['totalcustomer']       = count($cust);
                $pending                     = $this->user_model->master_fun_get_tbl_val("job_master", ['status' => 1], ["id", "asc"]);
                $data['pending']             = count($pending);
                $complete                    = $this->user_model->master_fun_get_tbl_val("job_master", ['status' => 2], ["id", "asc"]);
                $data['complete']            = count($complete);
                $data["all_jobs"]            = $this->user_model->master_fun_get_tbl_val("job_master", ['status!=' => 0], ["id", "asc"]);
                $data["prescription_upload"] = $this->user_model->master_fun_get_tbl_val("prescription_upload", ['status!=' => 0], ["id", "asc"]);
                $data['total_test']          = $this->user_model->total_test();
                $data['test_sunday']         = $this->user_model->last_7days_test('Sunday');
                $data['test_monday']         = $this->user_model->last_7days_test('Monday');
    ...
    public function Telecaller()
    {
        ...
        $data['customer']     = $this->job_model->master_fun_get_tbl_val("customer_master", ['status' => 1], ["full_name", "asc"]);
        $data['test_list']    = $this->job_model->master_fun_get_tbl_val("test_master", ['status' => 1], ["test_name", "asc"]);
        $data['package_list'] = $this->job_model->master_fun_get_tbl_val("package_master", ['status' => 1], ["title", "asc"]);
        $data['city']         = $this->job_model->master_fun_get_tbl_val("city", ['status' => 1], ["id", "asc"]);
        $data['unread']       = $this->job_model->master_fun_get_tbl_val("prescription_upload", ['status' => 1, "is_read" => "0"], ["id", "asc"]);
        $data["login_data"]   = logindata();
    ...
    public function TelecallerPriscription()
    {
        if (! is_loggedin()) {
            redirect('login');
        }
        ...
        $data['customer']   = $this->job_model->master_fun_get_tbl_val("customer_master", ['status' => 1], ["id", "asc"]);
        $data['city']       = $this->job_model->master_fun_get_tbl_val("city", ['status' => 1], ["id", "asc"]);
        $data['unread']     = $this->job_model->master_fun_get_tbl_val("prescription_upload", ['status' => 1, "is_read" => "0"], ["id", "asc"]);
        $data["login_data"] = logindata();
    ...
    public function job_details()
    {
        ...
        if ($this->form_validation->run() != false) {
            ...
            $this->job_model->master_fun_update("customer_master", ['id', $cust_fk], $user_data);
            $this->job_model->master_fun_delete("job_test_list_master", ['job_fk', $data['cid']]);
            $this->job_model->master_fun_delete("book_package_master", ['job_fk', $data['cid']]);
            $user_details = $this->job_model->master_fun_get_tbl_val("customer_master", ['id' => $cust_fk], ["id", "asc"]);

            $job_details = $this->job_model->master_fun_get_tbl_val("job_master", ['id' => $data['cid']], ["id", "asc"]);
            ...
            foreach ($test as $key) {
                $tn = explode("-", $key);
                if ($tn[0] == 't') {
                    $this->job_model->master_fun_insert("job_test_list_master", ['job_fk' => $data['cid'], "test_fk" => $tn[1]]);
                ...
                if ($tn[0] == 'p') {
                    $this->job_model->master_fun_insert("book_package_master", ["cust_fk" => $cust_fk, "package_fk" => $tn[1], 'job_fk' => $data['cid'], "status" => "1", "type" => "2"]);

                    $query = $this->db->get_where('package_master_city_price', ['package_fk' => $tn[0], "city_fk" => $job_details[0]["test_city"]]);
            ...
            $job_data = ["test_city" => $test_city, "note" => $note, "status" => "6", "price" => $total_amount, "payable_amount" => $payable, "added_by" => $data["login_data"]["id"]];

            $this->job_model->master_fun_update("job_master", ['id', $data['cid']], $job_data);

            $this->job_model->master_fun_insert("job_log", ["job_fk" => $data['cid'], "created_by" => "", "updated_by" => $data["login_data"]["id"], "deleted_by" => "", "message_fk" => "2", "date_time" => date("Y-m-d H:i:s")]);

            $this->session->set_flashdata("success", ["Test successfully approved."]);
            ...
            $data['report']      = $this->job_model->master_fun_get_tbl_val("report_master", ['status' => 1, "job_fk" => $data['cid']], ["id", "asc"]);
            $data['city']        = $this->job_model->master_fun_get_tbl_val("city", ['status' => 1], ["id", "asc"]);
            $data['state']       = $this->job_model->master_fun_get_tbl_val("state", ['status' => 1], ["id", "asc"]);
            $data['country']     = $this->job_model->master_fun_get_tbl_val("country", ['status' => 1], ["id", "asc"]);
            $data['test_cities'] = $this->job_model->master_fun_get_tbl_val("test_cities", ['status' => 1], ["id", "asc"]);
            $test                = $this->job_model->master_fun_get_tbl_val("job_test_list_master", ['job_fk' => $data['query'][0]["id"]], ["id", "asc"]);
            $package             = $this->job_model->master_fun_get_tbl_val("book_package_master", ['job_fk' => $data['query'][0]["id"]], ["id", "asc"]);
            ...
            $update = $this->job_model->master_fun_update("job_master", ['id', $data['cid']], ["views" => "1"]);

            $this->job_model->master_fun_insert("job_log", ["job_fk" => $data['cid'], "created_by" => "", "updated_by" => $data["login_data"]["id"], "deleted_by" => "", "message_fk" => "2", "date_time" => date("Y-m-d H:i:s")]);

            foreach ($package as $p_key) {
                $data['package_info'][] = $this->job_model->get_val("SELECT `package_master`.`id`,`package_master`.`title`,`package_master`.`image`,`package_master`.`back_image`,`package_master`.`desc_web`,`package_master`.`status`,`package_master_city_price`.`a_price`,`package_master_city_price`.`d_price` FROM `package_master` INNER JOIN `package_master_city_price` ON `package_master`.`id`=`package_master_city_price`.`package_fk` WHERE `package_master`.`id`='" . $p_key["package_fk"] . "' AND `package_master_city_price`.`city_fk`='" . $data['query'][0]["test_city"] . "'");
            }
            ...
            $data["package"] = $this->job_model->get_val("SELECT `package_master`.*, `package_master_city_price`.`a_price` AS `a_price1`, `package_master_city_price`.`d_price` AS `d_price1` FROM `package_master` INNER JOIN `package_master_city_price` ON `package_master`.`id` = `package_master_city_price`.`package_fk` WHERE `package_master`.`status` = '1' AND `package_master_city_price`.`status` = '1' AND `package_master_city_price`.`city_fk` = '" . $data['query'][0]["test_city"] . "'");
    ...
    public function check_email()
    {
        ...
        $cnt     = $this->job_model->master_fun_get_tbl_val("customer_master", ['status' => 1, "id !=" => $cust_id, "email" => $email], ["id", "asc"]);
    ...
    public function check_phone()
    {
        ...
        $data = ["status" => $cnt[0]["count"], "data" => $cnt];
    ...
    public function prescription_details()
    {
        ...
        if ($this->form_validation->run() != false) {
            ...
            if ($customer == 0) {
                ...
                $c_data       = ["full_name" => $name, "gender" => $gender, "email" => $email, "password" => $password, "mobile" => $phone, "address" => $address, "test_city" => $test_city];
                ...
                $this->email->send();
                $this->job_model->master_fun_update("prescription_upload", ["id", $data['cid']], ["cust_fk" => $uid]);
            }
            ...
            $upd = $this->job_model->master_fun_update("prescription_upload", ["id", $this->uri->segment(3)], ["status" => "2"]);
            ...
            if ($submit_type == '1') {
                ...
                $price             = 0;
                $cid               = $this->uri->segment('3');
                $this->job_model->master_fun_update_multi("suggested_test", ['p_id' => $cid], ["status" => 0]);
                foreach ($test as $key) {
                    $tn = explode("-", $key);
                    if ($tn[0] == 't') {
                        ...
                        $this->job_model->master_fun_insert("suggested_test", ['status' => 1, 'p_id' => $cid, 'test_id' => $tn[1]]);
                    }

                    if ($tn[0] == 'p') {
                        $query = $this->db->get_where('package_master_city_price', ['package_fk' => $tn[1], "city_fk" => $test_city]);

                        $result  = $query->result();
                        $query1  = $this->db->get_where('package_master', ['id' => $tn[1]]);
                ...
                $this->job_model->master_fun_update("prescription_upload", ["id", $this->uri->segment(3)], ["job_fk" => $insert]);
                ...
                foreach ($test as $key) {
                    ...
                    if ($tn[0] == 't') {
                        ...
                        $this->job_model->master_fun_insert("job_test_list_master", ['job_fk' => $insert, "test_fk" => $tn[1], "price" => $tst_price[0]["price"]]);
                    }
                    if ($tn[0] == 'p') {
                        $packageid[] = $tn[1];
                        ...
                        $this->job_model->master_fun_insert("book_package_master", ["cust_fk" => $uid, "package_fk" => $tn[1], 'job_fk' => $insert, "status" => "1", "type" => "2", "price" => $tst_price[0]["d_price"]]);
                    }
                }

                $pid         = implode($packageid, ',');
                $tid         = implode($testid, ',');
                $sms_message = $this->job_model->master_fun_get_tbl_val("sms_master", ['status' => 1, "title" => "book_login"], ["id", "asc"]);

                $user = $this->job_model->master_fun_get_tbl_val("customer_master", ['status' => 1, "id" => $uid], ["id", "asc"]);
                ...
                $destail = $this->job_model->master_fun_get_tbl_val("customer_master", ["status" => 1, "id" => $uid], ["id", "asc"]);

                $config['mailtype'] = 'html';

                $this->email->initialize($config);

                $message = '<div style="padding:0 4%;"><h4><b>Dear </b>' . $destail[0]['full_name'] . '</h4><p style="color:#7e7e7e;font-size:13px;">Your Booking successfully. </p><p style="color:#7e7e7e;font-size:13px;"> You Booked : . ' . implode($test_package_name, ', ') . '  </p><p style="color:#7e7e7e;font-size:13px;"> Your Booked Total Amount is Rs. ' . $this->input->post("total_amount") . '  </p><p style="color:#7e7e7e;font-size:13px;"> Discount is ' . $this->input->post('discount') . '%  </p><p style="color:#7e7e7e;font-size:13px;"> Your Booked Payable Amount is Rs. ' . $payable . '  </p><p style="color:#7e7e7e;font-size:13px;"> Payment Type : Cash on Blood Collection</p><p style="color:#7e7e7e;font-size:13px;">Thank You.</p></div>';
                ...
                $this->email->message($message);
                $this->email->send();
                $this->session->set_flashdata("success", ["Booked Successfully"]);
            ...
            if ($submit_type == '0') {
                ...
                $this->job_model->master_fun_update("prescription_upload", ["id", $this->uri->segment(3)], ["discount" => $discount]);

                $update = $this->job_model->master_fun_update_multi("suggested_test", ['p_id' => $cid], ["status" => 0]);
                ...
                foreach ($test as $key) {
                    ...
                    if ($tn[0] == 't') {
                        ...
                        $chk = $this->job_model->master_fun_get_tbl_val("suggested_test", ['status' => 1, 'p_id' => $cid, 'test_id' => $tn[1]], ["id", "asc"]);

                        $test_check = $this->job_model->master_fun_get_tbl_val("suggested_test", ['status' => 1, 'p_id' => $cid], ["id", "asc"]);

                        $insert = $this->job_model->master_fun_insert("suggested_test", $data);
                        $upd    = $this->job_model->master_fun_update("prescription_upload", ["id", $cid], ["status" => "2"]);
                    }

                    if ($tn[0] == 'p') {
                        $this->job_model->master_fun_insert("book_package_master", ["cust_fk" => $uid, "package_fk" => $tn[1], 'job_fk' => $insert, "status" => "1", "type" => "2"]);
                ...
                $data = $this->job_model->master_fun_get_tbl_val("prescription_upload", ["id" => $cid], ["id", "asc"]);
                ...
                $data1        = $this->job_model->master_fun_get_tbl_val("customer_master", ['status' => 1, "id" => $cust_fk], ["id", "asc"]);
                ...
                if ($device_type == 'android') {
                    $notification_data = ["title" => "Airmed Path Lab", "message" => $message, "type" => "suggested_test", "id" => $cid, "img" => $img, "desc" => $desc, "created_date" => $created_date, "order_id" => $orderid];
                ...
                $sms_message = $this->job_model->master_fun_get_tbl_val("sms_master", ['status' => 1, "title" => "Suggested_Test_Generated"], ["id", "asc"]);
                ...
                $this->email->initialize($config);
                $message1 = '<div style="padding:0 4%;"><h4><b>Dear, ' . $name . '</b></h4><p style="color:#7e7e7e;font-size:13px;">Your Suggested Test has been Generated. </p><p style="color:#7e7e7e;font-size:13px;">Your Suggested Test are ' . implode($test_name_mail, ',') . ' </p><p style="color:#7e7e7e;font-size:13px;">Your Order ID : ' . $orderid . '</p><p style="color:#7e7e7e;font-size:13px;">Thank You.</p></div>';
                ...
                $this->email->send();
                $this->session->set_flashdata("success", ["Test Suggested Succesfully"]);
                redirect("Admin/TelecallerPriscription", "refresh");
            }
        }
        ...
        $data['state'] = $this->job_model->master_fun_get_tbl_val("state", ['status' => 1], ["state_name", "asc"]);

        $data['city'] = $this->job_model->master_fun_get_tbl_val("test_cities", ['status' => 1], ["name", "asc"]);

        $data['user_info'] = [];
        if ($data['query'][0]["cust_fk"] != '') {
            $data['user_info'] = $this->job_model->master_fun_get_tbl_val("customer_master", ['id' => $data['query'][0]["cust_fk"]], ["id", "asc"]);
        }
        ...
        $this->job_model->master_fun_update("prescription_upload", ["id", $this->uri->segment(3)], ["is_read" => "1"]);
    ...
    public function TelecallerCallBooking_old()
    {
        if (! is_loggedin()) {
            redirect('login');
        }
        ...
        if ($this->form_validation->run() != false) {
            ...
            if ($customer != '') {
                ...
                $this->job_model->master_fun_update("customer_master", ["id", $customer], $c_data);
                $price = 0;
                foreach ($test as $key) {
                    $tn = explode("-", $key);
                    if ($tn[0] == 't') {
                    ...
                    if ($tn[0] == 'p') {
                        $query = $this->db->get_where('package_master_city_price', ['package_fk' => $tn[1], "city_fk" => $test_city, "status" => "1"]);
                ...
                if ($test_for == "new") {
                    ...
                    if ($f_name != '' && $family_relation != '') {
                        $family_mem_id = $this->job_model->master_fun_insert("customer_family_master", ["user_fk" => $customer, "gender" => $family_gender, "dob" => $f_dob, "name" => $f_name, "relation_fk" => $family_relation, "email" => $f_email, "phone" => $f_phone, "status" => "1", "created_date" => date("Y-m-d H:i:s")]);
                ...
                $booking_fk = $this->job_model->master_fun_insert("booking_info", ["user_fk" => $customer, "type" => $testforself, "family_member_fk" => $family_mem_id, "address" => $address, "date" => $date1, "time_slot_fk" => $time_slot_id, "emergency" => $emergency, "status" => "1", "createddate" => date("Y-m-d H:i:s")]);
                ...
                foreach ($test as $key) {
                    $tn = explode("-", $key);
                    if ($tn[0] == 't') {
                        $this->job_model->master_fun_insert("job_test_list_master", ['job_fk' => $insert, "test_fk" => $tn[1]]);

                        $tst_price = $this->job_model->get_val("select price from test_master_city_price where test_fk='" . $tn[1] . "' and city_fk='" . $test_city . "' and status='1'");

                        $this->job_model->master_fun_insert("booked_job_test", ["job_fk" => $insert, "test_city" => $test_city, "test_fk" => "t-" . $tn[1], "price" => $tst_price[0]["price"]]);
                    }
                    ...
                    if ($tn[0] == 'p') {
                        ...
                        $this->job_model->master_fun_insert("booked_job_test", ["job_fk" => $insert, "test_city" => $test_city, "test_fk" => "p-" . $tn[1], "price" => $tst_price[0]["d_price"]]);
                ...
                if (empty($result)) {
                    ...
                    if (empty($result1) || $email = "noreply@airmedlabs.com") {
                        ...
                        $message = '<div style="padding:0 4%;"><h4><b>Create Account</b></h4><p style="color:#7e7e7e;font-size:13px;">Your account successfully created. </p><p style="color:#7e7e7e;font-size:13px;"> Username/Email : . ' . $email . '  </p><p style="color:#7e7e7e;font-size:13px;"> Password : ' . $password . '  </p><p style="color:#7e7e7e;font-size:13px;">Thank You.</p></div>';
                ...
                if ($test_for == "new") {
                    ...
                    if ($f_name != '' && $family_relation != '') {
                        ...
                        $family_mem_id = $this->job_model->master_fun_insert("customer_family_master", ["user_fk" => $customer, "dob" => $f_dob, "gender" => $family_gender, "name" => $f_name, "relation_fk" => $family_relation, "email" => $f_email, "phone" => $f_phone, "status" => "1", "created_date" => date("Y-m-d H:i:s")]);
                ...
                $booking_fk = $this->job_model->master_fun_insert("booking_info", ["user_fk" => $customer, "type" => $testforself, "family_member_fk" => $family_mem_id, "address" => $address, "date" => $this->input->get_post("phlebo_date"), "time_slot_fk" => $this->input->get_post("phlebo_time"), "emergency" => 0, "status" => "1", "createddate" => date("Y-m-d H:i:s")]);
            ...
            if ($received_amount > 0) {
                $this->job_model->master_fun_insert("job_master_receiv_amount", ["job_fk" => $insert, "added_by" => $added_by, "payment_type" => $payment_via, "amount" => $received_amount, "createddate" => date("Y-m-d H:i:s")]);

                $this->job_model->master_fun_insert("job_log", ["job_fk" => $insert, "created_by" => "", "updated_by" => $added_by, "deleted_by" => "", "job_status" => '', "message_fk" => "22", "date_time" => date("Y-m-d H:i:s")]);
            }

            $file = $this->pdf_invoice($insert);
            $this->job_model->master_fun_update("job_master", ["id" => $insert], ["invoice" => $file]);
            if ($discount > 0) {
                $this->job_model->master_fun_insert("job_log", ["job_fk" => $insert, "created_by" => "", "updated_by" => $added_by, "deleted_by" => "", "job_status" => '', "message_fk" => "24", "date_time" => date("Y-m-d H:i:s")]);
            }

            $this->session->set_flashdata("success", ["Test successfully Booked."]);
            ...
            $data['phlebo_list'] = $this->job_model->master_fun_get_tbl_val("phlebo_master", ['status' => 1, "type" => 1], ["name", "asc"]);

            $data['source_list'] = $this->job_model->master_fun_get_tbl_val("source_master", ['status' => 1], ["name", "asc"]);

            $data['test_cities'] = $this->job_model->master_fun_get_tbl_val("test_cities", ['status' => 1], ["id", "asc"]);

            $data['unread'] = $this->job_model->master_fun_get_tbl_val("prescription_upload", ['status' => 1, "is_read" => "0"], ["id", "asc"]);

            $data["relation1"] = $this->job_model->master_fun_get_tbl_val("relation_master", ['status' => "1"], ["name", "asc"]);
    ...
    public function TelecallerCallBooking222()
    {
        if (! is_loggedin()) {
            redirect('login');
        }
        ...
        if ($this->form_validation->run() != false) {
            ...
            if ($customer != '') {
                ...
                foreach ($test as $key) {
                    $tn = explode("-", $key);
                    if ($tn[0] == 't') {
                        $this->job_model->master_fun_insert("job_test_list_master", ['job_fk' => $insert, "test_fk" => $tn[1], "is_panel" => "0"]);
                        ...
                        $new_price = round($new_price);
                        $this->job_model->master_fun_insert("booked_job_test", ["job_fk" => $insert, "test_city" => $test_city, "test_fk" => "t-" . $tn[1], "price" => $new_price]);
                    }
                    ...
                    if ($tn[0] == 'pt') {
                        $this->job_model->master_fun_insert("job_test_list_master", ['job_fk' => $insert, "test_fk" => $tn[1], "is_panel" => "1"]);
                        ...
                        $this->job_model->master_fun_insert("booked_job_test", ["job_fk" => $insert, "test_city" => $test_city, "panel_fk" => $panel_fk, "test_fk" => "pt-" . $tn[1], "price" => $tst_price[0]["price"]]);
                    }
                }
            } else {
                $result = $this->job_model->get_val("SELECT * FROM `customer_master` WHERE `status`='1' AND active='1' AND `mobile`='" . $phone . "' ORDER BY id ASC");
                ...
                if (empty($result)) {
                    ...
                    $customer = $result[0]["id"];
                    $c1_data  = ["full_name" => $name, "gender" => $gender, "email" => $email, "address" => $address, "dob" => $dob];
                    $this->job_model->master_fun_update("customer_master", ["id", $customer], $c1_data);
                }
                $price = 0;
                foreach ($test as $key) {
                    $tn = explode("-", $key);
            ...
            $file = $this->pdf_invoice($insert);
            $this->job_model->master_fun_update("job_master", ["id", $insert], ["invoice" => $file, "barcode" => $barcode]);
            if ($phlebo != "" && $insert != "") {
                $this->assign_phlebo_job($insert, $phlebo);
            }
            ...
            $data['success']     = $this->session->flashdata("success");
            $data['panel_list']  = $this->job_model->master_fun_get_tbl_val("test_panel", ['status' => 1], ["name", "asc"]);
            $data['phlebo_list'] = $this->job_model->master_fun_get_tbl_val("phlebo_master", ['status' => 1, "type" => 1], ["name", "asc"]);
            $data['source_list'] = $this->job_model->master_fun_get_tbl_val("source_master", ['status' => 1], ["name", "asc"]);
            $data['test_cities'] = $this->job_model->master_fun_get_tbl_val("test_cities", ['status' => 1], ["id", "asc"]);
            $data['unread']      = $this->job_model->master_fun_get_tbl_val("prescription_upload", ['status' => 1, "is_read" => "0"], ["id", "asc"]);
    ...
    public function Telecallerinvoice($jid)
    {
        if (! is_loggedin()) {
            redirect('login');
        }
    ...
    public function TelecallerCallBooking1()
    {
        if (! is_loggedin()) {
            redirect('login');
        }
    ...
    public function get_refered_by()
    {
        $selected = $this->input->get_post("selected");
        $city = $this->input->get_post("val");
        $tid = $this->input->get_post("tid");
        if ($tid > 0) {
            $ctiy_data = $this->job_model->master_fun_get_tbl_val("test_cities", ["id" => $tid], ["id", "asc"]);
            $city = $ctiy_data[0]["city_fk"];
        }
        $referral_list = $this->job_model->master_fun_get_tbl_val("doctor_master", ["status" => '1', "city" => $city], ["full_name", "asc"]);
        ...
        echo json_encode(["refer" => $refer, "test_list" => $test_list, "customer" => $customer_data]);
    }

    public function get_customer()
    {
        $selected = $this->input->get_post("selected");
        $referral_list = $this->job_model->master_fun_get_tbl_val("customer_master", ['status' => 1], ["full_name", "asc"]);
        ...
        echo json_encode(["customer" => $refer]);
    }

    public function get_test_list($city_fk = 1)
    {
        ...
        $test = $this->job_model->get_val("SELECT test_master.`id`, `test_master`.`TEST_CODE`, `test_master`.`test_name`, `test_master`.`test_name`, `test_master`.`PRINTING_NAME`, `test_master`.`description`, `test_master`.`SECTION_CODE`, `test_master`.`LAB_COST`, `test_master`.`status`, `test_master_city_price`.`price`, t_tst AS sub_test FROM `test_master` INNER JOIN `test_master_city_price` ON `test_master`.`id` = `test_master_city_price`.`test_fk` LEFT JOIN (SELECT GROUP_CONCAT(tmm.`test_name` SEPARATOR '%@%') AS t_tst, tm.`id` FROM `sub_test_master` LEFT JOIN `test_master` tm ON `sub_test_master`.`test_fk` = tm.`id` LEFT JOIN test_master tmm ON `sub_test_master`.`sub_test` = tmm.id WHERE `sub_test_master`.`status` = '1' GROUP BY tm.`id`) AS tst ON tst.id = `test_master`.`id` WHERE `test_master`.`status` = '1' AND `test_master_city_price`.`status` = '1' AND `test_master_city_price`.`city_fk` = '" . $city_fk . "' GROUP BY `test_master`.`id`");

        $package = $this->job_model->get_val("SELECT `package_master`.*, `package_master_city_price`.`a_price` AS `a_price1`, `package_master_city_price`.`d_price` AS `d_price1` FROM `package_master` INNER JOIN `package_master_city_price` ON `package_master`.`id` = `package_master_city_price`.`package_fk` WHERE `package_master`.`status` = '1' AND `package_master_city_price`.`status` = '1' AND `package_master`.`is_active`='1' AND `package_master_city_price`.`city_fk` = '$city_fk'");
        ...
        echo json_encode(["refer" => $refer, "test_list" => $test_list, "customer" => $customer_data, "test_ary" => $test, "discount" => $discount]);
    }

    public function get_doctor_test_list12($city_fk = 1)
    {
        $city_fk = $this->uri->segment(3);
        $branch_fk = $this->uri->segment(4);
        ...
        $test = $this->job_model->get_val("SELECT test_master.`id`, `test_master`.`TEST_CODE`, `test_master`.`test_name`, `test_master`.`test_name`, `test_master`.`PRINTING_NAME`, `test_master`.`description`, `test_master`.`SECTION_CODE`, `test_master`.`LAB_COST`, `test_master`.`status`, `test_master_city_price`.`price`, t_tst AS sub_test, lab_doc_discount.`price` AS d_price FROM `test_master` INNER JOIN `test_master_city_price` ON `test_master`.`id` = `test_master_city_price`.`test_fk` LEFT JOIN (SELECT GROUP_CONCAT(tmm.`test_name` SEPARATOR '%@%') AS t_tst, tm.`id` FROM `sub_test_master` LEFT JOIN `test_master` tm ON `sub_test_master`.`test_fk` = tm.`id` LEFT JOIN test_master tmm ON `sub_test_master`.`sub_test` = tmm.id WHERE `sub_test_master`.`status` = '1' GROUP BY tm.`id`) AS tst ON tst.id = `test_master`.`id` LEFT JOIN `lab_doc_discount` ON `lab_doc_discount`.`test_fk`=`test_master`.`id` WHERE `test_master`.`status` = '1' AND `test_master_city_price`.`status` = '1' AND `test_master_city_price`.`city_fk` = '" . $city_fk . "' GROUP BY `test_master`.`id`");
    ...
    public function get_quote_test_list()
    {
        $selected = $this->input->get_post("selected");
    ...
    public function get_user_info()
    {
        ...
        if ($user_id != null) {
            $customer = $this->job_model->master_fun_get_tbl_val("customer_master", ["id" => $user_id], ["full_name", "asc"]);

            $getjobcity = $this->job_model->getjobcity($customer[0]['id']);

            $data['relation_list'] = $this->job_model->get_val("SELECT `customer_family_master`.*, `relation_master`.`name` AS relation_name FROM `customer_family_master` INNER JOIN `relation_master` ON `customer_family_master`.`relation_fk` = `relation_master`.`id` WHERE `customer_family_master`.`status` = '1' AND `relation_master`.`status` = '1' AND `customer_family_master`.`user_fk` = '" . $user_id . "'");
            ...
            $active_package = $this->job_model->get_val("SELECT `active_package`.*, `customer_family_master`.`name`, `package_master`.`title` FROM `active_package` LEFT JOIN `customer_family_master` ON `active_package`.`family_fk` = `customer_family_master`.`id` LEFT JOIN `package_master` ON `package_master`.`id` = `active_package`.`package_fk` WHERE `active_package`.`status` = '1' AND `due_to` >= '" . date("Y-m-d") . "' AND `active_package`.`user_fk` = '" . $user_id . "' AND `active_package`.`parent`='0'");

            echo json_encode(["user_info" => $nw_ary[0], "active_package" => $active_package]);
        }
    }

    public function get_city_test()
    {
    ...
    public function get_call_user_data()
    {
        $data["login_data"] = logindata();
        $this->load->model("user_call_model");
        $result = [];
        ...
        $calls = $this->user_call_model->master_fun_get_tbl_val('exotel_calls', ['status' => 1, 'CallFrom' => $running_call[0]->CallFrom], ['id', 'asc']);
        if ($running_call[0]->maxid != "") {
            $register = $this->user_call_model->master_fun_get_tbl_val('customer_master', ['status' => '1', 'mobile' => $org_number], ["id", "desc"]);
    ...
    public function get_user_name()
    {
        ...
        $get_val = $this->job_model->master_fun_get_tbl_val("customer_master", ['status' => '1', "active" => '1', "id" => $id], ["id", "asc"]);
    ...
    public function test_email()
    {
        ...
        $message = '<div style="background:#a3a3a3 !important;font-family: "Roboto",sans-serif;"><script src="https://use.fontawesome.com/44049e3dc5.js"></script><div style="background: #6b6b6b none repeat scroll 0 0;float: left;padding: 7%;"><div style="border:1px solid #f1f1f1;border-bottom:1px solid #67B1A3;padding:1.5%;width:100%;float:left;background:#fff;"><div style="float:left;width:64%;"><img alt="" src="' . base_url() . 'user_assets/images/logo.png" style="height: 100px;width:213px;"/></div><div style="float:right;text-align: right;width:33%;padding-top:7px;"></div></div><div style="padding:1.5%;width:100%;float:left;background-color:#fff;border:1px solid #f1f2f3;"><div style="padding:0 4%;"><h4><b>Create Account</b></h4><p style="color:#7e7e7e;font-size:13px;">Your account successfully created. </p><p style="color:#7e7e7e;font-size:13px;"> Username/Email : . test  </p><p style="color:#7e7e7e;font-size:13px;"> Password : test  </p><p style="color:#7e7e7e;font-size:13px;">Thank You.</p></div></div><div style="width:100%;float:left;background:#F5F5F5;padding:1.5%;color:#828282;font-size:12px;border:1px solid #dfdfdf;text-align:center;"><a href="' . base_url() . 'user_master/about_us" style="color:#515151;text-decoration:none;">About Us</a> | <a href="' . base_url() . 'user_master/contact_us" style="color:#515151;text-decoration:none;">Contact Us</a> | <a href="' . base_url() . 'user_login" style="color:#515151;text-decoration:none;">Login</a> | <a href="' . base_url() . 'user_master/collection" style="color:#515151;text-decoration:none;">Home-collection</a> | <a href="https://www.facebook.com/airmedpathlabs" style="color:#515151;text-decoration:none;"><i aria-hidden="true"><img src="' . base_url() . 'user_assets/images/face_book_icon1.jpeg" style="height:15px;width:15px;"></i></a></div><div style="width:100%;float:left;background:#333333;padding:1.5%;color:#ccc;font-size:12px;border:1px solid #323232;text-align:center;">Copyright @ 2016 Airmed PATH LAB. All rights reserved</div></div></div>';

        $this->load->helper("email");
        $email = new Email();
        $email->send(["nishit@virtualheight.com"], "nishit@virtualheight.com", "Account Created Successfully", $message);
        echo "OK";
    }

    public function get_telecaller_remain()
    {
        $data["login_data"] = logindata();
    ...
    public function get_user_orders()
    {
        ...
        $data['query'] = $this->customer_model->master_fun_get_tbl_val("customer_master", ["id" => $user_id], ["id", "desc"]);
    ...
    public function assign_phlebo_job($job_id, $phlebo)
    {
        $this->load->model("service_model");
        $new_job_details = $this->service_model->master_fun_get_tbl_val("job_master", ['id' => $job_id], ["id", "asc"]);
        $booking_info = $this->service_model->master_fun_get_tbl_val("booking_info", ['id' => $new_job_details[0]["booking_info"]], ["id", "asc"]);
        if ($booking_info[0]["emergency"] == "0" || $booking_info[0]["emergency"] == null) {
            ...
            if (! empty($get_random_phlebo)) {
                $data = ["job_fk" => $job_id, "phlebo_fk" => $phlebo, "date" => $booking_info[0]["date"], "time_fk" => $booking_info[0]["time_slot_fk"], "address" => $booking_info[0]["address"], "notify_cust" => 1, "created_date" => date("Y-m-d H:i:s"), "created_by" => $data["login_data"]["id"]];

                $insert = $this->service_model->master_fun_insert("phlebo_assign_job", $data);
                $phlebo_details = $this->service_model->master_fun_get_tbl_val("phlebo_master", ['id' => $phlebo, "type" => 1], ["id", "asc"]);

                $phlebo_job_details = $this->service_model->master_fun_get_tbl_val("phlebo_assign_job", ['job_fk' => $job_id], ["id", "asc"]);

                $p_time = $this->service_model->master_fun_get_tbl_val("phlebo_time_slot", ['id' => $phlebo_job_details[0]["time_fk"]], ["id", "asc"]);

                $job_details = $this->service_model->master_fun_get_tbl_val("job_master", ['id' => $job_id], ["id", "asc"]);

                $customer_details = $this->service_model->master_fun_get_tbl_val("customer_master", ['id' => $job_details[0]['cust_fk']], ["id", "asc"]);

                if ($insert) {
                    $s_time = date('h:i a', strtotime($p_time[0]["start_time"]));
                    $e_time = date('h:i a', strtotime($p_time[0]["end_time"]));
                    $sms_message = $this->service_model->master_fun_get_tbl_val("sms_master", ['status' => 1, "title" => "phlebo_msg"], ["id", "asc"]);
                    ...
                    $notification = new Sms();
                    $notification->send($mobile, $sms_message);
                    $sms_message = $this->service_model->master_fun_get_tbl_val("sms_master", ['status' => 1, "title" => "phlebo_msg_cust"], ["id", "asc"]);
    ...
    {
        $this->load->model("service_model");
        $job_details = $this->job_model->master_fun_get_tbl_val("job_master", ["status !=" => "0", "id" => $job_id], ["id", "asc"]);

        if (! empty($job_details)) {
            $book_test = $this->job_model->master_fun_get_tbl_val("job_test_list_master", ["job_fk" => $job_id], ["id", "desc"]);
            $book_package = $this->job_model->master_fun_get_tbl_val("book_package_master", ["job_fk" => $job_id, "status" => "1"], ["id", "desc"]);
            ...
            foreach ($book_package as $key) {
                $price1 = $this->job_model->get_val("SELECT `package_master`.*, `package_master_city_price`.`a_price` AS `a_price`, `package_master_city_price`.`d_price` AS `d_price` FROM `package_master` INNER JOIN `package_master_city_price` ON `package_master`.`id` = `package_master_city_price`.`package_fk` WHERE `package_master`.`status` = '1' AND `package_master_city_price`.`status` = '1' AND `package_master_city_price`.`city_fk` = '" . $job_details[0]["test_city"] . "' AND `package_master`.`id`='" . $key["package_fk"] . "'");
    ...
    public function get_job_id($city = null)
    {
        $this->load->library("Util");
        $util = new Util();
    ...
    public function pdf_invoice($id)
    {
        ...
        $html = $this->load->view('user/user_job_invoice_pdf', $data, true);
        ob_end_flush();
        ...
        $this->add_result_model->master_fun_update('job_master', ['id', $id], ["invoice" => $name]);
        return $name;
    }

    public function book_active_package()
    {
        ...
        $get_active_package_data = $this->job_model->master_fun_get_tbl_val("active_package", ["id" => $active_package], ["id", "desc"]);
        ...
        $booking_fk = $this->job_model->master_fun_insert("booking_info", ["user_fk" => $get_active_package_data[0]["user_fk"], "type" => $test_for, "family_member_fk" => $get_active_package_data[0]["family_fk"], "address" => $active_address, "date" => $active_phlebo_date, "time_slot_fk" => $phlebo_time, "emergency" => 0, "status" => "1", "createddate" => date("Y-m-d H:i:s")]);
        ...
        $this->job_model->master_fun_insert("book_package_master", ["cust_fk" => $get_active_package_data[0]["user_fk"], "package_fk" => $get_active_package_data[0]["package_fk"], 'job_fk' => $insert, "status" => "1", "type" => "2", "price" => "0"]);

        $this->job_model->master_fun_insert("active_package", [
        ...
        $this->session->set_flashdata("success", ["Active package successfully Booked."]);

        redirect("Admin/TelecallerCallBooking");
    }

    public function check_active_package($pid, $jid, $uid)
    {
        $this->load->library("util");
        $util = new Util;
    ...
    public function collect_amount()
    {
        if (! is_loggedin()) {
    ...
    public function add_doctor()
    {
        ...
        $state = $this->job_model->master_fun_get_tbl_val("city", ["id" => $city], ["id", "desc"]);
    ...
    public function get_phlebo()
    {

        $cityfk = $this->input->post("val");

        $phlebo_list = $this->job_model->master_fun_get_tbl_val("phlebo_master", ['status' => 1, "test_city" => $cityfk], ["id", "asc"]);
        ...
        echo json_encode(["phlebolist" => $refer]);
    }

    public function get_doctor_test_list($city_fk = 1)
    {
...
GROUP BY `test_master`.`id`");
    ...
    public function TelecallerCallBooking()
    {
        ...
        if ($this->form_validation->run() != false) {
            ...
            if (! empty($coupon)) {
                $discount_api = 'https://usmanpuraimaging.com/paramedics/service/discount_service_v1/is_code_valid_patho';

                $curl = curl_init();

                curl_setopt_array($curl, [

                    CURLOPT_URL            => $discount_api,
                    ...
                    CURLOPT_POSTFIELDS     => 'code=' . $coupon,
                ...
                if ($obj->isvalid != 1) {
                    ...
                    $data["error"]['0'] = "Invalid Discount code.";

                    $this->session->set_flashdata("error", ["Invalid Discount code."]);
                ...
                foreach ($test as $key) {
                    ...
                    if ($tn[0] == 'p') {
                        ...
                        if (in_array($tn[1], $dipackar)) {
                            ...
                            $this->job_model->master_fun_insert("patient_packdiscount", ["patient_id" => $customer, "b_pdiscountid" => $packdisid[$tn[1]], "createddate" => date("Y-m-d H:i:s")]);
                        }

                        $query = $this->db->get_where('test_branch_price', ['test_fk' => $tn[1], "branch_fk" => $branch, "type" => '2', "status" => "1"]);
    ...
    ON `test_master`.`id` = `panel_tests`.`test_fk` WHERE `test_master`.`status`='1' AND `panel_tests`.`status`='1' AND `panel_tests`.`city_fk`='" . $test_city . "'  AND `panel_tests`.`panel_fk` = '" . $panel_fk . "'   AND `test_master`.`id`='" . $tn[1] . "'");
                ...
                if (! empty($coupon)) {
                    ...
                    if ($coupon_valid == 1) {
                        ...
                        $disc_code_usage = ["patho_disc_code" => $coupon, "patho_job_fk" => $insert, "patho_usage_time" => date("Y-m-d H:i:s")];

                        $this->job_model->master_fun_insert("discount_codes_used", $disc_code_usage);

                        $disc_code_usage["commission"] = $commi;

                        $discount_api = 'https://usmanpuraimaging.com/paramedics/service/discount_service_v1/save_used_coupon_patho';
                        ...
                        curl_setopt_array($curl, [
                            ...
                            CURLOPT_POSTFIELDS     => http_build_query($disc_code_usage),
  ...
  INNER JOIN `panel_tests`
    ...
    ON `test_master`.`id` = `panel_tests`.`test_fk` WHERE `test_master`.`status`='1' AND `panel_tests`.`status`='1' AND `panel_tests`.`city_fk`='" . $test_city . "'  AND `panel_tests`.`panel_fk` = '" . $panel_fk . "'   AND `test_master`.`id`='" . $tn[1] . "'");
                ...
                $booking_fk = $this->job_model->master_fun_insert("booking_info", ["user_fk" => $customer, "type" => $testforself, "family_member_fk" => $family_mem_id, "address" => $address, "date" => $this->input->get_post("phlebo_date"), "time_slot_fk" => $this->input->get_post("phlebo_time"), "emergency" => $emergency, "status" => "1", "createddate" => date("Y-m-d H:i:s")]);
                ...
                // $docMaster = $this->db->get_where('doctor_master', array('id' => $referral_by));

                // $docMasterResult = $docMaster->result();
                $doctor_pro_id = "";
                if (isset($docMasterResult[0]['doctor_pro_id'])) {
                    $doctor_pro_id = $docMasterResult[0]['doctor_pro_id'];
                }
                ...
                foreach ($test as $key) {
                    ...
                    if ($tn[0] == 'p') {
                        ...
                        if (in_array($tn[1], $dipackar)) {
                            ...
                            $this->job_model->master_fun_insert("patient_packdiscount", ["job_fk" => $insert, "patient_id" => $customer, "b_pdiscountid" => $packdisid[$tn[1]], "createddate" => date("Y-m-d H:i:s")]);
                        }
  ...
  `active_package` LEFT JOIN `package_master` ON `package_master`.`id` = `active_package`.`package_fk` WHERE `active_package`.`status` = '1' AND `due_to` >= '" . date("Y-m-d") . "' AND package_master.id='" . $tn[1] . "'  AND `active_package`.`user_fk` = '" . $customer . "' AND `active_package`.`parent`='0'");
                        ...
                        $this->job_model->master_fun_insert("booked_job_test", ["job_fk" => $insert, "test_city" => $test_city, "test_fk" => "p-" . $tn[1], "price" => $pricepack]);
...
WHERE job_master.id = '" . $insert . "'
  ...
  AND job_master.status != '0' ");
                        ...
                        $this->job_model->master_fun_insert("admin_alert_sms", ["mobile_no" => $job_details[0]["phone_no"], "message" => $c_message, "created_date" => date("Y-m-d H:i:s")]);
                    }
                }
                ...
                if ($branch == 188) {
                  ...
                  WHERE job_master.id = '" . $insert . "'
                    ...
                    $patient_mob = "919998050575";
                    $test_names  = implode(', ', $testnames);
                    if (isset($docMasterResult[0]['full_name'])) {
                        $dr_name = $docMasterResult[0]['full_name'];
                    }
                    //$docMasterResult[0]['full_name']??"";
                    ...
                    "parameters": "[{\'name\':\'id\', \'value\':\'' . $insert . '\'},{\'name\':\'dr_name\', \'value\':\'' . $dr_name . '\' },{\'name\':\'test_name\', \'value\':\'' . $test_names . '\'},{\'name\':\'date_time\', \'value\':\'' . date("d-M-Y g:i A", strtotime(date("Y-m-d H:i:s"))) . '\'},{\'name\':\'test_amount\', \'value\':\'' . $payable_price . '\' },{\'name\':\'patient_name\', \'value\':\'' . $name . '\' }]"
                    }',
                        CURLOPT_HTTPHEADER     => [
    ...
    public function getbranch_test_list()
    {
    ...
    public function getbranch_doctor_test_list($city_fk = 1)
    {
...
GROUP BY `test_master`.`id`");
    ...
    public function branch_doctor_test_list($city_fk = 1)
    {
...
ORDER BY `job_master`.`branch_fk`,job_master.date DESC");
    ...
    public function export_csv_outsource()
    {
        $this->load->library("util");
        $util              = new Util;
        $todayDate         = date('Y-m-d');
        ...
        //
        fputcsv($handle, ["No.", "Reg No.", "Order Id", "Test City", "Branch", "Date", "Patient Name", "Mobile No.", "Doctor", "Doctor_Mobile", "PRO", "Test/Package Name", "Job Status", "Total Price", "Received Amount", "Added By"]);

        /* Nishit code start */
        $cnt       = 1;
        $new_array = $data['outsourceData'];
        ...
        foreach ($new_array as $key) {
            ...
            $key["test_name"] = implode(",", $test_list);
            $status           = $key['status'];
            if ($key['status'] == 1) {
            ...
            if (! empty($key["phlebo_added_by"])) {
                $added_by = ucfirst($key["phlebo_added_by"]) . " (Phlebo)";
            } else if (! empty($key["added_by"])) {
                $added_by = ucfirst($key["added_by"]);
            } else {
                $added_by = "Online Booking";
            }
            fputcsv($handle, [$cnt, $key["id"], $key["order_id"], $key["test_city_name"], $key["branch_name"], $key["date"], $patient_name, $key["mobile"], $key["doctor_name"], $key["doctor_mobile"], $key["pro_name"], $key["test_name"], $status, $key["price"], $key["payable_amount"], $added_by]);
