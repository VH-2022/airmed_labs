    ...
    function test()
    {
        ...
        */
        ...
        // if ($_GET['debug'] == 1) {
            ...
            // echo "<pre>"; print_r($outsourceData); die;
...
// 'status' => 1
        ...
        foreach ($referral_list as $referral) {
            ...
            // $refer .= '>' . ucwords($referral['full_name']) . ' - ' . $referral['mobile'] .' '. $status .'</option>';
    ...
    function TelecallerCallBooking()
    {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            if (!empty($coupon)) {
                ...
                //$discount_api = 'http://localhost:9999/paramedics/service/discount_service_v1/is_code_valid_patho';
                ...
                if ($obj->isvalid != 1) {
                    ...
                    $this->session->set_flashdata("error", array("Invalid Discount code."));
    ...
    ON `test_master`.`id` = `panel_tests`.`test_fk` WHERE `test_master`.`status`='1' AND `panel_tests`.`status`='1' AND `panel_tests`.`city_fk`='" . $test_city . "'  AND `panel_tests`.`panel_fk` = '" . $panel_fk . "'   AND `test_master`.`id`='" . $tn[1] . "'");
                ...
                if (!empty($coupon)) {
                    ...
                    if ($coupon_valid == 1) {
                        ...
                        $disc_code_usage = array("patho_disc_code" => $coupon, "patho_job_fk" => $insert, "patho_usage_time" => date("Y-m-d H:i:s"));
                        ...
                        //$discount_api = 'http://localhost:9999/paramedics/service/discount_service_v1/save_used_coupon_patho';
...
ORDER BY `job_master`.`branch_fk`,job_master.date DESC");
    ...
    function export_csv_outsource()
    {
        ...
        //    
            fputcsv($handle, array("No.", "Reg No.", "Order Id","Test City", "Branch", "Date",  "Patient Name", "Mobile No.", "Doctor", "Doctor_Mobile", "PRO", "Test/Package Name", "Job Status","Total Price", "Received Amount","Added By"));
        ...
        foreach ($new_array as $key) {
                        ...
                        fputcsv($handle, array($cnt, $key["id"], $key["order_id"],$key["test_city_name"], $key["branch_name"], $key["date"], $patient_name, $key["mobile"], $key["doctor_name"] , $key["doctor_mobile"],$key["pro_name"], $key["test_name"], $status, $key["price"], $key["payable_amount"],$added_by));
