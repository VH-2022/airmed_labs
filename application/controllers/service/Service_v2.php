...
class Service_v2 extends CI_Controller {
...
//$this->email->to("jeel@virtualheight.com");
  ...
  WHERE `package_master`.`status` = '1' 
    ...
    function add_job() {
        echo $this->json_data("0", "Please update application and after booking.", "");die();
        $user_fk = $this->input->get_post('user_id');
        ...
        $booking_fk = $this->service_model->master_fun_insert("booking_info", array("user_fk" => $user_fk, "type" => $b_type, "family_member_fk" => $family_mem_id, "address" => $address, "date" => $date1, "time_slot_fk" => $time_slot_id, "emergency" => $emergency_req, "status" => "1", "createddate" => date("Y-m-d H:i:s")));
...
' . $payfrom_wallet . '
    ...
    function pdf_invoice($id) {
        ...
        $this->service_model->master_fun_update('job_master', array('id', $id), array("invoice" => $name));
