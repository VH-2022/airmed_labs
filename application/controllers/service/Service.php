...
' . $payfrom_wallet . '
    ...
    function send() {
        ...
        foreach ($sms_details as $key) {
            /* Nishit Change Start */
            $test1 = htmlspecialchars($key["message"], ENT_QUOTES);
            $parts = str_split($test1, 1000);
            if (count($parts) > 1) {
                $notification->send_long_sms($key["mobile_no"], $key["message"]);
            } else {
        ...
        /* CAMP sms start */
        $result = $this->curl->simple_get(base_url() . 'Camp_sms/get_send_sms');
        /* END */
        echo $this->json_data("1", "", "success");
    }
    ...
    function check_curl() {
        $this->load->library('curl');
        $result = $this->curl->simple_get(base_url() . 'Daily_dsr/send_report');
        //var_dump($result);
    }

    function send_camp_sms(){
        $this->load->model("user_model");
        $this->load->helper("sms");
        $this->load->library('curl');
        $notification = new Sms();

        $sms_details = $this->service_model->master_fun_get_tbl_val("camp_admin_alert_sms", array("is_send" => "0"), array("id", "asc"));
        foreach ($sms_details as $key) {
            /* Nishit Change Start */
                $notification->send($key["mobile_no"], $key["message"]); 
            /* END */
            $this->service_model->master_fun_update1("camp_admin_alert_sms", array("id" => $key["id"]), array('is_send' => 1));
        }
        /* END */
        echo $this->json_data("1", "", "success"); 
    }
