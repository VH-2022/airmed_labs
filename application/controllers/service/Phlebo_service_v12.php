...
class Phlebo_service_v12 extends CI_Controller {
...
FROM
  ...
  LEFT JOIN `phlebo_assign_job` 
    ...
    LEFT JOIN `phlebo_time_slot` 
      ON `phlebo_time_slot`.`id` = `phlebo_assign_job`.`time_fk` 
WHERE job_master.status IN (6,7,1,8) AND `job_master`.`date`<='" . date("Y-m-d") . " 23:59:59' AND `job_master`.`date`>='" . $last_date . " 00:00:00' AND `phlebo_assign_job`.phlebo_fk='" . $pid . "' AND `phlebo_assign_job`.`is_accept`='0'
ORDER BY `job_master`.`id` DESC ");
        if (!empty($job_list)) {
            $data = array();
            $i=0;
...
WHERE `booking_info`.`id` = '" . $key["booking_info"] . "' ");
                ...
                $phlebo_ass = $this->service_model->get_val("SELECT address 
                        from phlebo_assign_job where job_fk = '" . $key['id'] . "' 
...
//                }
                ...
                if (empty($details[0]["address"])) {
                    ...
                    if (!empty($booking_details[0]["address"])) {
                        $details[0]["address"] = $phlebo_ass[0]['address'];
//                        if ($phlebo_ass[0]['address'] != '') {
//                            $details[0]["address"] = $phlebo_ass[0]['address'];
//                        } else {
//                            $details[0]["address"] = $booking_details[0]["address"];
//                        }
                        ...
                        //$booking_details[0]["address"] = $key["address"];
                        if ($phlebo_ass[0]['address'] != '') {
                            $booking_details[0]["address"] = $phlebo_ass[0]['address'];
                        } else {
                            $booking_details[0]["address"] = $key["address"];
                        }
                    }
                    ...
                    $details[0]["address"] = $phlebo_ass[$i]['address'];
                    $booking_details[0]["address"] = $phlebo_ass[$i]['address'];
...
FROM
  ...
  INNER JOIN `job_master` 
    ...
    LEFT JOIN `phlebo_time_slot` 
          ON `phlebo_time_slot`.`id` = `phlebo_assign_job`.`time_fk` 
WHERE `phlebo_assign_job`.`status`='1'  AND `job_master`.`status` not in (2,0) AND `phlebo_assign_job`.phlebo_fk='" . $pid . "' AND `phlebo_assign_job`.`job_fk` NOT IN (" . implode(",", $checkin_job_ids) . ") group by `phlebo_assign_job`.`job_fk`  order by `phlebo_assign_job`.`job_fk` desc");
        } else {
            $job_list = $this->service_model->get_val("SELECT 
  `phlebo_assign_job`.*,
  `job_master`.`booking_info`,
...
FROM
  ...
  INNER JOIN `job_master` 
    ...
    LEFT JOIN `phlebo_time_slot` 
          ON `phlebo_time_slot`.`id` = `phlebo_assign_job`.`time_fk` 
WHERE `phlebo_assign_job`.`status`='1' AND `job_master`.`status` not in (2,0) AND `phlebo_assign_job`.phlebo_fk='" . $pid . "' group by `phlebo_assign_job`.`job_fk` order by `phlebo_assign_job`.`job_fk` desc");
        }
        if (!empty($job_list)) {
            $data = array();
            foreach ($job_list as $key) {
  ...
  LEFT JOIN `test_cities` 
    ...
    function start() {
        ...
        if ($userid != '' AND $start_time != '' AND $start_date != '') {
            ...
            //$insert = $this->service_model->timeadd($data);
                ...
                // add check in check out in hrms 
           ...
           if(isset($phleboUser) && $phleboUser->empcode !=""){
            ...
            if ($ch !== false) {
             ...
             // add check in check out in hrms close
    ...
    function start_220721() {
        ...
        if ($userid != '' AND $start_time != '' AND $start_date != '') {
            ...
            $data = array(
                "user_fk" => $userid,
                "start_time" => $start_time1, 				                "start_date" => $start_date1,
                "longitude" => $longitude,
                "latitude" => $latitude,
                "ip" => $ip,
                "address" => $address
...
WHERE `doctor_master`.`status` = '1' AND `doctor_master`.`city` = '" . $data1[0]["city_fk"] . "'
  AND (
    ...
    function phlebo_visit() {
        ...
        //$data = $this->service_model->get_visit($start_date, $end_date, $phlebo_fk);

        
        
        $data['query1'] = $this->service_model->get_val("select j.id,j.order_id, DATE_FORMAT(j.date, '%d %b %Y %h:%i %p') AS date, 
            j.status,c.full_name,j.payable_amount, 
            GROUP_CONCAT(distinct t.test_name SEPARATOR  ',') testname, 
            GROUP_CONCAT(distinct p.title SEPARATOR  ',') packagename,
            GROUP_CONCAT(distinct p.id SEPARATOR  ',') packagid 
            from job_master j 
            ...
            where 1=1 AND j.status!='0' AND j.phlebo_added='$phlebo_fk'  GROUP BY j.id  ORDER BY j.id DESC");

        $data['query2'] = $this->service_model->get_val("select j.id,j.order_id, DATE_FORMAT(j.date, '%d %b %Y %h:%i %p') AS date, 
            j.status,c.full_name, j.payable_amount,
            GROUP_CONCAT(distinct t.test_name SEPARATOR  ',') testname, 
            GROUP_CONCAT(distinct p.title SEPARATOR  ',') packagename,
            GROUP_CONCAT(distinct p.id SEPARATOR  ',') packagid 
            ...
            LEFT JOIN package_master p  ON p.id = pb.package_fk 
            LEFT JOIN phlebo_assign_job paj ON paj.job_fk = j.id  
            where 1=1 AND j.status!='0' AND paj.phlebo_fk = '$phlebo_fk'  GROUP BY j.id  ORDER BY j.id DESC");

        $data['query'] = array_merge($data['query1'], $data['query2']);

//AND j.payable_amount='0'
        $phlebo_job_list = [];
        foreach ($data['query'] as $key) {
            ...
            if ($key['packagid'] != '') {
                ...
                foreach ($packages as $p) {
                    $package_test_list = $this->job_model->get_val("SELECT 
...
//    }
    ...
    public function test_sample_type() {
        ...
        if ($jid != '') {
            ...
            foreach ($tid as $key) {
                $price1 = $this->service_model->get_val("SELECT test_master.`id`,`test_master`.`TEST_CODE`, 
                        `test_master`.`test_name`,`test_master`.`test_name`,`test_master`.`PRINTING_NAME`, 
                        `test_master`.`description`,`test_master`.`SECTION_CODE`, 
                        `test_master`.`LAB_COST`,`test_master`.`status`,`test_master_city_price`.`price`, 
                        branch_sample_type.sample_type,test_master.reporting As TAT 
                        FROM `test_master` 
                        INNER JOIN `test_master_city_price` ON `test_master`.`id`=`test_master_city_price`.`test_fk` 
                        LEFT JOIN `branch_sample_type` ON `test_master`.`id`=`branch_sample_type`.`test_fk` AND branch_sample_type.status = '1' 
                        LEFT JOIN `branch_master` ON `branch_master`.`id`=`branch_sample_type`.`branch_fk` AND branch_master.status = '1' 
                        WHERE `test_master`.`status`='1' 
                        AND `test_master_city_price`.`city_fk`='" . $job_details[0]["test_city"] . "' 
                        AND `test_master_city_price`.`status`='1' AND `test_master`.`id`='" . $key . "'");
    ...
    function phlebo_job_list() {
        ...
        $data['query1'] = $this->service_model->get_val("select j.id,j.order_id, DATE_FORMAT(j.date, '%d %b %Y %h:%i %p') AS date, 
            ...
            LEFT JOIN package_master p  ON p.id = pb.package_fk 
            LEFT JOIN phlebo_assign_job paj ON paj.job_fk = j.id  
            where 1=1 AND j.status!='0' AND j.phlebo_added='$pid'  GROUP BY j.id  ORDER BY j.id DESC");
        ...
        $data['query2'] = $this->service_model->get_val("select j.id,j.order_id, DATE_FORMAT(j.date, '%d %b %Y %h:%i %p') AS date, 
            ...
            LEFT JOIN package_master p  ON p.id = pb.package_fk 
            LEFT JOIN phlebo_assign_job paj ON paj.job_fk = j.id  
            where 1=1 AND j.status!='0' AND paj.phlebo_fk = '$pid'  GROUP BY j.id  ORDER BY j.id DESC");
