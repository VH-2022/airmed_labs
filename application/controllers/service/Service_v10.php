...
class Service_v10 extends CI_Controller {
    ...
    function check_otp() {
        ...
        if ($id != NULL && $otp != NULL) {
            ...
            if ($row == 1) {
                ...
                $data = $this->service_model->master_fun_get_tbl_val("customer_master", array("id" => $id), array("id", "asc"));
                echo $this->json_data("1", "OTP verified.", $data);
            } else {
    ...
    function doctor_register() {
        ...
        if ($name != null & $email != null && $password != null && $mobile != null && $address != null) {
            ...
            if ($row == 0) {
                $insert = $this->service_model->master_fun_insert("doctor_master", array("full_name" => $name, "email" => $email, "password" => $password, "mobile" => $mobile, "address" => urldecode($address), "status" => 2));
...
//$this->email->to("jeel@virtualheight.com");
  ...
  WHERE `package_master`.`status` = '1' 
    ...
    function add_job() {
        ...
        $booking_fk = $this->service_model->master_fun_insert("booking_info", array("user_fk" => $user_fk, "type" => $b_type, "landmark" => $landmark, "family_member_fk" => $family_mem_id, "address" => urldecode($address), "date" => $date1, "time_slot_fk" => $time_slot_id, "emergency" => $emergency_req, "status" => "1", "createddate" => date("Y-m-d H:i:s")));
        ...
        if ($user_fk != "" && $testfk != "") {
            ...
            if ($update) {
                ...
                if ($total >= $amount) {
                    ...
                    /* collection charge end */
                    $this->service_model->master_fun_update("job_master", $insert, array("price" => $amount, "payable_amount" => $payable_amount, "collection_charge" => $collection_charge, "collectioncharge_amount" => $collectioncharge_amount));
                    $j_id = $insert;
                }
                $query = $this->service_model->master_fun_get_tbl_val("wallet_master", array("status" => 1, "cust_fk" => $user_fk), array("id", "desc"));
                ...
                $message = '<div style="padding:0 4%;">
                    <h4><b>Dear </b>' . $destail[0]['full_name'] . '</h4>
                        ...
                        <p style="color:#7e7e7e;font-size:13px;"> Rs.100 Collection Charge </p>    
...
//$uid = "";
        ...
        /*
          ...
          $insert = $this->service_model->master_fun_insert("wallet_master", $data);
         */
        $update = $this->service_model->master_fun_update("job_master", $jobid, array("status" => 6, "payment_type" => "PayUMoney"));
...
/// ---------for half payment add_job --
    function add_job_half_payment() {
        ...
        $type = $this->input->get_post("type");
        $test_city = $this->input->get_post("test_city");
        $collection_charges = $this->input->get_post("collection_charges");
        if (empty($test_city)) {
            $test_city = 1;
        }
        $other_reference = $this->input->get_post('other_reference');
        ...
        if ($user_fk != "" && $testfk != "") {
            ...
            $booking_fk = $this->service_model->master_fun_insert("booking_info", array("user_fk" => $user_fk, "landmark" => $landmark, "type" => $b_type, "family_member_fk" => $family_mem_id, "address" => urldecode($address), "date" => $date1, "time_slot_fk" => $time_slot_id, "emergency" => $emergency_req, "status" => "1", "createddate" => date("Y-m-d H:i:s")));
            /* Nishit book phlebo end */
            ...
            for ($i = 0; $i < count($test); $i++) {
                ...
                $this->service_model->master_fun_insert("booked_job_test", array("job_fk" => $insert, "test_city" => $test_city, "test_fk" => "t-" . $test[$i], "price" => $data[0]['price']));
            }
            /* collection charge start */
            $collection_charge = 0;
			$collectioncharge_amount = 0;
            ...
            if ($collection_charges == "true") {
				$collection_charge = 1;
				$collectioncharge_amount = 100;
                $j_total_price = $j_total_price + $collectioncharge_amount;
            }
            /* collection charge end */
            $update = $this->service_model->master_fun_update("job_master", $insert, array("price" => $j_total_price, "collection_charge" => $collection_charge, "collectioncharge_amount" => $collectioncharge_amount));
    ...
    function health_feed() {
        ...
        /* Check api version */
        $check_version_array = array("android" => array("version" => "57", "compulsory" => "yes", "message" => "New version available please update it and enjoy new feature."), "ios" => array("version" => "2", "compulsory" => "no", "message" => "New version available please update it and enjoy new feature."));
...
<p style="color:#7e7e7e;font-size:13px;"> Rs. ' . $caseback_amount . ' Credited in your Wallet. </p>
    ...
    function package_payu_success() {
        ...
        //print_r(array("user_fk" => $userfk1, "type" => $b_type, "family_member_fk" => $family_mem_id, "address" => urldecode($address), "date" => $date1, "time_slot_fk" => $time_slot_id, "emergency" => $emergency_req, "status" => "1", "createddate" => date("Y-m-d H:i:s")));die();
        ...
        $insert1 = $this->service_model->master_fun_insert("job_master", array("cust_fk" => $userfk1, "price" => $price, "date" => $date, "order_id" => $order_id, "payment_type" => "PayUMoney", "status" => 6, "portal" => $type, "booking_info" => $booking_fk, "test_city" => $test_city, "address" => urldecode($address)));
        ...
        $this->service_model->master_fun_insert("booked_job_test", array("job_fk" => $insert1, "test_city" => $test_city, "test_fk" => "p-" . $packageid, "price" => $price));
...
//        }
        ...
        if ($uid != NULL && $select_tests != Null && $total != Null) {
            ...
            //echo "<pre>";print_r(array("user_fk" => $uid, "type" => $b_type, "family_member_fk" => $family_mem_id, "address" => urldecode($address), "date" => $date1, "time_slot_fk" => $time_slot_id, "emergency" => $emergency_req, "status" => "1", "createddate" => date("Y-m-d H:i:s"))); die();
            $booking_fk = $this->service_model->master_fun_insert("booking_info", array("user_fk" => $uid, "type" => $b_type, "landmark" => $landmark, "family_member_fk" => $family_mem_id, "address" => urldecode($address), "date" => $date1, "time_slot_fk" => $time_slot_id, "emergency" => $emergency_req, "status" => "1", "createddate" => date("Y-m-d H:i:s")));
            /* Nishit book phlebo end */
            $data = array(
                "order_id" => $order_id,
                "cust_fk" => $uid,
            ...
            if ($insert) {
                ...
                /* collection charge end */
                $update = $this->service_model->master_fun_update1("job_master", array("id" => $insert), array("collection_charge" => $collection_charge, "collectioncharge_amount" => $collectioncharge_amount));
                ...
                $message = '<div style="padding:0 4%;">
                    <h4><b>Dear </b>' . $destail[0]['full_name'] . '</h4>
                        ...
                        <p style="color:#7e7e7e;font-size:13px;">You booked ' . implode($test_name_mail, ',') . '</p>
                            <p style="color:#7e7e7e;font-size:13px;"> Sample Collection time :  ' . $datebb . ' ' . $s_time . '-' . $e_time . '  </p>  
<p style="color:#7e7e7e;font-size:13px;"> Collection Charge is Rs.100 </p>
<p style="color:#7e7e7e;font-size:13px;"> Your Test Amount is Rs. ' . $np . '  </p>
' . $payfrom_wallet . '
    ...
    function cash_on_delivery_package() {
        ...
        if ($uid != NULL && $select_package != Null && $total != Null) {
            ...
            if ($insert) {
                ...
                $collection_charge = 0;
				$collectioncharge_amount = 0;
                if ($data['package_price'][0]["d_price"] > 0) {
                    $collection_charge = 1;
					$collectioncharge_amount = 100;
                }
...
WHERE `active_package`.`status`='1' AND `package_master`.`status`='1' AND job_master.status !='0' AND `active_package`.`user_fk`='" . $uid . "' AND (`active_package`.`parent`='0' OR `active_package`.`parent`=NULL)");
    ...
    function book_active_package() {
        ...
        if ($uid != '' && $select_package != '') {
            ...
            $booking_fk = $this->service_model->master_fun_insert("booking_info", array("user_fk" => $uid, "type" => $b_type, "family_member_fk" => $family_mem_id, "address" => urldecode($address), "date" => $date1, "time_slot_fk" => $time_slot_id, "emergency" => $emergency_req, "status" => "1", "createddate" => date("Y-m-d H:i:s")));
            /* Nishit book phlebo end */
            $data = array(
                "order_id" => $order_id,
                "cust_fk" => $uid,
...
' . $payfrom_wallet . '
    ...
    function job_delete() {
        ...
        if (!empty($check_job)) {
            ...
            $debit_from_wallet = $this->service_model->master_fun_get_tbl_val("wallet_master", array('status' => 1, "job_fk" => $jid, "debit >" => 0), array("id", "asc"));
            $credit_from_wallet = $this->service_model->master_fun_get_tbl_val("wallet_master", array('status' => 1, "job_fk" => $jid, "credit >" => 0), array("id", "desc"));
            if (!empty($debit_from_wallet) != '' || !empty($credit_from_wallet)) {
                $job_data = $this->service_model->master_fun_get_tbl_val("job_master", array("id" => $jid), array("id", "desc"));
                $customer_wallet = $this->service_model->get_val("select * from wallet_master where status='1' and cust_fk='" . $job_data[0]["cust_fk"] . "' order by id desc limit 0,1");
                ...
                $this->service_model->master_fun_update("wallet_master", array("job_fk", $jid), array("status" => 0));
                $this->service_model->master_fun_insert("wallet_master", array("cust_fk" => $job_data[0]["cust_fk"], "credit" => $debit_amount, "debit" => $credit_amount, "total" => $final_amount, "created_time" => date("Y-m-d H:i:s"), "type" => "", "status" => "1"));
            }
            /* END */
            echo $this->json_data("1", "", "");
        } else {
    ...
    function home_page() {
        ...
        /* Check api version */
        $check_version_array = array("android" => array("version" => "60", "compulsory" => "yes", "message" => "New version available please update it and enjoy new feature."), "ios" => array("version" => "1.3.7", "compulsory" => "no", "message" => "New version available please update it and enjoy new feature."));
        $configuration = array("MERCHANT_ID" => $MERCHANT_ID, "MERCHANT" => $MERCHANT_KEY, "SALT" => $SALT, 'URl' => $URL, "WalletIsUsedInPackagePayment" => "0", "service_provider" => "payu_paisa");
        $data3 = $this->service_model->master_fun_get_tbl_val("offer_master", array("status" => 1), array("id", "asc"));
    ...
    function add_address() {
        ...
        $address = $this->get_address();
        $row = $this->service_model->master_num_rows("user_address", array("user_fk" => $uid, "address" => urldecode($address), "status" => 1));
        if ($row == 0) {
    ...
    function deliver_charge_list() {
        $deliver_price = $this->input->get_post("price");
        $data = $this->service_model->get_val("SELECT id,price FROM `delever_charge` WHERE status='1' AND price='" . $deliver_price . "' ORDER BY id ASC");
        if (!empty($data)) {
            echo $this->json_data("1", "", $data);
        } else {
            echo $this->json_data("0", "Data not available.", "");
    ...
    function getPatientStatistics() {
        $jid = $this->input->get_post("jid");
        $job_details = $this->service_model->get_val('SELECT 
  job_master.id,
  `job_master`.`cust_fk`,
  job_master.`order_id`,
...
FROM
  ...
  INNER JOIN `booking_info` 
    ON job_master.`booking_info` = `booking_info`.`id` WHERE `job_master`.`status`!="0" AND `job_master`.`id`="' . $jid . '"');
        $pid = $job_details[0]["cust_fk"];
        $fid = $job_details[0]["family_member_fk"];
        ...
        $this->load->library("util");
        $util = new Util;
        $user_data1 = $this->service_model->master_fun_get_tbl_val("customer_master", array('status' => 1, 'id' => $pid), array("id", "asc"));
        if (empty($user_data1[0]["gender"]) && empty($user_data1[0]["age"])) {
            $user_data1[0]["gender"] = 'male';
            $user_data1[0]["age"] = 24;
            $user_data1[0]["age_type"] = 'Y';
        }

        if ($data['user_booking_info'][0]["family_member_fk"] != 0) {
            $data['user_family_info'] = $this->service_model->master_fun_get_tbl_val("customer_family_master", array('status' => 1, 'id' => $data['user_booking_info'][0]["family_member_fk"]), array("id", "asc"));
            $user_data1[0]["gender"] = $data['user_family_info'][0]["gender"];

            $user_data1[0]["full_name"] = $data['user_family_info'][0]["name"];
            $user_data1[0]["email"] = $data['user_family_info'][0]["email"];
            $user_data1[0]["phone"] = $data['user_family_info'][0]["phone"];
            ...
            $user_data1[0]["age_type"] = $f_age_type;
            $user_data1[0]["age"] = $f_age;
            $user_data1[0]["dob"] = $data['user_family_info'][0]["dob"];
        }
        if (empty($user_data1[0]["dob"])) {
            $user_data1[0]["dob"] = '1992-09-30';
        }

        /* Check bitrth date start */
        $age = $util->get_age($user_data1[0]["dob"]);
        $ageinDays = 0;
        if ($age[0] != 0) {
            $ageinDays += ($age[0] * 365);
            $user_data1[0]["age"] = $age[0];
            $user_data1[0]["age_type"] = 'Y';
        }
        if ($age[0] == 0 && $age[1] != 0) {
            $ageinDays += ($age[1] * 30);
            $user_data1[0]["age"] = $age[1];
            $user_data1[0]["age_type"] = 'M';
        }
        if ($age[0] == 0 && $age[1] == 0 && $age[2] != 0) {
            $ageinDays += ($age[2]);
            $user_data1[0]["age"] = $age[2];
            $user_data1[0]["age_type"] = 'D';
        }
        ...
        if ($pid > 0 && $tid > 0 && !empty($job_details)) {
            if ($fid > 0) {
                $data = $this->service_model->get_val("SELECT 
  job_master.id,
  job_master.`order_id`,
  `booking_info`.`family_member_fk`,
  `processing_center`.`branch_fk`,
  job_master.date
FROM
  job_master 
  INNER JOIN `booking_info` 
    ON job_master.`booking_info` = `booking_info`.`id` 
    LEFT JOIN `processing_center` ON `processing_center`.`lab_fk`=`job_master`.`branch_fk` AND `processing_center`.`status`='1' 
WHERE job_master.`status` != '0' 
  AND job_master.`cust_fk` = '" . $pid . "' 
  AND job_master.`status` IN (8, 2) 
  AND booking_info.`family_member_fk` = '" . $fid . "' 
ORDER BY `job_master`.`id` ASC ");
            } else {
                $data = $this->service_model->get_val("SELECT 
  job_master.id,
  job_master.`order_id`,
...
ORDER BY `job_master`.`id` ASC ");
            ...
            foreach ($data as $key) {
                $get_test_parameter = $this->service_model->get_val("SELECT `test_parameter`.*,`test_master`.`test_name`,`test_master`.PRINTING_NAME,`test_master`.report_type FROM `test_parameter` INNER JOIN `test_master` ON `test_parameter`.`test_fk`=`test_master`.`id` WHERE `test_parameter`.`status`='1' AND `test_master`.`status`='1' AND `test_parameter`.`test_fk`='" . $tid . "' and `test_parameter`.`center`='" . $key["branch_fk"] . "' order by `test_parameter`.order asc");
                $pid = array();
                ...
                if (!empty($pid)) {
                    $para = $this->service_model->get_val("SELECT test_parameter_master.id,test_parameter_master.parameter_name,test_parameter_master.parameter_unit FROM `test_parameter_master` WHERE `status`='1' AND id IN (" . implode(",", $pid) . ") ORDER BY FIELD(id," . implode(",", $pid) . ") and test_parameter_master.parameter_name!=''");
                    if (!empty($para)) {
                        $cnt_1 = 0;
                        $para_new = array();
                        foreach ($para as $para_key) {
                            $para_user_val = $this->service_model->get_val("SELECT value from user_test_result where test_id='" . $tid . "' and parameter_id='" . $para_key["id"] . "' and job_id='" . $key['id'] . "' and  status='1'");
                            if (!empty($para_user_val)) {
                                $para_key["user_val"] = $para_user_val;


                                $final_qry = "SELECT parameter_referance_range.id,parameter_referance_range.ref_range_low,parameter_referance_range.ref_range_high,
  CASE
    WHEN (type_period = 'Y') 
    THEN (no_period * 365) 
    ELSE (
  ...
  END AS col1  FROM `parameter_referance_range` WHERE STATUS='1' AND `parameter_fk`='" . $para_key["id"] . "'";
                                ...
                                if (strtoupper($user_data1[0]["gender"]) == 'MALE') {
                                    ...
                                    $data["common"] = 0;
                                } else if (strtoupper($user_data1[0]["gender"]) == 'FEMALE') {
                                ...
                                $para_key["refrance_range"] = $this->service_model->get_val($final_qry);
                                $cnt_1++;
                                if (!empty($para_key["refrance_range"])) {
                                    $para_new[] = $para_key;
                                }
                            }
                        }
                        if (!empty($para_new)) {
                            $key["parameters"] = $para_new;
                        }
                    }
                }
                if (!empty($key["parameters"])) {
                    $new_data_array[] = $key;
                }
            }
            ...
            echo $this->json_data("0", "Parameter are required.", "");
        }
    }

    function getPatientStatisticsNew() {
        $jid = $this->input->get_post("jid");
        $job_details = $this->service_model->get_val('SELECT 
  job_master.id,
  `job_master`.`cust_fk`,
...
FROM
  ...
  INNER JOIN `booking_info` 
    ON job_master.`booking_info` = `booking_info`.`id` WHERE `job_master`.`status`!="0" AND `job_master`.`id`="' . $jid . '"');
        ...
        $total_parameter_id = array();
        $total_parameter = array();
        $this->load->library("util");
        $util = new Util;
        $user_data1 = $this->service_model->master_fun_get_tbl_val("customer_master", array('status' => 1, 'id' => $pid), array("id", "asc"));
        if (empty($user_data1[0]["gender"]) && empty($user_data1[0]["age"])) {
...
ORDER BY `job_master`.`id` ASC ");
                ...
                if (!empty($pid)) {
                    ...
                    if (!empty($para)) {
                        ...
                        foreach ($para as $para_key) {
                            $para_user_val = $this->service_model->get_val("SELECT value from user_test_result where test_id='" . $tid . "' and parameter_id='" . $para_key["id"] . "' and job_id='" . $key['id'] . "' and  status='1'");
                            if (!empty($para_user_val)) {
                                $para_key["user_val"] = $para_user_val[0]["value"];
  ...
  END AS col1  FROM `parameter_referance_range` WHERE STATUS='1' AND `parameter_fk`='" . $para_key["id"] . "'";
                                ...
                                $refrance_range = $this->service_model->get_val($final_qry);
                                $para_key["refrance_range_low"] = $refrance_range[0]["ref_range_low"];
                                $para_key["refrance_range_high"] = $refrance_range[0]["ref_range_high"];

                                $cnt_1++;
                                if (!empty($refrance_range)) {
                                    if (!in_array($para_key["id"], $total_parameter_id)) {
                                        $total_parameter[] = $para_key;
                                        $total_parameter_id[] = $para_key["id"];
                                    }
                                    $para_new[] = $para_key;
                                }
                            }
            ...
            //echo "<pre>";print_r($total_parameter);die();

            $new_response_formate = array();
            foreach ($total_parameter as $key):
                $key = array("id" => $key["id"], "parameter_name" => $key["parameter_name"], "parameter_unit" => $key["parameter_unit"]);
                $allParameterval = array();
                foreach ($new_data_array as $skey):
                    foreach ($skey['parameters'] as $tkey):
                        if ($key["id"] == $tkey["id"]):
                            $tkey["date"] = $skey["date"];
                            $allParameterval[] = array("user_val" => $tkey["user_val"], "refrance_range_low" => $tkey["refrance_range_low"], "refrance_range_high" => $tkey["refrance_range_high"], "date" => $tkey["date"]);
                        endif;
                    endforeach;
                endforeach;
                $key["details"] = $allParameterval;
                $new_response_formate[] = $key;
            endforeach;
            if (!empty($new_response_formate)) {
                echo $this->json_data("1", "", $new_response_formate);
            } else {
                echo $this->json_data("0", "Data not available.", "");
            }
        } else {
    ...
    function searchjob_doctor() {
        ...
        if ($name != "" && strlen($name) > 2) {
            $data = $this->service_model->search_doctornew($name);
            if (empty($data)) {
                echo $this->json_data("0", "no data available", "");
            } else {
                echo $this->json_data("1", "", $data);
    ...
    function branchtest_list() {
        ...
        if ($branch) {
            ...
            $qry = "SELECT tm.`id`,tm.`TEST_CODE`,tm.`test_name` FROM test_master tm INNER JOIN test_branch_price tp ON tp.test_fk = tm.id WHERE tp.branch_fk='$branch' AND tm.status ='1' AND tp.status ='1' AND tp.`type`='1' ORDER BY tm.`test_name` ASC";
