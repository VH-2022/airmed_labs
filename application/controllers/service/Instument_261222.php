...
class Instument extends CI_Controller {
	...
	function uploadData_test() {
        ...
        if (!empty($Lab_id) && !empty($Test_Date_Time) && !empty($Analyte_code) && !empty($Result_value) && !empty($Status)) {
            ...
            //$data = $this->patholab_api_model->master_fun_get_tbl_val("instument_data_storage_test", array("lab_id" => $Lab_id, "analyte_code" => $Analyte_code), array("id", "desc"));
			$data = $this->patholab_api_model->master_fun_get_tbl_val("instument_data_storage", array("lab_id" => $Lab_id, "analyte_code" => $Analyte_code), array("id", "desc"));
            if(count($data) == 0)
            {
                $insert = array(
                    "lab_id" => $Lab_id,
				...
				$app_info = $this->patholab_api_model->master_fun_insert("instument_data_storage", $insert);
                //$app_info = $this->patholab_api_model->master_fun_insert("instument_data_storage_test", $insert);
            }
            else
            {
                $update = array(
                ...
                //$app_info = $this->patholab_api_model->master_fun_update("instument_data_storage_test", array("lab_id" => $Lab_id, "analyte_code" => $Analyte_code), $update);
				$app_info = $this->patholab_api_model->master_fun_update("instument_data_storage", array("lab_id" => $Lab_id, "analyte_code" => $Analyte_code), $update);
            }
            echo $this->json_data("1", "", "success");
        } else {
            echo $this->json_data("0", "All fields are required.", "");
