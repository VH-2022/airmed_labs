  ...
  END AS col1  FROM `parameter_referance_range` WHERE STATUS='1' AND `parameter_fk`='" . $para_key["id"] . "'";
    ...
    function branchtest_list() {
        ...
        if ($branch) {
            ...
            $qry = "SELECT tm.`id`,tm.`TEST_CODE`,tm.`test_name` FROM test_master tm INNER JOIN test_branch_price tp ON tp.test_fk = tm.id WHERE tp.branch_fk='$branch' AND tm.status ='1' AND tp.`type`='1' ORDER BY tm.`test_name` ASC";
