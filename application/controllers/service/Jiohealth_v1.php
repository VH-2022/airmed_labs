...
class Jiohealth_v1 extends CI_Controller {
    ...
    function booking() {
        ...
        if (in_array($token, $this->token)) {
            ...
            if (!empty($name) && !empty($phone) && !empty($gender) && !empty($dob) && !empty($test_city) && (!empty($test) || !empty($package))) {
                ...
                if (!empty($order_id)) {
                    $msg = "You have new booking from JIO Health app.";
                    $send_sms = array('8511153892', '8511195347', '9726689090');

                    foreach ($send_sms as $kkey) {
                        $this->job_model->master_fun_insert("admin_alert_sms", array("mobile_no" => $kkey, "message" => $msg, "is_send" => "0", "created_date" => date("Y-m-d H:i:s")));
                    }
