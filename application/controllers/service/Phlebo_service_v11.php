...
class Phlebo_service_v11 extends CI_Controller {
...
WHERE `doctor_master`.`status` = '1' AND `doctor_master`.`city` = '" . $data1[0]["city_fk"] . "'
  AND (
    ...
    function check_api_version() {
        $check_version_array = array("android" => array("version" => "26", "compulsory" => "yes", "message" => "New version available please update it and enjoy new feature."), "ios" => array("version" => "2", "compulsory" => "no", "message" => "New version available please update it and enjoy new feature."));
	...
	function packagetest(){
		...
		foreach($packgeall as $packid){
			...
			foreach($package_test as $test){
				...
				$testall[]=array("id"=>'t-'.$test["id"],"testname"=>$test["test_name"],"sample_type"=>$test["sample_type"],"tat"=>$test["reporting"]);
				$testcheck[]=$test["id"];
				}
			}
			}
			...
			if(strtoupper($id[0])=="T"){
                ...
                $testall[]=array("id"=>'t-'.$p_test1[0]["id"],"testname"=>$p_test1[0]["test_name"],"sample_type"=>$p_test1[0]["sample"],"tat"=>$p_test1[0]["reporting"]);
            ...
            foreach ($p_test as $tp_key) {
                ...
                $testall[]=array("id"=>'t-'.$tp_key["id"],"testname"=>$tp_key["test_name"],"sample_type"=>$tp_key["sample"],"tat"=>$tp_key["reporting"]);
				$testcheck[]=$tp_key["id"];
				} 
        }
			}
