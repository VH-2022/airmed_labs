...
class Phlebo_service_v10 extends CI_Controller {
...
WHERE `doctor_master`.`status` = '1' AND `doctor_master`.`city` = '" . $data1[0]["city_fk"] . "'
  AND (
    ...
    function check_api_version() {
        $check_version_array = array("android" => array("version" => "25", "compulsory" => "yes", "message" => "New version available please update it and enjoy new feature."), "ios" => array("version" => "2", "compulsory" => "no", "message" => "New version available please update it and enjoy new feature."));
...
function branchtest_list() {
        ...
        if ($branch != "") {
            $new_array = array();
            $qry = "SELECT tm.`id`,tm.`TEST_CODE`,tm.`test_name`,tm.description,tp.`price` FROM test_master tm INNER JOIN test_branch_price tp ON tp.test_fk = tm.id WHERE tp.branch_fk='$branch' AND tm.status ='1' AND tp.`type`='1' ORDER BY tm.`test_name` ASC";
            ...
            foreach ($package as $key) {
                $new_array[] = array("id" => "p-" . $key["id"], "test_name" => $key['title'], "price" => $key["price"], "description" => $key['desc_app']);
            }
