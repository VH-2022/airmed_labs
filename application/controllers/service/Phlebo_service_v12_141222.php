...
//        }
    ...
    function branchtest_list() {
        ...
        if ($branch != "") {
            $new_array = array();
            $qry = "SELECT tm.`id`,tm.`TEST_CODE`,tm.`test_name`,tm.description,tp.`price`,sample as sample_type,reporting FROM test_master tm INNER JOIN test_branch_price tp ON tp.test_fk = tm.id WHERE tp.branch_fk='$branch' AND tm.status ='1' AND tp.`type`='1' ORDER BY tm.`test_name` ASC";
            ...
            $package = $this->service_model->get_val("SELECT tm.id,tm.title,tm.desc_app,tp.price FROM package_master tm INNER JOIN test_branch_price tp ON tp.test_fk = tm.id   WHERE tp.branch_fk='$branch' AND tm.status ='1' AND tp.`type`='2'");
