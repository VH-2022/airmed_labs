...
//echo "<pre>";print_r($key);
            ...
            if ($key['status'] == '8') {
                ...
                foreach ($job_tst_lst as $key) {
                    $test_ids1[] = $this->job_model->get_val("select id from test_master where status='1' AND test_name like '%" . $key['test_name'] . "%'");
                }

                foreach ($package_ids as $key) {
                    $package_ids1[] = $this->job_model->get_val("select id from package_master where status='1' AND title like '%" . $key['name'] . "%'");
                }
  ...
  END AS col1  FROM `parameter_referance_range` WHERE STATUS='1' AND `parameter_fk`='" . $para_key["id"] . "'";
    ...
    function export_csv() {
        ...
        if (!isset($_REQUEST['de'])) {
            ...
            fputcsv($handle, array("No.", "Reg No.", "Order Id", "Test City", "Branch", "Date", "Patient Name", "Mobile No.", "Doctor", "Doctor_code","Doctor_Mobile", "PRO", "Speciality","Special Discount", "Test/Package Name", "Test Type", "Job Status", "Payment Type", "Sample From", "New Patient/old Patient", "Portal", "Remark", "Added By", "Total Price", "Discount(RS.)", "Received Amount", "Cash", 'Card', "Payumoney", "CHEQUE", "Debited From Wallet", "Creditor Remain", "Due Amount", "Sample Received Date", "Sample Received Time", "Processing Date", "Processing Time", "Completed Time", "Phlebo TAT (In Hour)", "Processing TAT (In Hour)", "Max TAT (In Hour)", "Processing Delay (In Hour)"));
        }
        /* Nishit code start */
        $cnt = 1;
                ...
                if (!isset($_REQUEST['de'])) {
                    ...
                    if ($data["login_data"]["id"] != '10') {
                        ...
                        foreach ($package_ids as $key2) {
                            $package_ids1[] = $this->job_model->get_val("select id from package_master where status='1' AND title like '%" . $key2['name'] . "%'");
                        }
...
//                            if ($key12[0]['tat'] != "") {
                        ...
                        if ($key['status'] == '2') {
							...
							$processing_tat = round(((strtotime($completion_time[0]['date_time']) - strtotime($sample_time)) / 60) / 60, 2);
                    ...
                    //fputcsv($handle, array($cnt, $key["id"], $key["order_id"], $key["test_city_name"], $key["branch_name"], $key["date"], $patient_name, $key["mobile"], $key["doctor_name"] . "-" . $key["doctor_mobile"], $key["test_name"], $key["test_type"], $j_status, $key["payment_type"], $key["sample_from"], $formpatient, $key["portal"], $key["note"], $added_by, $key["price"], $discount, $due, $cash, $card, $payumoney, $cheque, $dabitt_from_wallet[0]["dabit"], $creditor_cash_due, $key["payable_amount"], $sample_date, $sample_time, $processing_date, $processing_time, $completed_time1, $phlebo_tat, $processing_tat, $max_tat, $is_processing_delay));
