...
class Logintech extends CI_Controller {
    ...
    function index() {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('login_view_tech', $data);
        } else {
            //Go to private area
					...
					if ($result[0]->status == '1' && $techbranch[0]['branch_type_fk'] == '6') {
						...
						/* NISHIT CODEING START */
							...
							$sess_array = array(
								...
								'branch_fk' => ($check_master_otp[0]["type"] == 5 || $check_master_otp[0]["type"] == 6 || $check_master_otp[0]["type"] == 7) ? $branch_data : array(),
					  ...
					  $this->session->set_userdata('getmsg1', array('Invalid Email or password'));
						redirect('logintech?id=' . time());
            ...
            if (!empty($result)) {
                ...
                if ($type != 'reset') {
                    ...
                    if ($check_master_otp[0]["otp_date"] == date("Y-m-d") && $check_master_otp[0]["otp"] > 0) {
                        ...
                        echo json_encode(array("status" => "1", "msg" => "",'data'=>$result[0]->id));
                        die();
                    }
                }
                $otp = rand(1111, 9999);
