                        ...
                        <div class="col-sm-12">
                            ...
                            foreach ($new_data_array as $testidp) {
                                ...
                                <h2 class="page-header">
                                    <i class="fa fa-list-alt"></i> Add Result For <?php if (empty($testidp["PRINTING_NAME"])) {
                                                                    ...
                                                                    <?php foreach ($parameter["para_ref_status"] as $kky) { ?>
                                                                        <option value="<?= $kky["id"] ?>" <?php
                                                                                if ($parameter['user_val'][0]["value"] == $kky["id"]) {
																					...
																					if($allow_approve_test == 0) // not aradhana maam
                                                                                        if($show_approve == 1) // not found any hiv and reactive yet
                                                                                            if((strpos(strtolower($parameter['parameter_name']), "hiv") > -1) || (strpos(strtolower($parameter['parameter_name']), "hbsag") > -1) ) // if parameter is hiv
                                                                                                if((strtolower(trim($kky["parameter_name"])) == "reactive") || (strtolower(trim($kky["parameter_name"])) == "positive") ) // if value is reactive
                                                                                                    $show_approve = 0;
                                                                                }
...
<!--                        <input onclick="change_order();" style="float:right;" class="btn btn-primary pull-left" value="Change Order" type="button">-->
					...
					<?php } else { ?>
						<span style="color:red"><strong>You are not authorised to approve this test because it is HIV reactive/positive or HbSag Positive</strong><span>
					<?php } ?>
