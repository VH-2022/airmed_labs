...
<select class="chosen-select" data-live-search="true" id="test" data-placeholder="Select Test" onchange="get_test_price();">
    ...
    <?php foreach ($test as $ts) { ?>
        <option value="t-<?php echo $ts['id']; ?>" <?php ?>> <?php echo ucfirst($ts['test_name']); ?> ( <?php echo ($ts['thyrocare_code']); ?> ) (Rs.<?php echo $ts['price']; ?>) (SpecialPrice.<?php echo $ts['special_price']; ?>)</option>
    <?php } ?>
