        ...
        <div class="col-md-12">
            ...
            <div class="box box-primary">
			 ...
			 <div class="box-body">
                    ...
                    <select class="chosen chosen-select" id ="branch_id" name="branch_fk" onchange="getReagent();">
                        ...
                        <?php
                            ...
                            foreach ($branch as $val) {
                                    ?>
								<option value="<?= $val["BranchId"]; ?>" <?php if($query[0]["branchfk"]==$val["BranchId"]){ echo "selected"; } ?> ><?= $val["BranchName"] ?></option>
                            <?php }
                            ?>
                        </select>
                    ...
                    <select class="chosen chosen-select" id ="vendor" name="vendorname"  >
                        ...
                        <?php
                            foreach ($vendor_list as $venal) {
                                    ?>
							<option value="<?= $venal["id"]; ?>" <?php if($query[0]["vendorid"]==$venal["id"]){ echo "selected"; } ?> ><?= ucwords($venal["vendor_name"]); ?></option>
                            <?php }
                            ?>
                        </select>
				...
				<div class="form-group">
				 ...
				 <input type="text" name="indentcode" id="indentcode" value="<?= $query[0]["indentcode"]; ?>" class="form-control" />
			...
			<select class="form-control calution1" name="itemtext[]" >
					   ...
					   <div class="table-responsive pending_job_list_tbl">
						...
						foreach ($itemtext as $txt) {
                                ?>
                                <option <?php if($txt["id"]==$row["txtid"]){ echo "selected"; } ?> rel="<?php if($txt["id"]==$row["txtid"]){ echo $row["itemtxt"]; }else{ echo $txt["tax"]; } ?>" value="<?= $txt["id"] ?>"><?= $txt["title"]."(".$txt["tax"]."%)"; ?></option>
                            <?php } ?>
                        </select></p></td>
...
var pogid=parseInt("<?= $query[0]["id"]; ?>");
	$(document).on('keydown', '.calution', function(e) {		
        ...
        $.ajax({
            url:"<?php echo base_url();?>inventory/intent_genrate/getsub",
            type:"POST",
            data:{branch_fk:id,pogid:pogid},
...
$(document).on('keyup','#pricediscount', function() {
 ...
 if(indentcode == ""){ $("#errorindedcode").html("Vendor field is required"); var check=1;  }else{ $("#errorindedcode").html(""); }
