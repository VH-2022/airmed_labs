        ...
        <div class="col-md-12">
            ...
            <div class="box box-primary">
			 ...
			 <div class="box-body">
                    ...
                    <div class="col-md-6">
					 ...
					 <?php  echo form_open("inventory/intent_genrate/poigenerate", array("method" => "POST", "role" => "form","id"=>'poform'));  ?>
                    ...
                    <select class="chosen chosen-select" id ="vendor" name="vendorname"  >
                        ...
                        <?php
                            foreach ($vendor_list as $venal) {
                                    ?>
							<option value="<?= $venal["id"]; ?>"><?= ucwords($venal["vendor_name"]); ?></option>
                            <?php }
                            ?>
                        </select>
				...
				<div class="form-group">
				 ...
				 <input type="text" name="indentcode" id="indentcode" class="form-control" />
			...
			<select class="form-control calution1" name="itemtext[]" >
					   ...
					   <div class="table-responsive pending_job_list_tbl">
						...
						<td colspan='8'></td>
						<td>Discount(%)</td>
						<td colspan='2'><input id="pricediscount" type="text"  name="pricediscount" value="0" /><br><span id="errordisccount" style="color: red;"></span></td>
						</tr>
						
						<tr>
						<td colspan='8'></td>
						<td>Total Amount Rs.</td>
						<td colspan='2'><input id="maintotalprice" type="text" readonly name="maintotal" value="0" /></td>
						</tr>
                    </table>
...
function tablecal(tid){
		...
		var trid=explode[1];
		var nos=parseInt($("#nos_"+trid).val());
		if(nos > 0){ var nos=nos; }else{ var nos=1; }
		var qty=1;
		var ratepertest=parseInt($("#rateqty_"+trid).val());
		 var itemdis =parseInt($("#itemdis_"+trid).val());
		 $("#errorperdis_"+trid).html("");
		if(itemdis >= 0 && itemdis < 99){ var perdiscount=parseInt(itemdis); }else{ $("#errorperdis_"+trid).html("Please enter valid discount"); var perdiscount=0; }
		...
		$("#testamount_"+trid).html(disamount);
		var itemtxt=parseInt($("#txtid_"+trid+" select option:selected").attr('rel'));
		if(itemtxt > 0){	
		
		var totalamount=(multiqty+(multiqty*itemtxt/100));
		$("#totalamount_"+trid).val(totalamount);
		}else{ $("#totalamount_"+trid).val(multiqty); }
...
$(document).on('change','.calution1', function() {
		var itemtxt=parseInt($('option:selected', this).attr('rel'));
		var curow=$(this).parent().attr("id");
		var explode = curow.split('_');
		var textid=explode[0];
		var trid=explode[1];
		var tamount1=parseInt($("#testamount_"+trid).html());
		var itemdis =parseInt($("#itemdis_"+trid).val());
		
		if(itemdis >= 0 && itemdis < 99){ var perdiscount=parseInt(itemdis); }else{ var perdiscount=0; }
	...
	function maintotalcchange(){
		var dis1=parseInt($("#pricediscount").val());
		if(dis1 > 0 ){
...
$(document).on('keyup','#pricediscount', function() {
 ...
 if(vendor == ""){ $("#errorvendor").html("Vendor field is required"); var check=1;  }else{ $("#errorvendor").html(""); }
 if(indentcode == ""){ $("#errorindedcode").html("Indent Code field is required"); var check=1;  }else{ $("#errorindedcode").html(""); }
...
$("select[name='itemtext[]']").each(function(){
	...
	it++;
	   ...
	   $.ajax({
            url:"<?php echo base_url();?>inventory/intent_genrate/poigenerate",
            type:"POST",
            data:{branch_fk:baranch,vendorname:vendor,indentcode:indentcode,maintotal:pricetotal,pricediscount:pricediscount,item:item,itemtext:itemtext,nosval:nosval,rateqty:rateqty,remark:remark,itemdis:itemdis},
            success:function(data){
