    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                ?>
                <input type="hidden" name="package_type" value="<?php echo $ttype; ?>">
...
<div class="modal fade" id="import-rcode" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="H4">Import R Code</h4>
            </div>
            <div class="modal-body">
                <?php $attributes = array('class' => 'form-horizontal', 'method' => 'POST'); ?>
                <?php
                echo form_open_multipart("Branch_Test_Price/importrcode_csv/$cid", $attributes);
                ?>
                <input type="hidden" name="package_type" value="<?php echo $ttype; ?>">
                <div class="">
                    <label>Upload</label>
                ...
                <div class="modal-footer">
                    <a class="btn btn-success" href="<?php echo base_url(); ?>/upload/test_r_code_sample.csv">Download Sample</a>
                    <button type="button" id="model_close" class="btn btn-danger" data-dismiss="modal">Close</button>
                    <input type="submit" id="add_admin_submit" name="add_menu" class="btn btn-primary" value="Upload" />
                    <!--                    <button type="button" id="add_admin_submit"  data-dismiss="modal"  onclick="sub('admin_add');" name="add_menu" class="btn btn-primary" disabled=''> Add </button>-->
                    </form>
...
<div class="modal fade" id="show-rcode-unimport" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="H4">Test Not Found</h4>
            </div>
            <div class="modal-body">
...
<div class="modal fade" id="show-price-unimport" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
...
<script type="text/javascript">
    ...
    function ra_change(ctrl, id) {
        let val = ctrl.value;
        $.post("<?php echo base_url(); ?>Branch_Test_Price/update_code", {
                id: id,
                r_code: val
            },
    ...
    $(document).ready(function() {
        console.log(<?= count($success[1])?>);
        console.log(<?= count($success[2])?>);
        console.log(<?= $success[2]?>);
        <?php if (isset($success[1]) && count($success[1]) > 0) { ?>
            $(`#show-rcode-table`).html('');
            var tableData = <?= $success[1] ?>;
            var tableAppend = '';
            ...
            $(`#show-rcode-table`).append(tableAppend);
            $(`#show-rcode-unimport`).modal('show');


        <?php } ?>

        <?php if (isset($success[2]) && count($success[2]) > 1) { ?>
            $(`#show-price-table`).html('');
            var tableData2 = <?= $success[2] ?>;
            var tableAppend2 = '';
            ...
            $(`#show-price-table`).append(tableAppend2);
            $(`#show-price-unimport`).modal('show');
