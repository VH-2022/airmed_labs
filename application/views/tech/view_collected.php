        ...
        <div class="row">
            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    ...
                    <?php echo form_open_multipart("tech/add_result/add_value_exists/" . $test_id, array("id" => "add_result", "method" => "post", "onsubmit" => "return check_hundred()")); ?>
                    ...
                    <div class="box-body">
                        ...
                        <div class="col-sm-12">
                            ...
                            foreach ($new_data_array as $testidp) {
                                ...
                                <input type="checkbox" id="is_formula_<?php echo $cnnt; ?>" checked name="use_formula_<?= $testidp["test_fk"] ?>" value="1" <?php
                                                    ...
                                                    <tr>
                                                        ...
                                                        ?>><?php echo $parameter['parameter_name']; ?></td>
                                                            ...
                                                            <td>
																...
																<input type="hidden" id="<?php echo $parameter['testcode']; ?>" name="<?php echo $parameter['testcode']; ?>" value="<?php echo $parameter['id'];?>">																									
                                                    ...
                                                    ?>><?php echo $parameter['parameter_name']; ?></td>
                                                        ...
                                                        <td><input type="hidden" name="test_id_<?php echo $cnt; ?>" value="<?php echo $testidp["test_fk"]; ?>">
                                                            ...
                                                            <?php if (!empty($parameter["para_ref_status"])) { ?>
                                                                <select  tabindex="<?php echo $tabindex++; ?>"   class="form-control" id="parameter_value_<?php echo $cnt; ?>" name="parameter_value_<?php echo $cnt; ?>" <?php
                                                                ...
                                                                ?>><option value="">--Select--</option>
                                                                        ...
                                                                        ?> data-<?php echo $parameter['testcode']; ?>="<?=trim($kky["testcode"]); ?>"><?= $kky["parameter_name"]; ?></option>
                                                                            <?php } ?>
                                                                </select>
                                                                <?php
                                                            } else {
...
<script>
    function get_html_design(id, textarea_id) {
        if (id.trim() != '' && textarea_id.trim() != '') {
            var url = "<?= base_url(); ?>tech/Add_result/get_culture_design/" + id;
            $.ajax({
                url: url,
...
<?php echo $closeFancyBox[0]; ?>
    ...
    function change_order() {
        $("#add_result").attr("action", "<?php echo base_url(); ?>tech/add_result/change_order");
        setTimeout(function () {
            $("#add_result").submit();
        }, 500);
    }
    ...
    function add_reference(val, id, val1) {
        jq.fancybox.open({
            href: '<?= base_url(); ?>tech/add_result/add_parameter/' + val + '/' + val1 + '/' + id,
            type: 'iframe',
            padding: 5,
            width: '100%',
        });
    ...
    function edit_parameter(val, val1, brnch) {
        jq.fancybox.open({
            href: '<?= base_url(); ?>tech/add_result/add_parameter/' + val + '/' + val1 + '/' + brnch,
            type: 'iframe',
            padding: 5,
            width: '100%',
            onComplete: function () {
    ...
    function manage_culure_para(test_fk, brnch) {
        jq.fancybox.open({
            href: '<?= base_url(); ?>tech/add_result/manage_culture?test_fk=' + test_fk + '&branch=' + brnch,
            type: 'iframe',
            padding: 5,
            width: '100%',
            onComplete: function () {
	...
	function check_autoapprove_cbctest(){
		/*var hemoglobin = $("#0_43790").val();
		var mcv = $("#0_44671").val();
		var platelets_cnt = $("#0_44522").val();
		var total_wbc_cnt = $("#0_44521").val();
		var Polymorphs = $("#0_43798").val();
		var lymphocytes = $("#0_43702").val();
		var Monocytes = $("#0_43873").val();
		var Eosinophils = $("#0_43874").val();
		var Basophils = $("#0_43803").val();*/
		//alert("0_"+$("#HB").val());
		var hemoglobin = $("#0_"+$("#HB").val()).val();
		var mcv = $("#0_"+$("#MCV").val()).val();
		var platelets_cnt = $("#0_"+$("#PLT").val()).val();
		var total_wbc_cnt = $("#0_"+$("#TWC").val()).val();
		var Polymorphs = $("#0_"+$("#POL").val()).val();
		var lymphocytes = $("#0_"+$("#LYM").val()).val();
		var Monocytes = $("#0_"+$("#MON").val()).val();
		var Eosinophils = $("#0_"+$("#EOS").val()).val();
		var Basophils = $("#0_"+$("#BAS").val()).val();
		
		
		if((hemoglobin>=6 && hemoglobin<=7.99) && mcv >100){
			//$("#parameter_value_32").val('14451');
			$("#parameter_value_32").val($('select').find('[data-ssrbc="RBC8"]').val());
		}else if((hemoglobin>=6 && hemoglobin<=7.99) && mcv <80){
			//$("#parameter_value_32").val('14448');
			$("#parameter_value_32").val($('select').find('[data-ssrbc="RBC5"]').val());
		}else if((hemoglobin>=6 && hemoglobin<=7.99) && (mcv >=80 && mcv<=100)){
			//$("#parameter_value_32").val('14715');
			$("#parameter_value_32").val($('select').find('[data-ssrbc="RBC29"]').val());
		}else if((hemoglobin>=8 && hemoglobin<=9.99) && mcv >100){
			//$("#parameter_value_32").val('14765');
			$("#parameter_value_32").val($('select').find('[data-ssrbc="RBC33"]').val());
		}else if((hemoglobin>=8 && hemoglobin<=9.99) && mcv <80){
			//$("#parameter_value_32").val('14658');
			$("#parameter_value_32").val($('select').find('[data-ssrbc="RBC23"]').val());
		}else if((hemoglobin>=8 && hemoglobin<=9.99) && (mcv >=80 && mcv<=100)){
			//$("#parameter_value_32").val('14668');
			$("#parameter_value_32").val($('select').find('[data-ssrbc="RBC25"]').val());
		}else if((hemoglobin>=10 && hemoglobin<=12.99) && mcv >100){
			//$("#parameter_value_32").val('14450');
			$("#parameter_value_32").val($('select').find('[data-ssrbc="RBC7"]').val());
		}else if((hemoglobin>=10 && hemoglobin<=12.99) && mcv <80){
			//$("#parameter_value_32").val('14447');
			$("#parameter_value_32").val($('select').find('[data-ssrbc="RBC4"]').val());
		}else if((hemoglobin>=10 && hemoglobin<=12.99) && (mcv >=80 && mcv<=100)){
			//$("#parameter_value_32").val('14445');
			$("#parameter_value_32").val($('select').find('[data-ssrbc="RBC2"]').val());
		}else if((hemoglobin>=13 && hemoglobin<=17) && mcv >100){
			//$("#parameter_value_32").val('14449');
			$("#parameter_value_32").val($('select').find('[data-ssrbc="RBC6"]').val());
		}else if((hemoglobin>=13 && hemoglobin<=17) && mcv <80){
			//$("#parameter_value_32").val("14446");
			$("#parameter_value_32").val($('select').find('[data-ssrbc="RBC3"]').val());
		}else if((hemoglobin>=13 && hemoglobin<=17) && (mcv >=80 && mcv<=100)){	
			//$("#parameter_value_32").val("14444");
			$("#parameter_value_32").val($('select').find('[data-ssrbc="RBC1"]').val());
		}
		
		if(platelets_cnt!=null && platelets_cnt<50000){
			//$("#parameter_value_36").val('14838');
			$("#parameter_value_36").val($('select').find('[data-ssplt="PLT10"]').val());
		}else if(platelets_cnt!=null && (platelets_cnt>=50000 && platelets_cnt<=99999)){
			//$("#parameter_value_36").val('14792');
			$("#parameter_value_36").val($('select').find('[data-ssplt="PLT7"]').val());
		}else if(platelets_cnt!=null && (platelets_cnt>=100000 && platelets_cnt<=149999)){
			//$("#parameter_value_36").val('14475');
			$("#parameter_value_36").val($('select').find('[data-ssplt="PLT3"]').val());
		}else if(platelets_cnt!=null && (platelets_cnt>=150000 && platelets_cnt<=450000)){
			//$("#parameter_value_36").val('14473');
			$("#parameter_value_36").val($('select').find('[data-ssplt="PLT1"]').val());
		}else if(platelets_cnt!=null && (platelets_cnt>=450001 && platelets_cnt<=600000)){
			//$("#parameter_value_36").val('14476');
			$("#parameter_value_36").val($('select').find('[data-ssplt="PLT4"]').val());
		}
		
		/*else if(platelets_cnt>=600000){
			return;
		}*/
		
		if(total_wbc_cnt>=4000 && total_wbc_cnt<=10000){
			if((Polymorphs>=40 && Polymorphs<=70) && (lymphocytes>=20 && lymphocytes<=40) && (Monocytes>=2 && Monocytes<=10) && (Eosinophils>=1 && Eosinophils<=6) && (Basophils>=0 && Basophils<=2)){
				//$("#parameter_value_34").val('14472');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC11"]').val());
			}else if(Polymorphs>70){
				//$("#parameter_value_34").val('14462');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC1"]').val());
			}else if(lymphocytes>40){
				//$("#parameter_value_34").val('14468');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC7"]').val());
			}else if(Eosinophils>6){
				//$("#parameter_value_34").val('14469');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC8"]').val());
			}else if(Monocytes>10){
				//$("#parameter_value_34").val('14470');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC9"]').val());
			}
		}else if(total_wbc_cnt>10000){
			if(Polymorphs>70){
				//$("#parameter_value_34").val('14465');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC4"]').val());
			}else if(lymphocytes>40){
				//$("#parameter_value_34").val('14466');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC5"]').val());
			}else if(Eosinophils>6){
				//$("#parameter_value_34").val('14595');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC12"]').val());
			}else if(Monocytes>10){
				//$("#parameter_value_34").val('14596');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC13"]').val());
			}
		}else if(total_wbc_cnt<4000){
			if(Polymorphs>70){
				//$("#parameter_value_34").val('14597');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC14"]').val());
			}else if(lymphocytes>40){
				//$("#parameter_value_34").val('14598');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC15"]').val());
			}else if(Eosinophils>6){
				//$("#parameter_value_34").val('14599');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC16"]').val());
			}else if(Monocytes>10){
				//$("#parameter_value_34").val('14600');
				$("#parameter_value_34").val($('select').find('[data-sswbc="WBC17"]').val());
			}
		}
		
		//alert($("#parameter_value_38").val());
		if($("#parameter_value_38").val()==""){
			//$("#parameter_value_38").val('14478');
			$("#parameter_value_38").val($('select').find('[data-sspsmp="PSMP1"]').val());
		}
	}
    ...
    function calculate_result(ids) {
        ...
        var url = "<?php echo base_url(); ?>tech/add_result/formula_calculation";
