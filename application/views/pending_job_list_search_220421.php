    ...
    foreach ($query as $row) {
        ...
        <!--                <a  href='<?php echo base_url(); ?>job_master/job_dispatch/<?php echo $row['id']; ?>/<?php
                    ...
                    <?php if ($row['status'] == 8 || $row['status'] == 2 || $row['status'] == 7) { ?>
                        <a href='javascript:voic();' onclick="open_barcode_popup('<?= $row["id"] ?>', '<?= $test_id_1 ?>', '<?= $package_id_1 ?>', '<?= $branch_id ?>', '<?= $status ?>');" data-toggle="tooltip" data-original-title="Print Barcode"><span class=""><i class="fa fa-barcode"></i></span></a>
                    <?php } ?>
