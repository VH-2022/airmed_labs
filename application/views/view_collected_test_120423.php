                                                                        ...
                                                                        if ($parameter['user_val'][0]["value"] == $kky["id"]) {
																			...
																			if($allow_approve_test == 0) // not aradhana maam
                                                                                if($show_approve == 1) // not found any hiv and reactive yet
                                                                                    if(strpos(strtolower($parameter['parameter_name']), "hiv") > -1) // if parameter is hiv
                                                                                        if(strtolower($kky["parameter_name"]) == "reactive") // if value is reactive
                                                                                            $show_approve = 0;
                                                                        }
...
<!--                        <input onclick="change_order();" style="float:right;" class="btn btn-primary pull-left" value="Change Order" type="button">-->
                        <?php if (empty($approve_job_test_status)) { 
							...
							<?php } else { ?>
								<small><b>You are not authorised to approve this test because it is HIV reactive</b></small>
                            <?php } ?>
                        <?php } else { ?>
