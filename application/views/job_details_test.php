...
?>
                ...
                <div class="box-body">
                    <?php if ($query[0]["model_type"] == 1) {
                        ...
                        <div class="col-md-6 res_pdng_0" style="border-right:1px solid #ccc;">
                            ...
                            <div class="form-group">
                                ...
                                if ($query[0]["test_city"] == 1) {
                                    ...
                                    echo "Kolkata Non Metro";
                                } else if ($query[0]["test_city"] == 8) {
                                    echo "Sonepat";
                                } else if ($query[0]["test_city"] == 9) {
                                    echo "Guwahati";
                                } else if ($query[0]["test_city"] == 11) {
                                    echo "Assam";
                                } else if ($query[0]["test_city"] == 12) {
                                    echo "Kolkata";
                                }
                                ?>
                        ...
                        <div class="col-md-6 res_pdng_0">
                            ...
                            <?php $booking_date = explode(" ", $query[0]['date']); if ($login_data['type'] == '1') { ?>
                                <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                ?>  <?php if (!in_array($branchdis, array(8, 23)) || $login_data['type'] == 1 || $login_data['type'] == 2) { ?> <a href="javascript:void(0)" onclick="$('#discount_model').modal('show');"> <i class="fa fa-edit"></i></a> <?php } ?>
                            </div>
                            <div class="form-group"> 
		...
		<div class="modal fade" id="paymentprocess" data-backdrop="static" data-keyboard="false" role="dialog">
                <div class="modal-dialog">

                    <!-- Modal content-->
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4 class="modal-title">Waiting for card swipe</h4>
                        </div>
                        
                        <div class="modal-body">
						<div id="paymentsuccess" class="widget"></div>
                           <center>                            <img src="<?= base_url()."img/signature_terminal_loader.gif" ?>" style="width:30%">
						   <br>
						   Waiting for card swipe
						   </center><br>
						   <b>Plutus transaction reference id :<span id="pltostranid"></span></b>
						   <div id="oldjobsdata"></div>
                        </div>
						<div class="modal-footer">
						<button type="button" id="checkstatus" class="btn btn-primary" data-loading-text="<i class='fa fa-spinner fa-spin '></i>Check Payment Process">Check Payment Status</button>
            ...
            <div class="modal fade" id="payment_model" role="dialog">
                <div class="modal-dialog">
                    ...
                    <div class="modal-content">
                        ...
                        <?php echo form_open("job_master/payment_received/" . $cid, array("method" => "POST", "role" => "form", "id" => "payment_receiv_form_id")); ?>
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <select id="paytype" name="p_type" class="form-control" style="width: 100%" required="">
                                    <option value="">--Select Type--</option>
                                    <?php foreach ($payment_type_list as $key) {
                                       if($key['id'] !="18"){ ?>
                                        <option value="<?php echo $key['name']; ?>"><?php echo ucwords($key['name']); ?></option>

									   <?php }else{ $pinename=$key['name']; }
                                    }
									if($user->pinelab_fk != "" && $user->pinelab_fk != "0"){ ?>
									<option value="<?= $pinename ?>"><?php echo ucwords($pinename); ?></option>
									<?php }
                                    ?>
                                </select>
                                <span style="color:red;" id="amount_error"></span>
    ...
    var $this = $(this);
  $this.button('loading');
   var plutoid=parseInt($("#pltostranid").html());
	
    $.ajax({ url: "<?php echo base_url(); ?>pineLabs/swipe_paymentcheck",
            type: 'GET',
            data: {plutoid:plutoid},
            success: function (data){
				var returnedData = JSON.parse(data);
				var status=returnedData.status;
			var msg=returnedData.msg;
			
			if(status==0){ $("#paymentsuccess").html('<div class="alert alert-success alert-dismissable">Payment Successfully received.</div>'); 
				setTimeout(function(){ window.location.reload(); },5000); }else{
					
					$("#paymentsuccess").html('<div class="alert alert-danger alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>'+msg+'</div>');
				}
...
$('#cancelpayment').on('click', function() {
	...
	$("#loader_div").show();
	 ...
	 $.ajax({ url: "<?php echo base_url(); ?>pineLabs/swipe_paymentcancel",
            type: 'GET',
            success: function (data){
				var returnedData = JSON.parse(data);
				var status=returnedData.status;
				...
				$("#loader_div").hide();
							...
							$("#payment_receiv_form_id").submit(function(e){
		if(confirm('Are you sure?')) {
			var paytype=$("#paytype").val();
			var jobid="<?= $cid ?>";
			
			if(paytype=="CREDIT CARD swiped thru PINE LABS"){
				$("#oldjobsdata").html('');
				var j_amount=$("#j_amount").val();
				 $('#payment_model').modal('toggle');
				 $("#loader_div").show();
				 
			$.ajax({ url: "<?php echo base_url(); ?>pineLabs/swipe_payment",
            type: 'POST',
            data: {jid:<?= $cid; ?>,j_amount:j_amount},
            success: function (data){
				
            var returnedData = JSON.parse(data);
            var status=returnedData.status;
			var msg=returnedData.msg;
			var plutoid=returnedData.plutoid;
			if(status==0){
				
				$("#loader_div").hide();
				$('#paymentprocess').modal('toggle');
				$("#pltostranid").html(plutoid);
				/* var myVar = setInterval(checkpaystatus, 5000,plutoid); */
				
			}else{
				if(plutoid != "0" && plutoid.plutoid != ""){
					
					$("#paymentsuccess").html('<div class="alert alert-danger alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>Last payment transactions is Pending.</div>');
					
					$("#oldjobsdata").html('<b>Order Id :<span>'+plutoid.orderid+'</span></b><br><b>Patient name :<span>'+plutoid.name+'</span></b><br><b>Pay amount :<span>'+plutoid.amount+'</span></b>');
					
					$('#paymentprocess').modal('toggle');
				    $("#pltostranid").html(plutoid.plutoid);
	...
	function checkpaystatus(plutoid) {
		
	 $.ajax({ url: "<?php echo base_url(); ?>pineLabs/swipe_paymentcheck",
            type: 'GET',
            data: {plutoid:plutoid},
            success: function (data){
				if(data==0){ clearInterval(myVar); 
				$("#paymentsuccess").html('<div class="alert alert-success alert-dismissable">Payment Successfully received.</div>'); 
...
<?php } ?>
    ...
    if ("<?= $query[0]["test_city"] ?>" == "9") {
    testcity = 159;
    }
    if ("<?= $query[0]["test_city"] ?>" == "11") {
    testcity = 1514;
    }
    if ("<?= $query[0]["test_city"] ?>" == "12") {
    testcity = 1475;
    }
    $.ajax({
...
<?php } ?>
            ...
            $("#test_" + job_fk + "_" + tst_fk).attr("onclick", "");
<?php if ($login_data['type'] == 1 || $login_data['type'] == 2 || $login_data['type'] == 5) { ?>
...
<?php } ?>
            ...
            if (check_test == 0) {
            if (job_status == '2' || job_status == '8') {
<?php if ($login_data['type'] == 1 || $login_data['type'] == 2 || $login_data['type'] == 5 || $login_data['type'] == 6) { ?>
                $("#test_" + job_fk + "_" + tst_fk).attr("style", "cursor:pointer;");
                $("#test_" + job_fk + "_" + tst_fk).attr("onclick", "approveTest1('" + job_fk + "','" + tst_fk + "')");
                $("#test_" + job_fk + "_" + tst_fk).attr("class", "text_highlight");
<?php } else { ?>
                $("#test_" + job_fk + "_" + tst_fk).removeAttr("onclick", "");
<?php } ?>
            }
            }
