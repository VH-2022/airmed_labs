...
<tbody>
    ...
    foreach ($query as $row) {
        ...
        <!--                <a  href='<?php echo base_url(); ?>job_master/job_dispatch/<?php echo $row['id']; ?>/<?php
                    ...
                    ?>" href='javascript:void(0)' data-toggle="tooltip" data-original-title="View TRF" onclick="dispatched_job_test('<?php echo $row['id']; ?>', '1');"> <span class="warning"><i class="fa fa-file-picture-o"> </i></span> </a>
                    ...
                    <?php /* if ($payable_amount > 0) { ?>
                      ...
                      <?php } */ ?>
                    <?php
                    if ($smsalert == 1 && in_array($login_data["type"], array(1, 2, 5, 6))) {
                        if ($payable_amount > 0) {
                            ?>
                    ...
                    <?php /* <a href='javascript:voic();' onclick="open_barcode_popup('<?= $row["id"] ?>');" data-toggle="tooltip" data-original-title="Print Barcode"><span class=""><i class="fa fa-barcode"></i></span></a> */ ?>

                    <?php if ($row['status'] == 8 || $row['status'] == 2 || $row['status'] == 7) { ?>
                        <a href='javascript:void();' onclick="open_barcode_popup('<?= $row["id"] ?>', '<?= $test_id_1 ?>', '<?= $package_id_1 ?>', '<?= $branch_id ?>', '<?= $status ?>');" data-toggle="tooltip" data-original-title="Print Barcode"><span class=""><i class="fa fa-barcode"></i></span></a>
                    <?php } ?>


                    <?php
                    if (in_array($login_data["type"], array(1, 2)) && in_array($row['status'], array("2", "8"))) {
                        ?>
                        <a href='javascript:void(0);' onclick="hard_report_status('<?= $row["id"] ?>', '<?= $row["order_id"] ?>', '<?= $row['full_name'] ?>', '<?= $status ?>');" data-toggle="tooltip" data-original-title="View Report Status"><span class=""><i class="fa fa-file"></i></span></a>
                        <?php
                    }
                    ?>
                ...
                if ($row['status'] == 8 || $row['status'] == 2) 
                    if ($login_data['type'] != 7) 
                        ...
                        { 
                            ...
                            //else if ($row['whatsappsent'] == 0) 
                            //    echo "<img src='" . base_url() . "whatsapp_fail_icon.png' style='width:20px;height:20px;margin-left:10px;'>";
                            //else
