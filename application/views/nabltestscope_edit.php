...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">Edit Nabl Test Scope</li>
    </ol>
</section>
...
<?php 
					foreach($query as $data)
						$id = $data['id'];
						$testcityid = $data['testcity_fk'];
						$branchid = $data['branch_fk'];
						$testids = explode(",",$data['tests_fk']);
						$testscope = $data['test_scope'];
					{
				?>
		...
		<div class="box-body">
				<div class="col-md-6">
                <!-- form start -->
                <form role="form" action="<?php echo base_url(); ?>Nabltestscope_master/nabltestscope_edit/<?= $id ?>" method="post" enctype="multipart/form-data">
					<div class="form-group">
						...
						<select class="form-control chosen-select" name="testcity">
							...
							<?php foreach ($testcity as $val) { ?>
								<option value="<?php echo $val['id'];?>" <?php if($testcityid==$val['id']){?>selected<?php } ?>><?php echo $val['name'];?></option>
							<?php } ?>					                                                      
						</select>
					...
					<div class="form-group">
						...
						<select class="form-control chosen-select" name="branch">
                            <option value="">--Select--</option>
					...
					<div class="form-group">
						...
						<select class="form-control multiselect-ui" title="Select Test" multiple="multiple" id="test" name="test[]">
							...
							<?php foreach ($tests as $ts) { ?>
								<option value="<?= $ts["id"] ?>" <?php if(in_array($ts['id'],$testids)){?>selected<?php } ?>><?= $ts["test_name"]; ?></option>
							<?php } ?>
						</select>
