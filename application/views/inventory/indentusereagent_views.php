                    ...
                    <tbody>
                        ...
                        foreach ($new_data as $row) {
                            ...
                            foreach($row["old_record"] as $oldkey){
                                $timeperfomace = ($oldkey["timeperfomace"]>0)?$oldkey["timeperfomace"]:1;
                               $used_item += $oldkey["useditem"] * $timeperfomace;
                            }
                            $ata = $row["packet_quantity"] * $row["quantity"];
                            $ata1 = round($ata * 15 /100);
                            $ata2 =  round($ata - $ata1);
                            $ata3 =  round($ata - $used_item);
                            ?>
                        <tr style="<?php if($ata1>=$ata3){ echo "background-color: #FFF1BC;"; } if($ata<$used_item){ echo "background-color: #FFBDBC;"; } ?>">
                                <td><?= $i; ?></td>
                                <td><a target="_blank" href="<?= base_url() . "inventory/indent_usereagent/usdedstocks/" . $row["id"]; ?>"><?= ucwords($row["branch_name"]); ?></a></td>
                                <td><?= $row["machinname"]; ?></td>
                                <td><?= $row["reagent_name"]; ?></td>
                                <td><?= $row["batch_no"]; ?></td>
                                <td><?= $row["quantity"]; ?></td>
                                <?php /* <td><?= $row["used"]; ?></td> */ ?>
                                <td><?php echo $row["packet_quantity"] * $row["quantity"]; ?>&nbsp;/&nbsp;<?php echo $used_item; ?></td>
                                <td><?= $row["adminname"]; ?></td>
                                <td><?= date("d-m-Y H:i:s", strtotime($row["creteddate"])); ?></td>
                                <td>
