                ...
                <div class="box-body">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <?= ($indent_remark != '') ? $indent_remark : "NA"; ?>
                        </div>
                    </div>
                        ...
                        <table class="table-striped">
                            ...
                            <tbody id="selected_items">
                                ...
                                foreach ($poitenm as $row) {
                                    ...
                                    $itrmtotal = round($itemprice * $row["quantity"]);
                                    ...
                                    <tr id="tr_<?= $cnt; ?>">
                                        ...
                                        <td><p id="vendorcontainid_<?= $cnt; ?>">
                                                <select class="form-control" name="vendorname[]" >
                                                    ...
                                                    foreach ($vendor_list as $ven) {
                                                        ?>
                                                        <option  value="<?= $ven["id"] ?>" <?php
                                                        if ($row["vender_id"] == $ven["id"]) {
                                                            echo "selected";
                                                        }
                                                        ?>><?= $ven["vendor_name"]; ?></option>
                                                             <?php } ?>
                                                </select>
                                                <span id="" class="vendorerror" style="color:red;"></span>
                                            </p>
                                        ...
                                        <td><input type="text" name="lot[]" value="<?= $row["lot_number"]; ?>"class="form-control"/></td>
                                        ...
                                        <td><p id="testamount_<?= $cnt; ?>"><?= $itrmtotal; ?></p></td><?php $selected_item[] = $row["itemid"]; ?> 
                                        ...
                                        <td>
                                            <input type="text" id="totalamount_<?= $cnt; ?>" readonly name="totalamount[]" value="<?= $itrmtotal ?>" class="form-control"/>
                                            ...
                                            if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) !== -1 ||
                                                ...
                                                function select_item(val, id) {
                                                    ...
                                                    $.ajax({
                                                        ...
                                                        type: "GET",
                                                        dataType: "json",
                                                        data: {itenid: pm, branch_fk: '<?= $branch_fk ?>'},
                                                        success: function (data) {
                                                            ...
                                                            var quantity12 = data.available_stock;
                                                            $("#selected_items").append('<tr id="tr_' + $city_cnt + '"><td>' + $city_cnt + '</td><td><p id="vendorcontainid_' + $city_cnt + '">' + itemvendor + '</p></td><td>' + catgory + '</td><td>' + prc + '<input type="hidden" name="item[]" value="' + pm + '"></td><td><input type="text" name="lot[]" value=""class="form-control"/></td><td><input type="text" name="nos[]" id="nos_' + $city_cnt + '" value="1" class="form-control calution"/></td><td><p id="totalqty_' + $city_cnt + '">' + data.qty + '</p></td><td>' + quantity12 + '</td><td><input type="text" id="rateqty_' + $city_cnt + '" value="' + iprice + '" name="rateqty[]" class="form-control calution"/></td><td><p id="testamount_' + $city_cnt + '">0</p></td><td><input type="text" id="itemdis_' + $city_cnt + '" name="itemdis[]" value="0" class="form-control calution"/><span id="errorperdis_' + $city_cnt + '" style="color: red;"></span></td><td><p id="txtid_' + $city_cnt + '">' + itemtext + '</p></td><td><input type="text" id="totalamount_' + $city_cnt + '" readonly name="totalamount[]" class="form-control"/><span id="errortamount_' + $city_cnt + '" class="" style="color:red;"></span></td><td><a href="javascript:void(0);" onclick="delete_city_price(\'' + $city_cnt + '\',\'' + prc + '\',\'' + pm + '\',\'' + val + '\')"><i class="fa fa-trash"></i></a></td></tr>');
                                                    ...
                                                    if (check == 0) {
                                                        ...
                                                        $("select[name='vendorname[]']").each(function () {
                                                            ...
                                                            if (it1 != 0) {
                                                                ...
                                                                if (vendorid == "") {
                                                                    itemerrorvendor.push(1);
                                                                    $("#vendorcontainid_" + errid + " .vendorerror").html("Vendor field is required");
                                                                } else {
                                                                    $("#vendorcontainid_" + errid + " .vendorerror").html("");
                                                                }
                                                        ...
                                                        /*nishit LOT start*/
                                                        var itemlot = new Array();
                                                        var itemerrorlot = new Array();
                                                        var it1 = 0;
                                                        $("input[name='lot[]']").each(function () {
                                                            var lot = $(this).val();
                                                            itemlot.push($(this).val());
                                                            it1++;
                                                        });
                                                        /*END*/
                                                        ...
                                                        $("input[name='totalamount[]']").each(function (index, element) {
                                                            ...
                                                            if (parseFloat($(element).val()) >= 0) {
                                                                $("#errortamount_" + err1id).html("");
                                                            } else {
                                                                itemerrorvendor.push(1);
                                                                $("#errortamount_" + err1id).html("Invalid price in total amount");
                                                            }
                                                        ...
                                                        if (itemerrorvendor == null || itemerrorvendor == '') {
                                                            ...
                                                            $.ajax({
                                                                url: "<?php echo base_url(); ?>inventory/intent_Request/poigenerateadd/<?= $indedrequiest[0]["id"]; ?>",
                                                                                            type: "POST",
                                                                                            data: {itemvendor: itemvendor, maintotal: pricetotal, pricediscount: pricediscount, item: item, itemtext: itemtext, nosval: nosval, rateqty: rateqty, itemdis: itemdis, lot: itemlot,itemprice:itemprice},
                                                                                            success: function (data) {
                                                                            ...
                                                                            function getReagent(branchid) {
                                                                                ...
                                                                                $.ajax({
                                                                                    ...
                                                                                    type: "POST",
                                                                                    data: {branch_fk: id, selected: '<?= implode(",", $selected_item); ?>'},
                                                                                    success: function (data) {
                                                                            ...
                                                                            /*Nishit code start*/
                                                                            var all_item = document.getElementsByClassName("calution1");
                                                                            for (var i = 0; i < all_item.length; i++) {
                                                                                var itemtxt = parseFloat($('option:selected', all_item[i]).attr('rel'));
                                                                                var curow = $(all_item[i]).parent().attr("id");
                                                                                var explode = curow.split('_');
                                                                                var textid = explode[0];
                                                                                var trid = explode[1];
                                                                                var tamount1 = parseFloat($("#testamount_" + trid).html());
