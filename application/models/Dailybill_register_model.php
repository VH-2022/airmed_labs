...
Class Dailybill_register_model extends CI_Model {
...
public function get_jobreport($limit,$start,$start_date=null,$end_date=null,$branch=null,$doctor=null,$credtejobuser=null,$branchuser=null){
...
if($credtejobuser != ""){ $this->db->where_in('j.added_by',explode(",",$credtejobuser));  }
	...
	$this->db->select('j.id,j.price,ROUND(j.price * j.discount / 100) AS discount,j.payable_amount,c.full_name,j.branch_fk,br.branch_name,br.branch_code');
	$this->db->join("branch_master br","br.id=j.branch_fk","left");
...
public function get_jobreporttotal($jobid,$start_date=null,$end_date=null,$branch=null,$doctor=null,$credtejobuser=null,$branchuser=null){
	
$this->db->select('sum(j.price) as price,SUM(ROUND(j.price * j.discount / 100)) AS discount,sum(j.payable_amount) as dueamount');
	$this->db->from('job_master j');
	$this->db->where('j.status !=','0');
	$this->db->where('j.model_type','1');
...
public function getdoctorval($term){
	   
        $this->db->select('id,full_name as name');
		$this->db->from('doctor_master j');
		$this->db->where('status','1');
		$this->db->like('LOWER(full_name)',strtolower($term),"match");
		$this->db->order_by("full_name","asc");
		$query = $this->db->get();
        return $query->result_array();
    }
	public function getbranchval($term,$branchid=null){
	   
        $this->db->select('id,branch_name as name');
		$this->db->from('branch_master');
		$this->db->where('status','1');
		$this->db->like('LOWER(branch_name)',strtolower($term),"match");
		if($branchid != null){ $this->db->where_in('id',explode(",",$branchid)); }
