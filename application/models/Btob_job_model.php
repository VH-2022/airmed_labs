...
FROM
  ...
  LEFT JOIN package_master p 
    ...
    public function job_details($id) {
        $query = $this->db->query("SELECT j.branch_fk,dm.full_name as dname,c.dob,dm.mobile as dmobile,j.b2b_id,j.clinical_history,j.prescription_message,j.prescription_file,j.status,j.barcode,j.portal,j.notify_cust,j.report_status,j.discount_note,j.other_reference,j.invoice,j.note,j.sample_collection,j.booking_info,j.payment_type,j.payable_amount,j.test_city,j.id,j.order_id,j.price,j.discount,GROUP_CONCAT(distinct t.test_name SEPARATOR  '#') testname,GROUP_CONCAT(distinct p.title SEPARATOR  '@') packagename,GROUP_CONCAT(distinct p.id SEPARATOR '%') packageid,GROUP_CONCAT(t.id) testid,j.date,j.status,j.sample_collection,c.id custid ,c.full_name ,c.mobile,c.gender,c.email,c.address,c.country,c.state,c.city,c.pic,c.type,c.password FROM job_master j   LEFT JOIN job_test_list_master jtl ON jtl.job_fk=j.`id` LEFT JOIN customer_master c ON c.id=j.`cust_fk` LEFT JOIN test_master t ON t.id=jtl.test_fk LEFT JOIN  book_package_master pb 
    ON pb.job_fk = j.id 
  LEFT JOIN package_master p 
    ON p.id = pb.package_fk
...
function descitiall_branchall($perbranch = null) {
	   ...
	   if($perbranch != "") {
			...
			$this->db->where_in('assign_branch',$perbranch);
        }
