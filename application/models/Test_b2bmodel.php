...
AND `test_cities`.`status`='1'");
    ...
    function test_list($one, $two) {
        ...
        $query = $this->db->query("SELECT * FROM sample_test_master WHERE STATUS='1' ORDER BY id desc LIMIT $two,$one");
    ...
    function test_list_search($search, $city, $one, $two) {
        ...
        $query = $this->db->query("SELECT DISTINCT t.id,t.test_name,t.thyrocare_code,t.description AS city FROM  `sample_test_master` t  LEFT JOIN test_master_city_price tc ON tc.test_fk=t.id  LEFT JOIN `test_cities` c ON tc.`city_fk`=c.`id` WHERE t.status = '1' $temp ORDER BY t.id desc LIMIT $two,$one");
        return $query->result_array();
    }
function test_list_search_num($search, $city) {
        ...
        $query = $this->db->query("SELECT DISTINCT t.id,t.test_name,t.description AS city FROM `sample_test_master` t LEFT JOIN `test_master_city_price` tc ON tc.`test_fk`=t.`id` LEFT JOIN `test_cities` c ON tc.`city_fk`=c.`id` WHERE t.`status`='1' $temp ORDER BY t.id desc");
