...
class Lab_model extends CI_Model {
    ...
    function sample_list($lab_id = null, $one = null, $two = null) {
        $limit = "";
...
WHERE collect_from.id='" . $lab_id . "' AND `logistic_log`.`status`='1' ORDER BY logistic_log.id DESC" . $limit;
        $query = $this->db->query($qry);
        return $query->result_array(); 
    }

    function sample_list_num($lab_fk = null, $name = null, $barcode = null, $date = null, $from = null, $one = null, $two = null) {
        $qry = "SELECT `logistic_log`.*,`phlebo_master`.`name`,`phlebo_master`.`mobile`,`collect_from`.`name` AS `c_name` FROM `logistic_log` 
left JOIN `phlebo_master` ON `logistic_log`.`phlebo_fk`=`phlebo_master`.`id`  and `phlebo_master`.`status`='1'
INNER JOIN `collect_from` ON `logistic_log`.`collect_from`=`collect_from`.`id`
WHERE  `logistic_log`.`status`='1'";
        ...
        if (!empty($lab_fk)) {
            $qry .= " AND `collect_from`.`id` = '" . $lab_fk . "'";
        }
        if ($one != null && $two != null) {
            $qry .= " LIMIT $two,$one";
        }
    ...
    public function lab_num_rows($state_serch = null) {
    ...
    public function lab_data($state_serch = null, $limit, $start) {
