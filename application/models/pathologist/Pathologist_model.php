...
Class Pathologist_model extends CI_Model {
    ...
    function get_job_status($center = null,$serachdata) {
        $qry = "SELECT `status`,COUNT(*) AS `count`,CASE `status` WHEN '1' THEN ' Waiting For Approval' WHEN '6' THEN ' Approved' WHEN '7' THEN '  Sample Collected' WHEN '8' THEN ' Processing' WHEN '2' THEN ' Completed'  ELSE NULL  END AS 'name'FROM  job_master  WHERE `status` in (7,8,2,1,6)";
        /* 
        $start_date = date('Y-m-d', strtotime(date('Y-m-d') . ' -30 day')); 
        
        $end_date = date("Y-m-d") . " 23:59:59"; */
		$start_date =$serachdata["startdate"];
		$end_date =$serachdata["enddate"];
    ...
    function patient_data($from = null, $to = null, $type = null, $branch = null,$selectbranch=null) {
  ...
  j.`price`,
  j.`status`,c.`full_name`,c.dob,c.`mobile`,c.`gender`,j.`doctor`,b.branch_name 
FROM
  job_master j 
   
   left JOIN customer_master c ON c.id=j.`cust_fk`
   left join branch_master b on b.id=j.branch_fk
WHERE j.`status` in (7,8,2,1,6) ";
        if ($from != null) {
            ...
            $sql = $sql . " AND j.`branch_fk` = '$selectbranch'";
        }
