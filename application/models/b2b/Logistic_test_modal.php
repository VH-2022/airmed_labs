...
AND `test_cities`.`status`='1'");
   ...
   function test_list($lab_fk,$one, $two){
	   
		$query = $this->db->query("SELECT l.id,l.`price`,l.`special_price`,t.`test_name`,l.b2b_price FROM sample_test_city_price l LEFT JOIN sample_test_master t ON t.`id`=l.`test_fk` WHERE l.`status`='1' and l.lab_fk='$lab_fk'  ORDER BY l.id desc LIMIT $two,$one");
        return $query->result_array();
...
function get_val($squery) {
    ...
    function test_list_search($lab_fk,$search, $one, $two) {
        $temp = "";
        if ($search != "") {
		...
		$query = $this->db->query("SELECT l.id,l.`price`,l.`special_price`,t.`test_name`,l.b2b_price FROM sample_test_city_price l LEFT JOIN sample_test_master t ON t.`id`=l.`test_fk` WHERE l.`status`='1' and l.lab_fk='$lab_fk' $temp ORDER BY l.id desc LIMIT $two,$one");
        return $query->result_array();
    }

    function test_list_search_num($lab_fk,$search) {
        $temp = "";
        if ($search != "") {
            $temp = " AND ( ";
        }
...
//        }
        ...
        /* $query = $this->db->query("SELECT t.id,tc.`price`, tc.`special_price`,t1.`test_name` FROM `sample_test_master` t LEFT JOIN `sample_test_city_price` tc ON tc.`test_fk`=t.`id` LEFT JOIN `test_cities` c ON tc.`city_fk`=c.`id` LEFT JOIN `sample_test_master` t1 ON t1.`test_name`=t.`id` WHERE t.`status`='1' and t.lab_fk='$lab_fk' $temp ORDER BY t.test_name ASC"); */
		$query = $this->db->query("SELECT l.id,l.`price`,l.`special_price`,t.`test_name` FROM sample_test_city_price l LEFT JOIN sample_test_master t ON t.`id`=l.`test_fk` WHERE l.`status`='1' and l.lab_fk='$lab_fk' $temp ORDER BY l.id desc");
		
        return $query->num_rows();
    }
	function test_listdetils($id) {
		
      $query = $this->db->query("SELECT l.id,l.`price`,l.`special_price`,t.`test_name` FROM sample_test_city_price l LEFT JOIN sample_test_master t ON t.`id`=l.`test_fk` WHERE l.`status`='1' and l.id='$id'");
        return $query->row();
    }
	function test_detils($name) {
	  $query = $this->db->query("SELECT l.id FROM sample_test_master l WHERE l.`status`='1' AND l.test_name like '%$name%'");
        return $query->row();
    }
