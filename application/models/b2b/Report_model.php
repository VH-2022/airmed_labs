...
//        echo $qry; die();
    ...
    function branchcreditors_report($from = null, $to = null, $branch, $id) {
        ...
        $start_date = $sd . " 00:00:00";
        $end_date = $ed . " 23:59:59";
        $client_lab = $this->db->query("SELECT GROUP_CONCAT(labid) AS labid FROM `b2b_labgroup` WHERE `status`='1' AND branch_fk='" . $branch . "' GROUP BY branch_fk ");
        $client_list = $client_lab->result_array();
        $client_ids = $client_list[0]["labid"];
  ...
  am.name AS added_by,
  cb.created_date,
  sample_job_master.id as sid,
  sample_job_master.barcode_fk
FROM
  sample_creditors_master cm 
  JOIN sample_creditors_balance cb 
    ON cb.creditors_fk = cm.id 
  LEFT JOIN admin_master am 
    ON am.id = cb.created_by 
    ...
    INNER JOIN `logistic_log` ON  logistic_log.id=`sample_job_master`.`barcode_fk`
     ...
     where cm.status = '1' and cb.debit>'0' and cb.status = '1' and cm.id='$id'
  ...
  LEFT JOIN admin_master am 
    ...
    function branchcreditors_report_id($branch, $id) {
        ...
        if (!empty($client_list)) {
            ...
            am.name as added_by,
            cb.created_date
                from sample_creditors_master cm 
                join sample_creditors_balance cb on cb.creditors_fk=cm.id left 
                join admin_master am on am.id=cb.created_by 
                    INNER JOIN `sample_job_master` ON sample_job_master.id=cb.`job_id`
    INNER JOIN `logistic_log` ON  logistic_log.id=`sample_job_master`.`barcode_fk` 
                where cm.status = '1' and cb.status = '1' AND logistic_log.`collect_from` IN (" . $client_ids . ") and cb.id='$id'";
        } else {
            $qry = "select
            cm.name,cm.id,
            cm.mobile,
            ...
            cb.created_date
                ...
                join admin_master am on am.id=cb.created_by 
                    INNER JOIN `sample_job_master` ON sample_job_master.id=cb.`job_id`
    INNER JOIN `logistic_log` ON  logistic_log.id=`sample_job_master`.`barcode_fk` 
                where cm.status = '1' and cb.status = '1' AND cb.id='$id'";
        }
        $qry .= " order by cb.created_date ASC";
        //echo $qry; die();
        $query = $this->db->query($qry);
...
LEFT JOIN `job_master_receiv_amount` jmr 
 ...
 WHERE   jm.`status` != '0' ";
    ...
    function jobcollection_received_report($from = null, $to = null, $branch = null, $all_branch = null, $payment = null, $report = null) {
        ...
        if (!empty($all_branch)) {
            $client_query = $this->db->query("SELECT GROUP_CONCAT(labid) AS labid FROM `b2b_labgroup` WHERE `status`='1' AND branch_fk in (" . implode(",", $all_branch) . ") GROUP BY status ");
            $client_lab = $client_query->result_array();
        } else {
            $client_lab = array();
        }
        //print_R($client_lab); die();
        $qry = "SELECT 
  jm.order_id AS order_id,
  jm.id AS jid,
  jm.barcode_fk as bcid,
  jm.date AS added_date,
  jm.`customer_name` AS patient,
  cb1.`name` AS received_name,
  cb.name as added_name,
...
FROM
  sample_job_master_receiv_amount jr 
  LEFT JOIN sample_job_master jm 
    ON jr.job_fk = jm.id 
  LEFT JOIN admin_master cb 
    ON cb.id = jm.added_by
    LEFT JOIN admin_master cb1 
    ON cb1.id = jr.added_by 
    inner join logistic_log on jm.barcode_fk=logistic_log.id
WHERE jm.status != '0' and jr.status != '0' 
   and jr.amount!='' AND logistic_log.status='1' ";
        if ($from != "" || $to != "") {
            $qry .= " AND jr.`createddate` >= '" . $start_date . "' AND jr.`createddate` <= '" . $end_date . "'";
        }
        if (!empty($all_branch) && !empty($branch)) {
            $clab = (!empty($client_lab[0]["labid"]))?$client_lab[0]["labid"]:0; 
            $qry .= " AND logistic_log.`collect_from` IN (" . $clab . ")";
        }
        if ($payment == 'CASH') {
            ...
            $qry .= " AND jr.payment_type in (" . implode(",", array("'CASH'", "'CREDITORS'")) . ") ";
        } else if ($payment == 'CREDITORS') { 
            $qry .= " AND jr.payment_type = '" . $payment . "' ";
        } 
        $qry .= "  order by jm.id ASC";
        //echo $qry; die();
        $query = $this->db->query($qry);
        $query1 = $query->result_array();
        return $query1;
...
WHERE jm.status != '0'
AND jr.`paid`=0  
  AND jr.status = '1' AND logistic_log.status='1' ";
        if ($from != "" || $to != "") {
            $qry .= " AND jr.`created_date` >= '" . $start_date . "' AND jr.`created_date` <= '" . $end_date . "'";
        }
        if (!empty($client_lab)) {
            $qry .= " AND logistic_log.`collect_from` IN (" . $client_lab[0]["labid"] . ")";
        }
        ...
        $qry .= " group by `rc`.`name` order by jm.id ASC";
        //echo $qry; die();
        $query = $this->db->query($qry);
        $query1 = $query->result_array();
        return $query1;
...
sample_job_master_receiv_amount jr 
  ...
  LEFT JOIN admin_master rc 
    ...
    inner join logistic_log on jm.barcode_fk=logistic_log.id
WHERE jm.status != '0' and jr.status != '0' AND logistic_log.status='1' ";
        if ($from != "" || $to != "") {
            $qry .= " AND jr.`createddate` >= '" . $start_date . "' AND jr.`createddate` <= '" . $end_date . "'";
        }
        if (!empty($client_lab)) {
...
WHERE jm.status != '0' and jr.status != '0' 
  AND jm.model_type = 1 
  AND bm.status = '1' AND logistic_log.status='1' ";
        if ($from != "" || $to != "") {
            $qry .= " AND jr.`createddate` >= '" . $start_date . "' AND jr.`createddate` <= '" . $end_date . "'";
        }
        if (!empty($client_lab)) {
