...
AND `test_cities`.`status`='1'");
    ...
    function test_list($lab_fk,$one, $two) {
        $query = $this->db->query("SELECT * FROM sample_test_master WHERE STATUS='1' and lab_fk='".$lab_fk."' ORDER BY test_name ASC LIMIT $two,$one");
        return $query->result_array();
    }
    ...
    function test_list_search($lab_fk,$search, $city, $one, $two) {
        $temp = "";
        if ($search != "") {
    ...
    function test_list_search_num($lab_fk,$search, $city) {
        $temp = "";
        if ($search != "" || $city != "") {
            $temp = " AND ( ";
        }
