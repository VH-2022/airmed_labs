...
Class Phlebo_master_model extends CI_Model {
    ...
    public function phlebolist_list($srch, $one, $two) {
        ...
        $query = $this->db->query("SELECT * from phlebo_master where status IN (1,2) $temp order by id asc LIMIT $two,$one");
        $data['user'] = $query->result_array();
        return $data['user'];
    }

    public function phlebolist() {
      
        $query = $this->db->query("SELECT * from phlebo_master where status IN (1,2) order by id asc ");
