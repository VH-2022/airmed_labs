...
Class Refferringdrregister_model extends CI_Model {
...
if($credtejobuser != ""){ $this->db->where_in('j.added_by',explode(",",$credtejobuser));  }
	...
	$this->db->select('j.id,j.price,ROUND(j.price * j.discount / 100) AS discount,j.payable_amount,c.full_name,j.branch_fk,j.doctor,d.full_name as dockname,j.date');
	$this->db->join("doctor_master d","d.id=j.doctor","left");
	$this->db->order_by("j.doctor",'asc');
    $query = $this->db->get();
