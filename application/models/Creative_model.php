...
class Creative_model extends CI_Model {
  ...
  public function get_server_time() {
	...
	function get_active_record(){
        $query = $this->db->query("SELECT * FROM `creative_master` WHERE `status`='1' ORDER BY id desc");
        return $query->result_array();
    }
        function get_active_record1($one,$two){
       
    $query = $this->db->query("SELECT * FROM `creative_master` WHERE `status`='1' ORDER BY id desc LIMIT $two,$one ");
