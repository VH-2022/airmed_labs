...
Class Paymentdueregister_model extends CI_Model {
...
if($credtejobuser != ""){ $this->db->where_in('j.added_by',explode(",",$credtejobuser));  }
	...
	$this->db->select('j.id,j.date,j.price,ROUND(j.price * j.discount / 100) AS discount,j.payable_amount,c.full_name,j.branch_fk,br.branch_name,br.branch_code,a.name as operater,d.full_name as dockname');
	$this->db->join("branch_master br","br.id=j.branch_fk","left");
	$this->db->join("customer_master c","c.id=j.cust_fk","left");
	$this->db->join("doctor_master d","d.id=j.doctor","left");
