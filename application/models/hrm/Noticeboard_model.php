...
Class Noticeboard_model extends CI_Model {
    ...
    function search_num($search) {
        ...
        $query = $this->db->query("SELECT * FROM `hrm_notice_board` where status IN('1','2') and type='notice' $temp ");
    ...
    function list_search($search, $one, $two) {
        ...
        $query = $this->db->query("SELECT * FROM `hrm_notice_board` where status IN('1','2')  and type='notice' $temp  ORDER BY id desc LIMIT $two,$one");
        return $query->result();
    }
