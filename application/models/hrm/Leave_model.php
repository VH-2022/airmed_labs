...
//        $query = $this->db->query("SELECT e.*,d.name as department,ds.name as designation FROM `hrm_employees` e left join hrm_department d on d.id=e.department left join hrm_designation ds on ds.id=e.designation where e.status ='1' $temp  ORDER BY e.id desc LIMIT $two,$one");
        $query = $this->db->query("SELECT e.*,d.name as department,ds.name as designation FROM `hrm_employees` e left join hrm_department d on d.id=e.department left join hrm_designation ds on ds.id=e.designation where e.status!='3' $temp  ORDER BY e.id desc LIMIT $two,$one");
        return $query->result();
    }
    ...
    public function get_all($dtatabase, $condition, $order = null) {
        if ($order != null) {
    ...
    function get_master_get_data1($name, $condition, $order) {
        $this->db->order_by($order[0], $order[1]);
        $query = $this->db->get_where($name, $condition);
        return $query->result();
    }
