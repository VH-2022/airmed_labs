...
Class Expense_model extends CI_Model {
    ...
    function search_num($search) {
        $temp = "";
        if ($search != "") {
            $temp = " AND item_name LIKE '%" . $search . "%' ";
        }
        $query = $this->db->query("SELECT * FROM `hrm_expense` where status ='1' $temp ");
    ...
    function list_search($search, $one, $two) {
        ...
        $query = $this->db->query("SELECT * FROM `hrm_expense` where status ='1' $temp  ORDER BY id desc LIMIT $two,$one");
        return $query->result();
    }
