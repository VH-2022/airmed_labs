...
Class Award_model extends CI_Model {
    ...
    function search_num($search) {
        ...
        $query = $this->db->query("SELECT * FROM `hrm_award` where status ='1' $temp ");
    ...
    function list_search($search, $one, $two) {
        $temp = "";
        if ($search != "") {
            $temp = " AND a.name LIKE '%" . $search . "%' ";
        }
        $query = $this->db->query("SELECT a.*,e.name as awardee_name FROM `hrm_award` a left join hrm_employees e on e.employee_id=a.employee_id where a.status ='1' $temp  ORDER BY a.id desc LIMIT $two,$one");
        return $query->result();
    }
