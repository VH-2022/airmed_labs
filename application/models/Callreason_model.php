...
Class Callreason_model extends CI_Model {
    ...
    public function num_row_srch_state_list($state_serch = null) {
        ...
        $query = "SELECT * FROM call_reason_master WHERE status='1' ";

        if ($state_serch != "") {

            $query .= " AND reason like '%$state_serch%'";
        }
    ...
    public function srch_state_list($limit, $start) {
        ...
        $query = $this->db->query("SELECT * FROM call_reason_master WHERE status='1' ORDER BY id desc LIMIT $start , $limit ");
        return $query->result_array();
    }
