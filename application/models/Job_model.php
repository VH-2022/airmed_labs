...
WHERE 1=1";
  ...
  LEFT JOIN package_master p 
    ...
    {
        $start_dt = explode("/", $data["date"]);
        $strt_dat = $start_dt[2] . "-" . $start_dt[1] . "-" . $start_dt[0] . " 00:00:00";
 ...
 IF(booking_info.`family_member_fk`>0,( SELECT customer_family_master.`name` FROM customer_family_master WHERE  customer_family_master.`id`=`booking_info`.`family_member_fk`  ),c.full_name ) AS full_name ,  
  ...
  doctor_master.`doctor_ratio` AS ra,
    ...
    ON temp_package_master.job_fk = j.`id`";
        ...
        */
        ...
        if ($data["date"] != "") {
            ...
            $priviewdate = date('Y-m-d', strtotime('-1 day', strtotime($start_date)));
            ...
            if ($data["withsample"] != "") {
                $sampleIDs = $this->getsamplelist($start_date, $end_date);

                if ($sampleIDs->jobsid != '') {
                    $sampleId = $sampleIDs->jobsid;

                    $query .= "  or j.id in ($sampleId)";
                }
            }
            $query .= " )";
        }
        ...
        if ($start != null && $limit != null) {
            ...
            $query .= " AND j.model_type='1' AND j.status != '0'   GROUP BY j.id ";
        }
...
FROM
  ...
  INNER JOIN customer_master c 
    ...
    public function getsamplelist($start_date, $end_date)
    {

        $query = "SELECT GROUP_CONCAT(DISTINCT job_fk) AS jobsid FROM `job_log`  LEFT JOIN job_master ON job_master.id=job_log.`job_fk` WHERE job_status='6-7'  AND DATE_FORMAT(date_time,'%Y-%m-%d') >='$start_date'  and DATE_FORMAT(date_time,'%Y-%m-%d') <='$start_date'  AND `job_master`.`status`!=0 AND `job_log`.`status`=1 ";
        $result = $this->db->query($query);
        return $result->row();
    }
...
LEFT JOIN doctor_master dm ON dm.id=j.`doctor`	where j.id=$id GROUP BY j.id ORDER BY j.id DESC");
  ...
  c.dob,
  dm.mobile AS dmobile,
  j.model_type,
  j.collection_charge,
  j.sample_from,
  sample_from.name AS sample_from_name,
  j.clinical_history,
  j.prescription_message,
  j.prescription_file,
  j.status,
  j.barcode,
  j.portal,
...
ORDER BY j.id DESC ");
    ...
    public function num_row_srch_prescription($user = null, $date = null, $mobile = null, $status = null)
    {
        ...
        if ($user != "") {
            ...
            $query .= " AND Lower(c.full_name) like '%$user_test%'";
        }
        if ($date != "") {
            $old_date = explode('/', $date);
            $sub_date = $old_date[2] . '-' . $old_date[1] . '-' . $old_date[0];
            $new_date = $sub_date . ' 00:00:00';

            $old_date1 = explode('/', $date);
            $sub_date1 = $old_date1[2] . '-' . $old_date1[1] . '-' . $old_date1[0];
            $sub_new_date = $sub_date1 . ' 23:59:59';
            $query .= " AND p.created_date >='$new_date' AND p.created_date <='$sub_new_date' ";
            /* $query .= " AND DATE_FORMAT(p.created_date, '%d/%m/%Y')='$date'"; */
        }
        if ($mobile != "") {
    ...
    public function row_srch_prescription($user = null, $date = null, $mobile = null, $status = null, $limit, $start)
    {
        ...
        if ($date != "") {
            ...
            $old_date1 = explode('/', $date);
            $sub_date1 = $old_date1[2] . '-' . $old_date1[1] . '-' . $old_date1[0];
            $sub_new_date = $sub_date . ' 23:59:59';
    ...
    public function prescription_report($full_name = null, $mobile = null, $date = null, $status = null)
    {
        $query = "SELECT c.`full_name`,c.id cid,p.created_date AS date ,p.*  FROM `prescription_upload` p LEFT JOIN customer_master c ON c.id=p.`cust_fk` WHERE p.`status` IN (1,2) ";
        if ($full_name != "") {

            $user_test = strtolower($full_name);
            $query .= " AND Lower(c.full_name) like '%$user_test%'";
        }
        if ($date != "") {
            $old_date = explode('/', $date);

            $old_sub_date = $old_date[2] . '-' . $old_date[1] . '-' . $old_date[0];
            $new_date = $old_sub_date . ' 00:00:00';

            $old_date1 = explode('/', $date);
            $old_sub_date1 = $old_date1[2] . '-' . $old_date1[1] . '-' . $old_date1[0];
            $second_date = $old_sub_date1 . ' 23:59:59';

            $query .= " AND p.created_date >= '$new_date' AND p.created_date <= '$second_date'";
            //   $query .= " AND DATE_FORMAT(p.created_date, '%d/%m/%Y')='$date'";
        }
        if ($mobile != "") {
...
FROM
  ...
  ON customer_family_master.`id`= `booking_info`.`family_member_fk` join user_test_outsource tou on tou.job_fk = j.id join outsource_master os on tou.outsource_fk=os.id";
    ...
    public function sampleCollectedList($data = null, $limit = null, $start = null)
    {
        ...
        $query .= " WHERE 1=1";
        // $query .= " AND j.id='431425'";

        if (!empty($data["center"])) {

            $query .= " AND j.branch_fk  in (" . implode(",", $data["center"]) . ") ";
        }
        ...
        if (count($data["barcode"]) > 0) {
            $query .= " AND j.barcode NOT IN (" . implode(",", $data["barcode"]) . ")";
        }
        if ($start != '' || $limit != '') {
            $query .= " AND j.status != '0' GROUP BY j.id ORDER BY j.id DESC";
        } else {
            $query .= " AND j.model_type='1' AND j.status IN (7,8) GROUP BY j.id ORDER BY j.id DESC";
        }
 ...
 public function srch_job_list_count($data = null, $limit = null, $start = null)
    {
        return [];

        $hours168 = date('Y-m-d H:i:s', strtotime('-1080 hours'));
        $hours72 = date('Y-m-d H:i:s', strtotime('-72 hours'));
        $hours36 = date('Y-m-d H:i:s', strtotime('-36 hours'));
        $hours24 = date('Y-m-d H:i:s', strtotime('-24 hours'));
        $hours20 = date('Y-m-d H:i:s', strtotime('-20 hours'));
        $hours16 = date('Y-m-d H:i:s', strtotime('-16 hours'));
        $hours8 = date('Y-m-d H:i:s', strtotime('-8 hours'));
 ...
 and j.status=2            THEN 1 
    ...
    SUM(CASE 
      ...
      SUM(CASE 
        ...
        if ($data["date"] != "") {
            ...
            $start_date = date('Y-m-d', strtotime('-7 day', strtotime($todaydate)));
            $query .= "  AND ( j.date >='" . $start_date . " 00:00:00'";
            $query .= " AND j.date <='" . date("Y-m-d H:i:s") . "'";
            $query .= " )";
        }
        if (!empty($data["cntr_arry"])) {
            $query .= " AND j.branch_fk  in (" . implode(",", $data["cntr_arry"]) . ")";
        ...
        if ($start != '' || $limit != '') {
            $query .= " AND j.status != '0'  ORDER BY j.id DESC LIMIT $limit,$start";
        } else {
            $query .= " AND j.model_type='1' AND j.status != '0'    ORDER BY j.id DESC";
        }
        if (isset($_GET['debug']) && $_GET['debug'] == 1234) {
            echo $query;
            die();
        }
    ...
    public function srch_job_list_count22($data = null, $limit = null, $start = null)
    {
 ...
 and j.status=2            THEN 1 
    ...
    public function srch_job_list_by_hours($data = null, $limit = null, $start = null)
    {
...
FROM
  ...
  LEFT JOIN
    ...
    public function new_row_srch_hours_job_list($data = null, $limit = null, $start = null)
    {
        $query = "SELECT
  j.id,
  j.pro_id,
...
FROM
  ...
  LEFT JOIN
    ...
    ON temp_package_master.job_fk = j.`id` ";
        ...
        // Hours filter
        $hours1080 = date('Y-m-d H:i:s', strtotime('-1080 hours'));
        $hours72 = date('Y-m-d H:i:s', strtotime('-72 hours'));
        $hours36 = date('Y-m-d H:i:s', strtotime('-36 hours'));
        $hours24 = date('Y-m-d H:i:s', strtotime('-24 hours'));
        $hours20 = date('Y-m-d H:i:s', strtotime('-20 hours'));
        ...
        $hoursMap = [
            ...
            '24to36' => [$hours36, $hours24],
            '20to24' => [$hours24, $hours20],
            '16to20' => [$hours20, $hours16],
            '8to16' => [$hours16, $hours8],
            '4to8' => [$hours8, $hours4],
        ];

        if (!empty($data["hours"]) && isset($hoursMap[$data["hours"]])) {
            list($startTime, $endTime) = $hoursMap[$data["hours"]];
            $query .= " AND j.date BETWEEN '$startTime' AND '$endTime' ";
        }
