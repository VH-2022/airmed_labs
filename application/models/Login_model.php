...
Class Login_model extends CI_Model {
    ...
    function get_login_branch($id) {
        $query = $this->db->query("SELECT `user_branch`.* FROM `user_branch` INNER JOIN `branch_master` ON `branch_master`.`id`=`user_branch`.`branch_fk` AND `branch_master`.`status`='1' WHERE `user_branch`.`status`='1' AND `user_branch`.`user_fk`='" . $id . "'");
