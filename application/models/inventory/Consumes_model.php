...
Class Consumes_model extends CI_Model {

    public function manage_condition_view($relid, $one, $two) {

        $query = "SELECT * FROM `inventory_lab_consumes_master` WHERE `status`='1' ";
    ...
    public function getitem_num($srchdata = null) {
       $temp = "";
        ...
        $query = $this->db->query("SELECT it.*,bm.brand_name,tc.name,bnc.branch_name from inventory_item as it LEFT JOIN inventory_brand as bm on bm.id = it.brand_fk and bm.status='1' LEFT JOIN test_cities as tc on tc.id = it.city_fk and tc.status='1' LEFT JOIN branch_master as bnc on bnc.id = it.branch_fk and bnc.status='1' WHERE it.status ='1' and it.category_fk='2' $temp");
...
if ($srchdata['city'] != '') {
            ...
            $temp .= " AND inventory_item.city_fk = '" . $city. "'";
        }
          if ($srchdata['unit_name'] != '') {
...
if ($srchdata['brand_name'] != '') {
        ...
        $query = $this->db->query("SELECT inventory_item.*,inventory_unit_master.name as UnitName,bm.brand_name as BrandName ,tc.name,bnc.branch_name from inventory_item LEFT JOIN inventory_unit_master on inventory_unit_master.id =inventory_item.unit_fk and inventory_unit_master.status='1' LEFT JOIN inventory_brand as bm on bm.id = inventory_item.brand_fk and bm.status='1' LEFT JOIN test_cities as tc on tc.id = inventory_item.city_fk and tc.status='1' LEFT JOIN branch_master as bnc on bnc.id = inventory_item.branch_fk and bnc.status='1' WHERE inventory_item.status ='1' and inventory_item.category_fk='2' $temp  ORDER BY inventory_item.id DESC LIMIT $two,$one");
...
function getBranch($id){
    $query = $this->db->query("select * from branch_master where city ='".$id."' and status='1'");
    return $query->result_array();
}
