...
AND `test_cities`.`status`='1'");
    ...
    function autho_list($one, $two, $start_date, $end_date, $branch) {
 ...
 $temp = "";
        if (!empty($branch)) {
            $temp .= " AND j.branch_fk in (".implode(",",$branch).")";
        }

$query = $this->db->query("SELECT DISTINCT bar.`barcode`,b.id as branch_fk,b.`branch_name`,IF(
    `booking_info`.`family_member_fk` > 0,
    customer_family_master.`name`,
    c.`full_name` ) AS full_name,i.`testdate`,i.id,bar.job_fk FROM test_sample_barcode bar INNER JOIN (SELECT MAX(id) as id,lab_id, MAX(test_date_time) AS testdate FROM instument_data_storage WHERE status='N' $temp1 GROUP BY lab_id) i ON i.`lab_id`=bar.`barcode` JOIN job_master j ON j.id = bar.job_fk AND j.`status` in(7,8) JOIN branch_master b 
    ON b.id = j.branch_fk JOIN `booking_info` 
    ON `booking_info`.`id` = j.`booking_info` 
  LEFT JOIN `customer_family_master` 
    ON `customer_family_master`.`id` = `booking_info`.`family_member_fk` 
  JOIN customer_master c 
    ON c.id = j.cust_fk   WHERE bar.status='1' and j.status in (7,8) AND bar.`sample_collection`='1' $temp group by bar.id ORDER BY bar.id DESC LIMIT $two,$one ");

        return $query->result_array();
    }

    function autho_list_count($start_date, $end_date, $branch) {
...
$query = $this->db->query("SELECT DISTINCT bar.barcode FROM test_sample_barcode bar INNER JOIN (SELECT lab_id, MAX(test_date_time) AS testdate FROM instument_data_storage WHERE status='N' $temp1 GROUP BY lab_id) i ON i.`lab_id`=bar.`barcode` JOIN job_master j ON j.id = bar.job_fk AND j.`status` !='0' JOIN branch_master b ON b.id = j.branch_fk JOIN `booking_info` ON `booking_info`.`id` = j.`booking_info` WHERE bar.status='1' and j.status in (7,8) AND bar.`sample_collection`='1' $temp group by bar.id ORDER BY bar.id DESC");
...
FROM
  ...
  JOIN test_sample_barcode j1 
    ON j1.barcode = i.lab_id 
...
WHERE i.status = 'N' 
  ...
  AND `test_parameter`.`test_fk` IN (" . implode(",", $tids) . ") group by p.id
