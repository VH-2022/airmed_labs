...
Class Branch_Package_Discount_model extends CI_Model {
...
//        $query = $this->db->query("SELECT DISTINCT t.id,t.test_name,t.description,t.popular
//                AS city FROM  `test_master` t  
//                LEFT JOIN test_master_city_price tc ON tc.test_fk=t.id  
//                LEFT JOIN `test_cities` c ON tc.`city_fk`=c.`id` 
//                WHERE t.status = '1' $temp ORDER BY t.test_name ASC LIMIT $two,$one");


        $query = $this->db->query("SELECT DISTINCT t.id,t.test_name,t.description,t.popular
                AS city FROM  `test_master` t  
                INNER JOIN test_branch_price tb ON tb.test_fk=t.id  
                WHERE t.status = '1' $temp ORDER BY t.test_name ASC LIMIT $two,$one");
