...
$data["dailyCollectionData"]=$this->getDailyCollectionData(     $start ,($branch));
 ...
 foreach($data["dailyCollectionData"] as $branchData){
		...
		$output[]=array("Branch"=>$branchData['branch_name'],"Received By"=>$am_br['username']['name'],"Gross Amt"=> ($am_br["gross_amount"]=="")?"00":round($am_br["gross_amount"]),"Discount"=>($am_br["discount"]=="")?"00":round($am_br["discount"]),"Net Amt"=>($am_br["price"]=="")?"00":round($am_br["price"]),"cash"=>($am_br["cash"]=="")?"00":round($am_br["cash_total"]),"Cheque"=>($am_br["cheque_total"]=="")?"00":round($am_br["cheque_total"]),"Credit/Debit"=>($am_br["credit_total"]=="")?"00":round($am_br["credit_total"]),"Other Credit"=>($am_br["other_total"]=="")?"00":round($am_br["other_total"]),"Same Day"=>($am_br["same_day"]=="")?"00":round($am_br["same_day"]),"Back Day"=>($am_br["back_day"]=="")?"00":round($am_br["back_day"]),"Net"=>($am_br["net"]=="")?"00":round($am_br["net"]));
 }
 }
